/**
 * PanKit DevTools Extension - Panel
 *
...
function getPort() {
  if (port) return port;
  port = chrome.runtime.connect({ name: `pankit-panel-${tabId}` });
...
// Maps each anti-pattern to the most relevant PanKit skill(s) for fixing it.
...
async function formatFindingsForCopy(findings) {
  if (!findings.length) return 'PanKit found no anti-patterns on this page.';
  const url = await getInspectedUrl();
  const lines = ['# PanKit findings'];
  ...
  if (skills.length) {
    lines.push(`Suggested PanKit skills to fix: ${skills.join(', ')}`);
    lines.push('');
  }

  lines.push('---');
  lines.push('Detected by [PanKit](https://pankit.style). Skills are suggestions, not required.');
  return lines.join('\n');
}
...
async function formatSingleFindingForCopy(item, finding) {
  ...
  const lines = [`# PanKit: ${finding.name}`];
  if (url) lines.push(`URL: ${url}`);
  ...
  lines.push('');
  lines.push(`Suggested PanKit skill(s) to fix: ${fixSkillFor(finding.type)}`);
  return lines.join('\n');
}
