/**
 * PanKit DevTools Extension - Service Worker
 *
...
async function getSettings() {
  return chrome.storage.sync.get({
    ...
    autoScan: 'panel',        // 'panel' = scan when PanKit UI opens, 'devtools' = scan when DevTools opens
  });
}
...
async function sendScanToTab(tabId) {
  ...
  if (!ok) {
    ...
    const message = url.startsWith('file:')
      ? 'Can\u2019t scan local files. Enable \u201CAllow access to file URLs\u201D for PanKit in chrome://extensions.'
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  if (msg.action === 'findings' && tabId) {
    ...
    chrome.scripting.executeScript({
      ...
      // Detector will post pankit-ready, content script handles the rest
    }).catch((err) => {
      console.warn('[pankit] Fallback injection failed:', err);
    });
    sendResponse({ ok: true });
  }
...
chrome.runtime.onConnect.addListener((port) => {
  ...
  if (port.name.startsWith('pankit-devtools-')) {
    const tabId = parseInt(port.name.replace('pankit-devtools-', ''), 10);
  ...
  if (port.name.startsWith('pankit-panel-')) {
    const tabId = parseInt(port.name.replace('pankit-panel-', ''), 10);
  ...
  // Connecting the sidebar is a strong signal of "user engaged with PanKit"
  // so we trigger a scan if no findings exist yet (matches the panel port behavior).
  if (port.name.startsWith('pankit-sidebar-')) {
    const tabId = parseInt(port.name.replace('pankit-sidebar-', ''), 10);
    if (!panelPorts.has(tabId)) panelPorts.set(tabId, new Set());
    panelPorts.get(tabId).add(port);
