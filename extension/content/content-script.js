/**
 * PanKit DevTools Extension - Content Script
 *
 ...
 */
(function () {
  if (window.__PANKIT_CS_LOADED__) return;
  window.__PANKIT_CS_LOADED__ = true;
  ...
  chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
    if (msg.action === 'scan') {
      ...
      window.postMessage({ source: 'pankit-command', action: 'toggle-overlays' }, '*');
      sendResponse({ ok: true });
      ...
      window.postMessage({ source: 'pankit-command', action: 'highlight', selector: msg.selector }, '*');
      sendResponse({ ok: true });
    } else if (msg.action === 'unhighlight') {
      window.postMessage({ source: 'pankit-command', action: 'unhighlight' }, '*');
      sendResponse({ ok: true });
    }
    return true;
  });
  ...
  window.addEventListener('message', (e) => {
    ...
    if (e.data.source === 'pankit-overlays-toggled') {
      chrome.runtime.sendMessage({
  ...
  function sendScanCommand() {
    const msg = { source: 'pankit-command', action: 'scan' };
  ...
  function injectAndScan() {
    ...
    script.dataset.pankitExtension = 'true';
    pendingScan = true;
