/**
 * CLI-side reader/writer for the unified `.pankit` config.
 *
...
export function getConfigPath(root) {
  return join(root, '.pankit', 'config.json');
}

export function getLocalConfigPath(root) {
  return join(root, '.pankit', 'config.local.json');
}
...
function applyDetectionConfigSource(config, raw) {
  ...
  // so config round-trips (e.g. `pankit hooks ignore-value`) preserve it.
...
/**
 * Detector filters shared by `npx pankit detect` and the design hook.
...
const EXCLUDE_OPEN = '# pankit-config-ignore-start';
const EXCLUDE_CLOSE = '# pankit-config-ignore-end';
const EXCLUDE_PATTERNS = ['.pankit/config.local.json'];
