 ...
 *
 * Usage:
 *   npx pankit detect [file-or-dir-or-url...]
 *   npx pankit ignores <list|add-file|add-value|remove-...>
 *   npx pankit help|install|update
 *   npx pankit --help
 */
...
// Is this a detect target (the `npx pankit src/` shorthand) or a mistyped
...
async function main() {
  ...
  if (!command || command === '--help' || command === '-h') {
    console.log(`Usage: pankit <command> [options]

Commands:
  detect [file-or-dir-or-url...]   Scan for UI anti-patterns (carried over from the design toolchain)
  ignores                          Manage detector ignore rules, files, and values
  help                             List all available skills and commands
  install                          Install pankit skills into your project or global harness
...
Compatibility:
  pankit skills <command>       Legacy namespace; still supported.`);
    process.exit(0);
  }
  ...
  if (command === 'detect') {
    ...
    // Default: treat as detect arguments (allow `npx pankit src/` shorthand)
    process.argv = [process.argv[0], process.argv[1], ...args];
    const { detectCli } = await import('../engine/detect-antipatterns.mjs');
    await detectCli();
  } else if (command === 'init') {
    // The follow-up mistake from issue #472: `/pankit init` belongs in an AI
    // coding agent's chat, and a user who typed it into their shell is likely to
    // retry it here as `npx pankit init`.
    console.error(`"init" is not a CLI command. Type /pankit init in your AI coding agent's chat (Claude Code, Cursor, Codex, ...), not in this terminal.`);
    process.exit(1);
  } else {
...
main().catch(error => {
  if (error?.code === 'PANKIT_PROMPT_ABORT') {
    console.log('\nAborted.');
    process.exit(130);
  }
