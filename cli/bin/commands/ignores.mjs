...
function printUsage() {
  console.log(`Usage: pankit ignores <action> [options]

Manage detector ignores in .pankit config.
...
Scope:
  --shared                              Write .pankit/config.json (default)
  --local                               Write .pankit/config.local.json
...
Examples:
  pankit ignores add-file "src/legacy/**"
  pankit ignores add-value overused-font Inter --reason "Brand font"
  pankit ignores add-value design-system-color "*" --file "src/demo.css"
  pankit ignores remove-value overused-font Inter`);
}
...
function parseValueArgs(args, { allowUnscopedWildcard = false } = {}) {
  ...
  if (!rule || !value) throw new Error('Pass a rule id and value, e.g. pankit ignores add-value overused-font Inter');
...
function addRule(cwd, args) {
  ...
  if (!rule) throw new Error('Pass a rule id, e.g. pankit ignores add-rule side-tab');
...
function addFile(cwd, args) {
  ...
  if (!glob) throw new Error('Pass a glob, e.g. pankit ignores add-file "src/legacy/**"');
  const config = readScopeConfig(cwd, local);
...
function removeRule(cwd, args) {
  return removeFromScopes(cwd, args, (config, rest) => {
    ...
    if (!rule) throw new Error('Pass a rule id, e.g. pankit ignores remove-rule side-tab');
...
function removeFile(cwd, args) {
  return removeFromScopes(cwd, args, (config, rest) => {
    const glob = String(rest[0] || '').trim();
    if (!glob) throw new Error('Pass a glob, e.g. pankit ignores remove-file "src/legacy/**"');
...
export async function run(args = [], opts = {}) {
  ...
  if (!action) {
    throw new Error(`Unknown ignores action: ${actionArg}. Run "pankit ignores --help".`);
  }
