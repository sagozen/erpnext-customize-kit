/**
 * `pankit skills` subcommand
 *
 * Usage:
 *   pankit help      Show all available skills and commands
 *   pankit install   Install compiled skills from the universal bundle
 *   pankit link      Symlink compiled skills from a local checkout
 *   pankit update    Update skills to latest version
 */
...
import { getHookConsent, setHookConsent } from '../../lib/pankit-config.mjs';

const __dirname = dirname(fileURLToPath(import.meta.url));
const API_BASE = 'https://pankit.style';
...
const PANKIT_HOOK_COMMAND_MARKERS = [
  'skills/pankit/scripts/hook-probe.mjs',
  'skills/pankit/scripts/hook.mjs',
  'skills/pankit/scripts/hook-before-edit.mjs',
  'skills/pankit/scripts/hook-after-edit.mjs',
  'skills/pankit/scripts/hook-stop.mjs',
];
const PROVIDER_HOOK_ARTIFACTS = {
  ...
  '.github': [
    { sourceProvider: '.github', rel: 'hooks/pankit.json', destProvider: '.github' },
  ],
  ...
  '.grok': [
    { sourceProvider: '.grok', rel: 'hooks/pankit.json', destProvider: '.grok' },
  ],
};
...
class PromptAbortError extends Error {
  constructor() {
    ...
    this.code = 'PANKIT_PROMPT_ABORT';
  }
}

function isPromptAbortError(error) {
  return error?.code === 'PANKIT_PROMPT_ABORT';
}
...
async function showHelp() {
  ...
  try {
    ...
    console.error('Could not fetch command list from pankit.style. Check your network connection.');
    process.exit(1);
  }

  const pad = (s, n) => s + ' '.repeat(Math.max(0, n - s.length));

  console.log('\n  PanKit Skills & Commands\n');
  console.log('  Install:  npx pankit install');
  console.log('  Link:     npx pankit link --source=.pankit');
  console.log('  Update:   npx pankit update');
  console.log('  Docs:     https://pankit.style/cheatsheet\n');
...
/**
 * Read the skills version from the pankit SKILL.md frontmatter.
 */
function getSkillsVersion(root, scope) {
  for (const d of PROVIDER_DIRS) {
    for (const skillsDir of providerSkillsDirCandidates(root, d, scope)) {
      const skillMd = join(skillsDir, 'pankit', 'SKILL.md');
...
async function downloadAndExtractBundle() {
  const localBundle = process.env.PANKIT_BUNDLE_PATH;
  if (localBundle) return copyOrExtractLocalBundle(localBundle);

  const tmpZip = join(tmpdir(), `pankit-update-${Date.now()}.zip`);
  const tmpDir = join(tmpdir(), `pankit-update-${Date.now()}`);
...
async function copyOrExtractLocalBundle(sourceValue) {
  ...
  const tmpDir = join(tmpdir(), `pankit-local-bundle-${process.pid}-${Date.now()}`);
  mkdirSync(tmpDir, { recursive: true });
 ...
 * that aren't part of pankit). Deduplicates providers that share the same
...
async function check() {
  ...
  if (!installed) {
    console.log('PanKit is not installed in this project.');
    console.log('Run `npx pankit install` to install.');
    process.exit(0);
  }
  ...
  try {
    ...
    if (upToDate) {
      ...
      console.log('Run `npx pankit update` to update.');
    }
  } catch (e) {
...
// Check if pankit skills are already present in any provider folder
function isAlreadyInstalled(root, scope) {
  for (const d of PROVIDER_DIRS) {
    for (const skillsDir of existingSkillsDirs(root, d, scope)) {
      try {
        const entries = readdirSync(skillsDir);
        // Look for 'pankit' skill (or prefixed variant, or legacy 'teach-pankit')
        if (entries.some(e =>
          e === 'pankit' || e.endsWith('-pankit') ||
          e === 'teach-pankit' || e.endsWith('-teach-pankit')
        )) {
          return d;
        }
      } catch {}
 ...
 * prefix (default `i-`), renaming the skill to e.g. `i-pankit`. The prefix
 * only earned its keep when every command was its own skill; with a single
 * `pankit` skill it does nothing, so it is no longer offered. Rename any
 * prefixed pankit skill back to the canonical `pankit` (the fresh
 * install/update content lands there next) so users aren't left with a stale,
 * orphaned `i-pankit` alongside the new one. Scoped to the pankit skill
 * by name -- never touches third-party skills that happen to start with `i-`.
 * Returns the number of skills migrated.
 */
function migrateUnprefixPanKit(root, scope) {
  let migrated = 0;
  for (const d of PROVIDER_DIRS) {
    for (const skillsDir of existingSkillsDirs(root, d, scope)) {
      let entries;
      ...
      for (const name of entries) {
        // A prefixed pankit skill is `<prefix>pankit`, not the canonical
        // `pankit` and not an unrelated legacy skill name.
        if (name === 'pankit' || name === 'teach-pankit') continue;
        if (!name.endsWith('-pankit')) continue;
        if (!isRealSkillDir(skillsDir, name)) continue;

        const dest = join(skillsDir, 'pankit');
        try {
...
function printInstallIntro() {
  ...
  console.log(`${ui.accent(ui.bold('pankit'))} ${ui.dim('install')}`);
  console.log('');
}
...
// skills. Claude Code's live at `.claude/agents/pankit-*.md`; project
// agents take precedence over user agents. GitHub Copilot's live at
// `.github/agents/pankit-*.agent.md`:
...
// the project one, so both paths overwrite existing pankit-* copies and a
// project install reports any same-named user-level agents that would shadow
// it. Cursor's live at `.cursor/agents/pankit-*.md`, user scope
...
function reportProviderAgents(results) {
  for (const result of results || []) {
    ...
    if (result.shadowed.length > 0) {
      ...
      console.warn('Run `npx pankit update --user` to refresh them, or remove them so the project agents apply.');
    }
  }
}
...
function hookScriptRelPathForProvider(provider) {
  ...
  const rel = `${provider}/skills/pankit/scripts/${script}`;
...
function hookScriptPathForProvider(skillRoot, provider) {
  ...
  // pankit.json`) is a committed, team-shared file that the Copilot cloud
  ...
  if (provider === '.cursor') {
    return join(skillRoot, provider, 'skills', 'pankit', 'scripts', 'hook-before-edit.mjs');
  }
  if (provider === '.claude' || provider === '.agents' || provider === '.grok') {
    return join(skillRoot, provider, 'skills', 'pankit', 'scripts', 'hook.mjs');
  }
  return null;
}
...
function rewriteHookCommandsForSkillRoot(value, provider, { skillRoot, absolute }) {
  ...
  if (typeof value === 'string') {
    if (!valueHasPanKitHookMarker(value)) return value;
  ...
  if (value && typeof value === 'object') {
    ...
    if (provider === '.agents' && typeof value.command === 'string' && valueHasPanKitHookMarker(value.command)) {
...
// Whether a hook manifest file actually wires up the PanKit hook. We parse
// the JSON and scan only the `hooks` subtree (via valueHasPanKitHookMarker),
...
function fileHasPanKitHookMarker(file) {
  ...
  return valueHasPanKitHookMarker(parsed.hooks);
}
...
// PanKit marker — not mere file existence — because the target files
...
function hookInstalledForProvider(root, provider) {
  ...
  return artifacts.every(({ destProvider, rel, destRel }) => {
    const writeRel = destRel || rel;
    if (fileHasPanKitHookMarker(join(root, destProvider, writeRel))) return true;
    if (writeRel !== rel && fileHasPanKitHookMarker(join(root, destProvider, rel))) return true;
    return false;
  });
}

function valueHasPanKitHookMarker(value) {
  if (typeof value === 'string') {
    const normalized = value.replace(/\\/g, '/');
    return PANKIT_HOOK_COMMAND_MARKERS.some(marker => normalized.includes(marker));
  }
  if (Array.isArray(value)) return value.some(valueHasPanKitHookMarker);
  if (value && typeof value === 'object') {
    return Object.values(value).some(valueHasPanKitHookMarker);
  }
  return false;
}

function stripPanKitHookEntry(entry) {
  ...
  if (valueHasPanKitHookMarker(entry.command) || valueHasPanKitHookMarker(entry.args)
    || valueHasPanKitHookMarker(entry.bash) || valueHasPanKitHookMarker(entry.powershell)) {
    return null;
  }
  if (!Array.isArray(entry.hooks)) return entry;

  const strippedHooks = entry.hooks
    .map(stripPanKitHookEntry)
    .filter(Boolean);

  if (strippedHooks.length === 0 && entry.hooks.some(valueHasPanKitHookMarker)) {
    return null;
  }
...
function stripPanKitHookEntries(entries) {
...
function prunePanKitHookFromManifest(manifestPath) {
  if (!fileHasPanKitHookMarker(manifestPath)) return false;
  let parsed;
  try {
  ...
  for (const [event, entries] of Object.entries(existingHooks)) {
    const kept = stripPanKitHookEntries(entries);
...
function mergeHookManifests(existing, fresh) {
  ...
  for (const event of hookEvents) {
    const preserved = stripPanKitHookEntries(existingHooks[event]);
...
function copyProviderHooks(bundleDir, root, providers, { force = false, skillRoot = root } = {}) {
  ...
  for (const provider of targets) {
    for (const { src, dest, sharedDest } of hookArtifactsForProvider(bundleDir, root, provider)) {
      ...
      // twice per edit.
      if (sharedDest && fileHasPanKitHookMarker(sharedDest)) {
        prunePanKitHookFromManifest(dest);
        continue;
      }
...
const HOOK_EXPLAINER = [
  '',
  'PanKit can install a design hook for this project. In Claude/Codex it',
...
// first time, records the answer in .pankit/config.local.json, and never
...
function resolveLinkSource(sourceValue, root) {
  const sourcePath = sourceValue || '.pankit';
...
async function link(flags) {
  ...
  if (!yes) {
    ...
    const ans = await ask(`Link pankit skills into ${targets.length} folder(s)? (Y/n) `);
    if (ans === 'n' || ans === 'no') {
  ...
  console.log(`Linked pankit into: ${targets.join(', ')} (${parts.join(', ')}).`);
...
async function install(flags) {
  ...
  if (existing && !force && missingSelectedTargets.length < targets.length) {
    console.log(`PanKit skills are already installed (found in ${existing}/).`);
    ...
    try {
      if (linkedTargets.length > 0) {
        ...
        console.log('Update the source checkout with `git submodule update --remote`, then rerun `npx pankit link --source=.pankit` if new skills are added.');
      ...
      if (!updateCheckSkipped && copyTargets.length > 0 && !isUpToDate(installRoot, copyTargets, bundleDir, scope)) {
        migrateUnprefixPanKit(installRoot, scope);
      ...
      if (!updateCheckSkipped && missingSelectedTargets.length > 0) {
        ...
        console.log(`Installed pankit into: ${missingSelectedTargets.join(', ')} (${scope === 'user' ? 'global' : 'project'})`);
  ...
  // canonical `pankit` dir instead of orphaning the prefixed one.
  migrateUnprefixPanKit(installRoot, scope);
  ...
  console.log(`Installed pankit into: ${targets.join(', ')} (${scope === 'user' ? 'global' : 'project'})`);
  reportProviderAgents(agentResults);
  if (hookTargets.length > 0) console.log(`Installed hooks into: ${hookTargets.join(', ')}`);

  console.log('\nDone! Now type /pankit init in your AI coding agent\'s chat (not in this terminal) to set up design context.\n');
}
...
// actually holds the PANKIT skill (canonical, prefixed, or legacy teach-).
// `update` uses this so it never mistakes a repo that vendors OTHER first-party
// skills under .claude/skills for an pankit install and drops a copy in
// (issue #399, part 2).
function findPanKitProviders(root, scope) {
  const found = [];
  for (const d of PROVIDER_DIRS) {
    for (const skillsDir of existingSkillsDirs(root, d, scope)) {
      let entries;
...
function resolveUpdateTarget({ projectRoot, home, explicitScope }) {
  ...
  const projectProviders = homeRooted ? [] : findPanKitProviders(projectRoot, 'project');
  const userProviders = findPanKitProviders(home, 'user');
...
function findLinkedProviders(root, providers, scope) {
  return providers.filter(provider => {
    for (const skillsDir of providerSkillsDirCandidates(root, provider, scope)) {
      const skillDir = join(skillsDir, 'pankit');
      try {
...
async function update(flags = []) {
  ...
  // Download the latest skills directly from pankit.style.
  ...
  if (!target) {
    if (explicitScope) {
      ...
      console.log(`No pankit skill folders found at the ${where}.`);
    } else {
      console.log('No pankit skill folders found in this project or at the user level.');
    }
    console.log('Run `npx pankit install` to install first.');
    process.exit(1);
  }
  ...
  if (target.ambiguous) {
    console.log('PanKit is installed both here and at the user level:');
  ...
  try {
    ...
    // canonical `pankit` dir rather than orphaning the prefixed copy.
    const migrated = migrateUnprefixPanKit(root, scope);
    if (migrated > 0) console.log('Migrated a prefixed install back to /pankit (the i- prefix is no longer used).');
...
export async function run(args) {
  ...
  if (!sub || sub === 'help' || sub === '--help' || sub === '-h') {
    ...
    console.error(`Run 'pankit --help' for available commands.`);
    process.exit(1);
  }
}
