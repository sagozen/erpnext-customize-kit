...
// boundaries; `.pankit` is our own project marker.
const PROJECT_ROOT_MARKERS = ['.git', 'package.json', '.pankit'];
...
function resolveDesignSidecarPath(cwd = process.cwd(), contextDir = cwd) {
  const candidates = [
    path.join(cwd, '.pankit', 'design.json'),
...
// Same two groups as context.mjs's readProjectPatternGroups: PanKit
// projectRoots govern any path they match (positive or negated); package-manager
// globs only apply to paths the PanKit group does not match.
function readWorkspacePatternGroups(dir) {
  const pankit = [];
  for (const name of ['config.json', 'config.local.json']) {
    const roots = safeReadJson(path.join(dir, '.pankit', name))?.projectRoots;
    if (Array.isArray(roots)) {
      pankit.push(...roots.filter(entry => typeof entry === 'string' && entry.trim()).map(entry => entry.trim()));
    }
  }
  const pkg = [];
...
function monorepoOwnsPath(root, boundaryDir) {
  ...
  const [pankit, pkg] = readWorkspacePatternGroups(root);
  const fromPanKit = groupOwns(pankit);
  if (fromPanKit !== null) return fromPanKit;
  const fromPkg = groupOwns(pkg);
  if (fromPkg !== null) return fromPkg;
  if ([...pankit, ...pkg].some((pattern) => !normalizeWorkspacePattern(pattern).startsWith('!'))) {
    return false;
  }
...
//   - A directory carrying a project marker (.git / package.json / .pankit)
