...
export async function wrapCli() {
  ...
  if (args.includes('--help') || args.includes('-h')) {
    console.log(`Usage: pankit wrap [options]
...
The agent should insert variant HTML at insertLine.`);
  ...
  const wrapperLines = isJsx ? [
    indent + '<div data-pankit-variants="' + id + '" data-pankit-variant-count="' + count + '"' + ' ' + styleContents + '>',
    indent + '  ' + commentSyntax.open + ' pankit-variants-start ' + id + ' ' + commentSyntax.close,
    indent + '  ' + commentSyntax.open + ' Original ' + commentSyntax.close,
    indent + '  <div data-pankit-variant="original">',
    reindentOriginal('    '),
    indent + '  </div>',
    indent + '  ' + commentSyntax.open + ' Variants: insert below this line ' + commentSyntax.close,
    indent + '  ' + commentSyntax.open + ' pankit-variants-end ' + id + ' ' + commentSyntax.close,
    indent + '</div>',
  ] : [
    indent + commentSyntax.open + ' pankit-variants-start ' + id + ' ' + commentSyntax.close,
    indent + '<div data-pankit-variants="' + id + '" data-pankit-variant-count="' + count + '"' + ' ' + styleContents + '>',
...
function buildCssSelectorPrefixExamples(styleMode, count) {
  ...
  return Array.from({ length: count }, (_, i) => `[data-pankit-variant="${i + 1}"]`);
}
...
function buildCssAuthoring(styleMode, count) {
  ...
  if (styleMode.mode === 'astro-global-prefixed') {
    return {
      ...
      rulePattern: '[data-pankit-variant="N"] > .variant-class { ... }',
      selectorExamples: variantNumbers.map((n) => `[data-pankit-variant="${n}"] > .variant-class`),
      requirements: [
        ...
        'Prefix every preview selector with the matching [data-pankit-variant="N"] selector.',
  ...
  return {
    ...
    styleTag: styleMode.styleTag,
    strategy: 'scope-rule',
    rulePattern: '@scope ([data-pankit-variant="N"]) { :scope > .variant-class { ... } }',
    selectorExamples: variantNumbers.map((n) => `@scope ([data-pankit-variant="${n}"]) { :scope > .variant-class { ... } }`),
    requirements: [
      'Use @scope blocks keyed to each [data-pankit-variant="N"] wrapper.',
    ...
    forbidden: [
      'Do not use global [data-pankit-variant="N"] selector prefixes for this styleMode.',
 ...
 *
 * Only `node_modules`, `.git`, and `.pankit` are skipped outright.
...
function findElement(lines, query, tag = null) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (lines[i].includes('data-pankit-variant')) continue;
