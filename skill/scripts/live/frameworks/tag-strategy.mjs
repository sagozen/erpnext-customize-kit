 ...
 * `.pankit/live/config.json`. This module owns that block: building it,
...
export const MARKER_OPEN_TEXT = 'pankit-live-start';
export const MARKER_CLOSE_TEXT = 'pankit-live-end';

/** Markers that identify a file as still carrying our tag-strategy patch. */
export const TAG_PATCH_MARKERS = Object.freeze([MARKER_OPEN_TEXT, 'data-pankit-csp-original']);
...
export function removeTag(content, _syntax) {
  const patterns = [
    /([ \t]*)<!--\s*pankit-live-start\s*-->[\s\S]*?<!--\s*pankit-live-end\s*-->([ \t]*(?:\r\n|\n|\r|$)?)/,
    /([ \t]*)\{\/\*\s*pankit-live-start\s*\*\/\}[\s\S]*?\{\/\*\s*pankit-live-end\s*\*\/\}([ \t]*(?:\r\n|\n|\r|$)?)/,
  ];
...
// and stash the original `content` value in a `data-pankit-csp-original`
...
const CSP_MARKER_ATTR = 'data-pankit-csp-original';
