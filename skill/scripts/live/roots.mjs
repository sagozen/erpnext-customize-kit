 ...
 *   sessionRoot <appRoot>/.pankit/live — durable live state.
 *
 ...
 *
 * The resolved manifest is persisted at <appRoot>/.pankit/live/roots.json
 * plus a pointer at <repoRoot>/.pankit/live/app-root.json when the two
...
function isAppRoot(dir) {
  ...
  return hasDevConfig(dir) || exists(path.join(dir, '.pankit', 'live', 'config.json'));
}
...
export function resolveRoots({ cwd = process.cwd(), targetPath = null } = {}) {
  ...
  return {
    manifest: {
      ...
      contextRoot,
      sessionRoot: path.join(appRoot, '.pankit', 'live'),
      productPath,
      designPath,
      resolvedFrom,
    },
...
function rootsFilePath(appRoot) {
  return path.join(appRoot, '.pankit', 'live', ROOTS_FILE);
}

function pointerFilePath(repoRoot) {
  return path.join(repoRoot, '.pankit', 'live', POINTER_FILE);
}
...
function hasLiveServer(appRoot) {
  ...
  let token;
  try {
    const info = JSON.parse(fs.readFileSync(path.join(appRoot, '.pankit', 'live', 'server.json'), 'utf-8'));
...
function hasActiveDurableSession(appRoot) {
  const dir = path.join(appRoot, '.pankit', 'live', 'sessions');
  let entries;
  try {
 ...
 *
 *   1. Walk up from cwd looking for .pankit/live/roots.json.
 *   2. At the git root, follow .pankit/live/app-root.json to the app.
      ...
      if (candidates.length > 0) {
        ...
        if (tier.length > 1) {
          ...
          process.stderr.write(
            `[pankit live] Multiple apps in this repo have live state; using ${chosen}. `
...
export function enterLiveRoot(cwd = process.cwd()) {
  ...
  try {
    ...
    console.error(`[pankit live] ${err.message}`);
    process.exit(1);
  }
  ...
  if (path.resolve(cwd) !== path.resolve(appRoot)) {
    ...
    // to guess.
    if (!isDir(appRoot)) {
      console.error(`[pankit live] resolved app root does not exist: ${appRoot} (stale roots manifest? re-run the live boot, or pass --target <path>)`);
      process.exit(1);
    }
    try {
      process.chdir(appRoot);
    } catch (err) {
      console.error(`[pankit live] could not enter app root ${appRoot}: ${err.message}`);
      process.exit(1);
    }
  }
