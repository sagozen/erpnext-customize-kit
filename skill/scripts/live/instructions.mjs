...
function generateInstructions(event, scriptsPath) {
  ...
  steps.push(event.action && event.action !== 'pankit'
...
function insertScaffoldInstructions(event, scriptsPath) {
  ...
  if (scaffold?.previewMode === 'svelte-component') {
    return `${base} Write each inserted variant as a single-root Svelte component under ${scaffold.componentDir} (no data-pankit-* attributes, CSS in each component's <style>). Never edit the route during generation; reply with --file ${scaffold.file}.`;
  }
...
function acceptInstructions(event, scriptsPath) {
  ...
  if (result.handled === true && result.carbonize === true) {
    return `${prefix}Carbonize cleanup is REQUIRED now, before the next poll, in ${result.file}: (1) locate the pankit-carbonize-start/end block and read the pankit-param-values comment; (2) move the CSS rules into the stylesheet that owns this area; (3) bake params while rewriting selectors (@scope wrappers to semantic classes, keep only the chosen data-p branch, substitute range literals); (4) unwrap the accepted content and drop every data-pankit-* / data-p-* attribute; (5) delete the inline <style>, the param-values comment, and both markers plus dead @scope rules. Then run node ${scriptsPath}/live-complete.mjs --id ${event.id} and verify phase "completed"; it refuses with source_dirty while leftovers remain. Poll again only after that.`;
  }
  ...
  return `${prefix}No mechanical accept result; read ${result.file || 'the session source file'}, find the pankit markers, and finish the merge by hand. Poll again after.`;
}
