#!/usr/bin/env node
/**
 * PanKit design hook — Cursor preToolUse write gate.
 *
...
async function detectProposedHtml(detector, content, filePath, scanOptions) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'pankit-pre-'));
...
const BLOCK_PREFIX = 'PanKit design hook blocked this write before it landed. ';
...
function cursorBlockMessage(findings, filePath, config, cwd, footerMode, reserveChars) {
  ...
  return rendered.replace(
    '[pankit@1] Design hook findings requiring review',
    `[pankit@1] ${BLOCK_PREFIX}Design hook findings requiring review`,
  );
}
...
async function main() {
  if (truthy(process.env.PANKIT_HOOK_DISABLED)) {
  ...
  if (denial.count > EDIT_COUNT_THRESHOLD) {
    const warning = `${message}\n\nThis is the ${denial.count}th repeated denial for the same file and finding signature, so PanKit is allowing this write to avoid a loop. Reconsider the issue immediately after the tool runs.`;
    return allow({
      ...audit,
      findings: (findings || []).length,
...
main().catch((err) => {
  if (process.env.PANKIT_HOOK_DEBUG) {
    process.stderr.write(`[pankit-hook-before-edit] ${err}\n`);
  }
