 ...
 * browser is available (PANKIT_QUESTION_DISABLED, or a detected
 * CI/headless/remote environment; PANKIT_QUESTION_FORCE=1 overrides
 ...
 *       "comp": ".pankit/mocks/decision/assigned.webp",  // optional; the card's
...
if (process.env.PANKIT_QUESTION_DISABLED) {
...
if (wantsBrowser && !process.env.PANKIT_QUESTION_FORCE) {
  const headless =
    process.env.CI ||
    (process.env.SSH_CONNECTION && !process.env.DISPLAY) ||
    (process.platform === 'linux' && !process.env.DISPLAY && !process.env.WAYLAND_DISPLAY);
  if (headless) {
    console.log('serve-question: no browser detected in this environment (CI/headless/remote); use the structured question tool instead. Set PANKIT_QUESTION_FORCE=1 to serve anyway.');
    process.exit(2);
  }
}
  ...
  try {
    ...
    if (a.buildPath === 'comp' || a.buildPath === 'code') {
      ...
      const origin = a.buildPathFlipped
        ? 'flipped on the page, so it binds this session only, and the page never writes it back; the sole exception is new-work’s one-time offer, on a project that had no recorded default at all, which asks after the round closes and writes the answer to .pankit/config.json'
...
const QUESTION_DIR = path.join(process.cwd(), '.pankit', 'questions');
...
function page(waiting = false) {
  ...
  const anatomy = (option) => {
    ...
    if (Array.isArray(option.raised) && option.raised.length) {
      ...
      const raisesHead = (count) => `<div class="raises-head"><span class="fact-label">Improved by PanKit's worlds</span>${count > 1 ? `<span class="raises-count" data-raises-count>1/${count}</span>` : ''}</div>`;
      if (raiseLines.length > 1) {
        rows.push(`<div class="raises raises-cycle" role="button" tabindex="0" title="Click or press Enter for the next improvement" aria-label="How PanKit's worlds improved this direction; activate to see the next improvement">
...
<title>${esc(payload.title || 'pankit · decision')}</title>
...
<style>
  /* Neo kinpaku tokens, mirrored from pankit.style kinpaku-tokens.css */
  :root {
    color-scheme: dark;
...
<header>
  <div class="brand">
    ...
    <span class="wordmark">PanKit</span>
  </div>
</header>
<main>
  <div class="stage">
