#!/usr/bin/env node
/**
 * Deep staleness pass over PanKit's own project artifacts.
 *
...
function usage() {
  return [
    ...
    '',
    "Report drift between this project's PanKit artifacts and what the",
    'installed version reads: PRODUCT.md, DESIGN.md and its sidecar,',
    '.pankit/config.json, surface briefs, and the design hook.',
    '',
...
function readProjectRootPatterns(repoRoot) {
  ...
  for (const name of ['config.json', 'config.local.json']) {
    try {
      const raw = JSON.parse(fs.readFileSync(path.join(repoRoot, '.pankit', name), 'utf-8'));
...
function renderText(report, fixes) {
  ...
  lines.push(`PanKit doctor: ${rel(report.projectRoot, process.cwd()) || '.'}`);
  ...
  if (fixes) {
    ...
    lines.push(`Run \`node doctor.mjs --fix\` to apply the automatic migrations, `
      + `or \`${PANKIT_COMMAND} doctor\` to work through all of them.`);
  }
...
if (invokedAsScript()) {
  cli().catch((err) => {
    process.stderr.write(`pankit doctor failed: ${err?.message || err}\n`);
    process.exit(1);
  });
}
