...
3. If `<action>` is `off`, follow up with a one-line note: "Done. New edits will not trigger the design hook in this project until you run `{{command_prefix}}pankit hooks on`."
...
- If the user asks to "disable the hook" globally, lead with `{{command_prefix}}pankit hooks off` (persistent for this project; writes `hook.enabled: false` to config). The legacy `PANKIT_HOOK_DISABLED=1` env var also works as a one-shot override that follows the shell.
