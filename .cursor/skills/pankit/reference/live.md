...
1. `live.mjs`: boot. If the request names or implies a file, route, or app inside a monorepo, infer the concrete path and run `node .cursor/skills/pankit/scripts/live.mjs --target <path>` instead; then run the rest of this live session from the returned `projectRoot`. The boot resolves the app root from dev-server config files and persists it in `.pankit/live/roots.json`; every helper re-anchors to that manifest at startup (a wrong cwd cannot fork session state), PRODUCT.md / DESIGN.md are discovered upward to the git root, and relative helper args like `--file` resolve against the app root.
...
```
LOOP:
  node .cursor/skills/pankit/scripts/live-poll.mjs   # default long timeout; no --timeout=
...
```bash
node .cursor/skills/pankit/scripts/live.mjs
```
...
```bash
node .cursor/skills/pankit/scripts/live-status.mjs      # helper state, active sessions, queued events; works with the helper down
node .cursor/skills/pankit/scripts/live-resume.mjs --id SESSION_ID   # active snapshot, pending event, next safe action
node .cursor/skills/pankit/scripts/live-complete.mjs --id SESSION_ID # canonical manual final acknowledgement after verified cleanup
```
...
```bash
node .cursor/skills/pankit/scripts/live-insert.mjs --id EVENT_ID --count EVENT_COUNT --position after \
...
```bash
node .cursor/skills/pankit/scripts/live-wrap.mjs --id EVENT_ID --count EVENT_COUNT --element-id "ELEMENT_ID" --classes "class1,class2" --tag "div" --text "TEXT_SNIPPET"
```
...
```bash
node .cursor/skills/pankit/scripts/live-poll.mjs --reply EVENT_ID done --file RELATIVE_PATH
```
...
```bash
node .cursor/skills/pankit/scripts/live-server.mjs stop
```
