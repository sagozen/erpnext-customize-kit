 ...
 *   1. Check .pankit/live/config.json (returns config_missing if first-ever run)
 ...
 *   - Open the project's live dev/preview URL in the browser (optional, if browser automation exists)—not `serverPort`; that port is the PanKit helper for /live.js and /poll
...
import { readLiveServerInfo } from './lib/pankit-paths.mjs';
...
Prepare everything for live variant mode in a single command:
  - Checks .pankit/live/config.json (required, created once per project)
...
The agent should then:
  ...
  if (targetSelection) {
    console.log(JSON.stringify({
      ...
      ...targetSelection,
      hint: 'Ask the user which app PanKit should use, then rerun live from that child app cwd. Use --target <path> only as a fallback or explicit path diagnostic.',
    }, null, 2));
    process.exit(0);
  }
  ...
  try {
    // Briefs live under .pankit/surfaces, which in a nested-app repo sits
