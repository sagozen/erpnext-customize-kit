...
export { PANKIT_COMMAND_PREFIX } from './provider.mjs';

export const PANKIT_DIR = '.pankit';
export const LIVE_DIR = 'live';
export const CRITIQUE_DIR = 'critique';

export function getPanKitDir(cwd = process.cwd(), options = {}) {
  return path.join(resolveProjectRoot(cwd, options), PANKIT_DIR);
}

export function getDesignSidecarPath(cwd = process.cwd(), options = {}) {
  return path.join(getPanKitDir(cwd, options), 'design.json');
}
...
export function getLiveDir(cwd = process.cwd(), options = {}) {
  return path.join(getPanKitDir(cwd, options), LIVE_DIR);
}
...
export function resolveLiveConfigPath({ cwd = process.cwd(), scriptsDir, env = process.env, targetPath } = {}) {
  if (env.PANKIT_LIVE_CONFIG && env.PANKIT_LIVE_CONFIG.trim()) {
    const configured = env.PANKIT_LIVE_CONFIG.trim();
...
export function getLegacyLiveServerPath(cwd = process.cwd(), options = {}) {
  return path.join(resolveProjectRoot(cwd, options), '.pankit-live.json');
}
 ...
 * `.pankit/live/`. Real IDs are 8 hex chars; the tests use short slugs.
 */
...
export function getLegacyLiveSessionsDir(cwd = process.cwd(), options = {}) {
  return path.join(resolveProjectRoot(cwd, options), '.pankit-live', 'sessions');
}
...
export function getCritiqueDir(cwd = process.cwd(), options = {}) {
  return path.join(getPanKitDir(cwd, options), CRITIQUE_DIR);
}

export function getLegacyLiveAnnotationsDir(cwd = process.cwd(), options = {}) {
  return path.join(resolveProjectRoot(cwd, options), '.pankit-live', 'annotations');
}
