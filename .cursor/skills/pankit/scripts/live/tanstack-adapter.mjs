 ...
 *
 * The managed component lives OUTSIDE `src/routes/` (in `src/pankit/`) so
...
export const TANSTACK_MARKER_OPEN = '{/* pankit-live-tanstack-start */}';
export const TANSTACK_MARKER_CLOSE = '{/* pankit-live-tanstack-end */}';
export const TANSTACK_COMPONENT_DIR = 'src/pankit';
export const TANSTACK_COMPONENT_BASENAME = 'PanKitLiveRoot';
...
export function applyTanStackLiveAdapter({ cwd = process.cwd(), port, token, project = detectTanStackStartProject(cwd) } = {}) {
  ...
  if (componentExisted && !isManagedComponent(fs.readFileSync(componentAbs, 'utf-8'))) {
    // A non-PanKit file already sits at our managed path — refuse to clobber.
    return {
      file: project.componentFile,
      error: 'tanstack_component_conflict',
      hint: `${project.componentFile} already exists and is not managed by PanKit Live`,
    };
  }
...
export function patchTanStackRoot(content, componentImport) {
  ...
  const importStatement = `import PanKitLiveRoot from '${componentImport}';`;
...
export function unpatchTanStackRoot(content) {
  ...
  out = out.replace(
    new RegExp("^import PanKitLiveRoot from '[^']*';[ \\t]*\\r?\\n", 'gm'),
    '',
  );
  return out;
}
...
export function buildTanStackLiveRootComponent(port, token) {
  ...
  return `/* pankit-live-tanstack-start */
import { useEffect } from 'react';

const LIVE_SRC = '${liveSrc}';
const LIVE_SELECTOR = 'script[data-pankit-live-tanstack]';

// Dev-only mount for PanKit Live. TanStack Start server-renders the root
...
export default function PanKitLiveRoot() {
  useEffect(() => {
    ...
    script.src = LIVE_SRC;
    script.async = true;
    script.setAttribute('data-pankit-live-tanstack', '');
    script.setAttribute('data-pankit-live-script', 'true');
...
// The managed mount component carries the `pankit-live-tanstack` marker in
// its leading comment and its script data-attribute; user files never do.
function isManagedComponent(content) {
  return String(content || '').includes('pankit-live-tanstack');
}
