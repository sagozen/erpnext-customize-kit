...
function printUsage() {
  console.log(`Usage: pankit detect [options] [file-or-dir-or-url...]
...
Options:
  ...
  --no-inline-ignores Do not honor in-file pankit-disable* ignore comments
  --no-design-system  Do not load local DESIGN.md / .pankit/design.json context
...
Project config:
  Respects .pankit/config.json and .pankit/config.local.json detector
  settings: detector.ignoreRules, detector.ignoreFiles, detector.ignoreValues,
  and detector.designSystem.enabled.

Inline ignores:
  In-file comments waive a finding where it lives and travel with the file:
    <!-- pankit-disable overused-font -- exported brand doc -->
    .brand { font-family: Inter } /* pankit-disable-line overused-font */
    // pankit-disable-next-line bounce-easing: intentional bounce
  pankit-disable applies to the whole file; -line / -next-line are scoped.
...
Examples:
  pankit detect src/
  pankit detect index.html
  pankit detect https://example.com
  pankit detect --json .
  pankit detect --no-config src/`);
}
...
async function detectCli() {
  ...
  // Inline `pankit-disable*` waivers are part of the scanned file, so they
            ...
            if (fwConfig) {
              ...
              if (probe.listening && probe.matched) {
                process.stderr.write(
                  ...
                  `  npx pankit detect http://localhost:${fwConfig.port}\n\n`
                );
