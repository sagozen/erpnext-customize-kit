 ...
 * service repo, evals, and tests set PANKIT_CATALOG_DIR), then the roll
 * API at pankit.style, then a degraded assignment-only seed when both are
 ...
 * never leave the machine. DO_NOT_TRACK or PANKIT_NO_TELEMETRY disables
 * the ping entirely.
 *
 * Env vars:
 *   PANKIT_CONCEPT_SEED — same as --from; for reproducible eval runs.
 *   PANKIT_CATALOG_DIR  — directory holding the four catalog JSON files.
 *   PANKIT_API_URL      — roll API base (default https://pankit.style/api).
 *   PANKIT_NO_TELEMETRY — disables the choice ping (DO_NOT_TRACK also honored).
 */
...
// tests point PANKIT_CATALOG_DIR at one), then the roll API, then a
// degraded assignment-only seed. The full catalog does not ship with the skill.
const CATALOG_DIR = process.env.PANKIT_CATALOG_DIR || here;
const API_BASE = (process.env.PANKIT_API_URL || 'https://pankit.style/api').replace(/\/$/, '');
const API_TIMEOUT_MS = Number(process.env.PANKIT_API_TIMEOUT || 4000);
...
function requireLocalConcepts() {
  ...
  if (!local) {
    throw new Error('concept-seed: no local catalog (set PANKIT_CATALOG_DIR or pass sourceConcepts)');
  }
  return local;
}
...
function telemetryDisabled() {
  return Boolean(process.env.PANKIT_NO_TELEMETRY || process.env.DO_NOT_TRACK);
}
...
const CARD_BASE = process.env.PANKIT_CARD_BASE || 'https://pankit.style/worlds/cards';
...
export function renderConceptSeed({
  scope = 'surface',
  key = process.env.PANKIT_CONCEPT_SEED || crypto.randomBytes(4).toString('hex'),
  reroll = 0,
  register = null,
  mode = null,
  grain = null,
  ...
  it is not the assigned direction, kicker PANKIT’S PICK, with an honest risk line
  ...
  comp-led default (.pankit/config.json buildPath; the page toggle
...
network contact is one GET to https://pankit.style/api/roll whose query
...
`;
  ...
  // widens it. PANKIT_COMPOSITIONS=1 re-enables rendering for catalog
  // development; the draw machinery, axes, and grain report stay intact.
  const compositionsEnabled = process.env.PANKIT_COMPOSITIONS === '1';
  ...
  try {
    if (chosenIdx !== -1 || kindIdx !== -1) {
      ...
      process.stdout.write(await renderConceptSeed({
        ...
        key: fromIdx !== -1
          ? args[fromIdx + 1]
          : (process.env.PANKIT_CONCEPT_SEED || crypto.randomBytes(4).toString('hex')),
