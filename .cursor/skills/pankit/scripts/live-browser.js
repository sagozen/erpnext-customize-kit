/**
 * PanKit Live Variant Mode - Browser Script
 *
 * Injected into the user's page via <script src="http://localhost:PORT/live.js">.
 * The server prepends window.__PANKIT_TOKEN__ and window.__PANKIT_PORT__
 * before this code.
 *
...
(function () {
  ...
  if (window.__PANKIT_LIVE_INIT__) return;
  window.__PANKIT_LIVE_INIT__ = true;

  const TOKEN = window.__PANKIT_TOKEN__;
  const PORT = window.__PANKIT_PORT__;
  const APP_ROOT = window.__PANKIT_APP_ROOT__ || null;
  if (!TOKEN || !PORT) {
    window.__PANKIT_LIVE_INIT__ = false; // reset so the real load can init
    return;
  }
  ...
  const PREFIX = 'pankit-live';
  const PANKIT_COMMAND = (window.__PANKIT_COMMAND_PREFIX__ || '/') + 'pankit';
  const PICK_CURSOR_STYLE_ID = PREFIX + '-pick-cursor-style';
  const MANUAL_APPLY_STATE_TTL_MS = 15 * 60 * 1000;
  const sessionState = window.__PANKIT_LIVE_SESSION__?.createLiveBrowserSessionState({
    prefix: PREFIX,
    storage: localStorage,
    idFactory: () => crypto.randomUUID().replace(/-/g, '').slice(0, 8),
  });
  if (!sessionState) {
    console.error('[pankit] live-browser-session.js was not loaded. Live mode cannot start safely.');
    window.__PANKIT_LIVE_INIT__ = false;
    return;
  }
  ...
  // window.__PANKIT_VOCAB__ when it serves /live.js (same injection path as
  ...
  const VOCAB = Array.isArray(window.__PANKIT_VOCAB__) ? window.__PANKIT_VOCAB__ : [];
  const ICONS = {};
  ...
  // script, so it cannot import that module; the private pankit-site repo
  ...
  const LIVE_CHROME_MOUNT_CONTRACT = Array.isArray(window.__PANKIT_LIVE_MOUNT_CONTRACT__)
    ? window.__PANKIT_LIVE_MOUNT_CONTRACT__
    : ['root', 'transport', 'state', 'actions'];
  const LIVE_UI_SURFACES = Array.isArray(window.__PANKIT_LIVE_UI_SURFACES__)
    ? window.__PANKIT_LIVE_UI_SURFACES__
    : [];
  ...
  const SCROLL_ANCHOR_LOCK_ID = 'pankit-scroll-anchor-lock';
  const VARIANT_STATE_STYLE_ID = 'pankit-variant-state';
  const DISCARD_STATE_STYLE_ID = 'pankit-discard-state';
  ...
  const domHelpers = window.__PANKIT_LIVE_DOM__?.createLiveBrowserDomHelpers({
    prefix: PREFIX,
    skipTags: SKIP_TAGS,
    document,
  });
  if (!domHelpers) {
    console.error('[pankit] live-browser-dom.js was not loaded. Live mode cannot start safely.');
    window.__PANKIT_LIVE_INIT__ = false;
    return;
  }
  const {
  ...
  window.__PANKIT_LIVE_CHROME_CORE__ = {
    version: 1,
    adapter: window.__PANKIT_LIVE_ADAPTER__ || 'dom',
  ...
  function showHighlight(el) {
    ...
    if (el.hasAttribute?.('data-pankit-insert-placeholder')) return;
  ...
  function onAnnotDown(e) {
    ...
    const resizeEdge = e.target.closest?.('[data-pankit-placeholder-resize]')?.dataset.pankitPlaceholderResize;
  ...
  function stripManualEditRuntimeState(root) {
    ...
    const nodes = [root, ...root.querySelectorAll('[data-pankit-editable], [data-pankit-original-text], [data-pankit-text-wrap]')];
    for (const node of nodes) {
      const runtimeEditable = node.hasAttribute('data-pankit-editable')
        || node.hasAttribute('data-pankit-original-text');
      node.removeAttribute('data-pankit-editable');
      node.removeAttribute('data-pankit-original-text');
      node.removeAttribute('data-pankit-text-wrap');
      if (runtimeEditable) {
  ...
  function selectionTagLabel(el) {
    if (!el) return '';
    if (el.hasAttribute?.('data-pankit-insert-placeholder')) return 'slot';
  ...
  function applyPlaceholderSizingStyles(placeholder, sizing) {
    placeholder.dataset.pankitPlaceholderWidth = sizing.kind;
  ...
  function materializePlaceholderWidth(placeholder) {
    ...
    const kind = placeholder.dataset.pankitPlaceholderWidth;
    ...
    placeholder.dataset.pankitPlaceholderWidth = 'explicit';
  }
  ...
  function setLiveState(next) {
    state = next;
    window.__PANKIT_LIVE_STATE__ = next;
    syncPageInteractionCursor();
  ...
  function resolveBarAnchor() {
    ...
    if (currentSessionId && (state === 'GENERATING' || state === 'CYCLING')) {
      const wrapper = document.querySelector('[data-pankit-variants="' + currentSessionId + '"]');
      if (wrapper) {
        const variantCount = wrapper.querySelectorAll('[data-pankit-variant]:not([data-pankit-variant="original"])').length;
  ...
  function isInsertGeneratingSession() {
    ...
    const wrapper = document.querySelector('[data-pankit-variants="' + currentSessionId + '"]');
    return !!wrapper && wrapper.dataset.pankitMode === 'insert';
  }
  ...
  function buildPlaceholderResizeHandles() {
    ...
    for (const spec of specs) {
      ...
      handle.dataset.pankitPlaceholderResize = spec.edge;
  ...
  function createInsertPlaceholder(anchor, position, layoutAxis) {
    ...
    placeholder.setAttribute('data-pankit-insert-placeholder', 'true');
  ...
  function ensureConfigureInputStyle() {
    ...
    s.textContent =
      '@keyframes pankit-configure-voice-pulse { 0%, 100% { opacity: 0.55; } 50% { opacity: 1; } }' +
      ...
      '#' + PREFIX + '-configure-voice[data-listening="true"] svg, #' + PREFIX + '-insert-voice[data-listening="true"] svg { animation: pankit-configure-voice-pulse 1.1s ease-in-out infinite; }' +
    ...
    if (hasParams || paramsPending) {
      ...
      if (paramsPending) {
        const spinner = el('span', {
          ...
          animation: 'pankit-spin 0.6s linear infinite',
  ...
  function recoverEmptyCycling(reason) {
    ...
    try {
      console.warn('[pankit] Refusing to render empty variant cycling state:', reason);
  ...
  // variant wrapper:
  //
  //   <div data-pankit-variant="1"
  //        data-pankit-params='[{"id":"density","kind":"steps",...}]'>
  //
  ...
  function getVisibleVariantEl() {
    ...
    return wrapper.querySelector('[data-pankit-variant="' + visibleVariant + '"]');
  }

  function parseVariantParams(variantEl) {
    // Svelte component variants can't carry a `data-pankit-params` attribute:
    ...
    if (!variantEl) return [];
    const raw = variantEl.getAttribute('data-pankit-params');
    if (!raw) return [];
    try {
      const parsed = JSON.parse(raw);
      return Array.isArray(parsed) ? parsed : [];
    } catch (err) {
      console.warn('[pankit] Invalid data-pankit-params JSON:', err.message);
      return [];
    }
  }
  ...
  function applyParamValue(variantEl, param, value) {
    ...
    // [data-pankit-component-mount] with no [data-pankit-variant="N"]
  ...
  function wrapMixedContentTextNodes(rootEl) {
    ...
    if (hasText && hasElement) {
      for (const node of children) {
        if (node.nodeType === 3 && /\S/.test(node.nodeValue || '')) {
          ...
          wrap.dataset.pankitTextWrap = 'true';
    ...
    for (const child of Array.from(rootEl.children)) {
      if (!child.dataset || !child.dataset.pankitTextWrap) {
  ...
  function unwrapMixedContentTextNodes(rootEl) {
    if (!rootEl || rootEl.nodeType !== 1) return;
    const wraps = rootEl.querySelectorAll('[data-pankit-text-wrap="true"]');
  ...
  function enableInlineEdit(targetEl) {
    ...
    for (const row of rows) {
      ...
      row.el.dataset.pankitEditable = 'true';
      row.el.dataset.pankitOriginalText = row.text;
  ...
  function disableInlineEdit(opts = {}) {
    for (const row of inlineEditRows) {
      ...
      delete row.el.dataset.pankitEditable;
      delete row.el.dataset.pankitOriginalText;
  ...
  function restoreInlineEditDrafts() {
    for (const row of inlineEditRows) {
      if (inlineEditDrafts.has(row.el)) {
        row.el.textContent = row.el.dataset.pankitOriginalText;
      }
    }
  }
  ...
  function documentRefClassSuffix(el) {
    ...
    for (const cls of el.classList) {
      if (!cls || cls.indexOf('pankit-') === 0) continue;
  ...
  function copyEditLeafContext(el, originalText, newText) {
    ...
    return {
      ...
      id: el.id || null,
      classes: el.classList ? [...el.classList].filter((cls) => cls.indexOf('pankit-') !== 0) : [],
      originalText,
      newText,
      textContent: (el.textContent || '').slice(0, 500),
  ...
  function nearbyEditableTextsForManualEdit(rows, activeEl, originalText, newText) {
    ...
    for (const row of rows || []) {
      ...
      out.push({
        ...
        classes: row.el?.classList ? [...row.el.classList].filter((cls) => cls.indexOf('pankit-') !== 0) : [],
        text,
      });
  ...
  async function applyEditing() {
    ...
    try {
      ...
      console.error('[pankit] manual edit stash failed:', err);
  ...
  function ensureSpinKeyframes() {
    ...
    style.textContent = '@keyframes pankit-spin { to { transform: rotate(360deg); } }';
    uiAppendStyle(style);
  }
  ...
  async function fetchPendingCount() {
    try {
      ...
      console.warn('[pankit] failed to fetch pending count:', err);
    }
  }
  ...
  async function onPendingPillClick() {
    ...
    try {
      ...
      if (result.failed && result.failed.length > 0) {
        console.warn('[pankit] some copy edits failed:', result.failed);
        ...
        if (n > 0) {
          ...
          console.warn('[pankit] apply returned no verified edits:', result);
          showToast('No edits applied - see console', 4000);
        }
      }
    } catch (err) {
      console.error('[pankit] commit failed:', err);
  ...
  async function onPendingTrashClick() {
    ...
    try {
      ...
      console.error('[pankit] discard failed:', err);
  ...
  async function onPendingKeepFixingClick() {
    ...
    try {
      ...
      console.error('[pankit] repair retry failed:', err);
  ...
  async function onPendingRollbackClick() {
    ...
    try {
      ...
      console.error('[pankit] manual Apply rollback failed:', err);
  ...
  function restoreDiscardedManualEdits(entries) {
    ...
    if (failures > 0) {
      console.warn('[pankit] skipped unsafe copy edit DOM restore for', failures, 'edit(s). Refresh to reset the page DOM.');
    }
    return failures;
  }
  ...
  function mixedTextWrapRestoreHint(el) {
    if (!el || !el.dataset || el.dataset.pankitTextWrap !== 'true' || !el.parentElement) return null;
  ...
  function directMixedTextRestoreNodes(parent) {
    return Array.from(parent?.childNodes || []).filter((node) => {
      ...
      return node.nodeType === 1
        && node.dataset
        && node.dataset.pankitTextWrap === 'true'
  ...
  function syncEditBadgeHitProxies() {
    ...
    for (const target of targets) {
      ...
      if (!proxy) {
        ...
        proxy.type = 'button';
        proxy.tabIndex = -1;
        proxy.dataset.pankitEditBadgeProxy = 'true';
  ...
  function initEditBadge() {
    ...
    if (!uiGetById(PREFIX + '-edit-badge-focus-style')) {
      ...
      s.textContent =
        ...
        '[data-pankit-editable="true"] { outline: none !important; box-shadow: none !important; }' +
        '[data-pankit-editable="true"]:focus { outline: none !important; box-shadow: none !important; }' +
        '[data-pankit-editable="true"]:focus-visible { outline: none !important; box-shadow: none !important; }';
      uiAppendStyle(s);
    }
  }
  ...
  function mountedParameterCount() {
    ...
    const wrapper = document.querySelector('[data-pankit-variants="' + currentSessionId + '"]');
    if (!wrapper) return 0;
    return [...wrapper.querySelectorAll('[data-pankit-variant]:not([data-pankit-variant="original"])')]
  ...
  async function showVariantInDOM(sessionId, num) {
    ...
    const wrapper = document.querySelector('[data-pankit-variants="' + sessionId + '"]');
  ...
  function parseOriginalMarkupElement(originalMarkup) {
    ...
    const doc = parser.parseFromString('<div id="pankit-anchor">' + originalMarkup + '</div>', 'text/html');
    return doc.getElementById('pankit-anchor')?.firstElementChild || null;
  }
  ...
  function isUsableInjectionAnchor(el) {
    return !!el
      ...
      && !own(el)
      && !el.closest?.('[data-pankit-variants]');
  }
  ...
  function waitForVariantAnchorAndRetry({ filePath, sessionId, srcWrapper, checkpointReason }) {
    ...
    const origContent = srcWrapper?.querySelector('[data-pankit-variant="original"] > :first-child');
    ...
    pendingVariantAnchorRetryObserver = new MutationObserver(() => {
      ...
      const wrapperLanded = !!document.querySelector('[data-pankit-variants="' + sessionId + '"]');
      if (!wrapperLanded) {
  ...
  async function probePreviewTree(manifest) {
    ...
    try {
      ...
      return { ok: false, tried: err.pankitTriedUrls || candidates };
    }
  }

  function loadSvelteRuntime(runtimeModule, manifest) {
    const modulePath = runtimeModule || '/src/lib/pankit/__runtime.js';
  ...
  // componentDir (keyed by variant number), because a `data-pankit-params`
      ...
      if (entry.kind === 'collection' && entry.item && entry.item.rootTag) {
        ...
        values[entry.prop] = matches.map((itemEl, index) => {
          ...
          if (entry.item.keyField && item[entry.item.keyField] == null) {
            item[entry.item.keyField] = 'pankit-live-' + index;
          }
          return item;
        });
  ...
  function cloneWithoutElements(rootEl, excludedEls) {
    ...
    const MARK = 'data-pankit-hydration-excluded';
  ...
  async function mountSvelteComponentVariant(variantNum) {
    ...
    try {
      ...
      console.error('[pankit] Failed to mount component variant ' + variantNum + ' for ' + sessionId + ':', err);
  ...
  async function injectSvelteComponentsFromManifest(manifestPath, sessionId) {
    ...
    try {
      ...
      const existingWrapper = document.querySelector('[data-pankit-variants="' + sessionId + '"]');
      ...
      if (!liveEl?.parentElement) {
        console.warn('[pankit] Could not find original element in live DOM.');
        arrivedVariants = availableVariants;
        expectedVariants = Number(manifest.count) || expectedVariants || arrivedVariants;
      ...
      wrapper.dataset.pankitVariants = sessionId;
      wrapper.dataset.pankitVariantCount = String(manifest.count || expectedVariants || 1);
      wrapper.dataset.pankitPreview = componentPreviewMode;
      wrapper.style.display = 'contents';

      const mountTarget = document.createElement('div');
      mountTarget.dataset.pankitComponentMount = sessionId;
      ...
      saveSession();
      completeParameterGenerationIfReady();
      console.log('[pankit] Mounted ' + arrivedVariants + ' ' + manifest.framework + ' component variants.');
    } catch (err) {
      console.error('[pankit] Failed to mount component-preview variants:', err);
  ...
  function renderMountErrorCard() {
    ...
    retry.dataset.pankitMountRetry = 'true';
  ...
  function abortSvelteComponentInjection(sessionId, details) {
    try {
      if (svelteComponentSession?.sessionId === sessionId) {
        ...
        const orphan = document.querySelector('[data-pankit-variants="' + sessionId + '"]');
        if (orphan) orphan.remove();
      }
    } catch (err) {
      console.warn('[pankit] Svelte component abort cleanup failed:', err);
    }
    hideShaderOverlay();
  ...
  function resetSvelteComponentSession(sessionId, message) {
    try {
      ...
      console.warn('[pankit] Svelte component reset cleanup failed:', err);
    }
    hideShaderOverlay();
    if (variantObserver) { variantObserver.disconnect(); variantObserver = null; }
    if (pendingSvelteComponentRetryObserver) { pendingSvelteComponentRetryObserver.disconnect(); pendingSvelteComponentRetryObserver = null; }
  ...
  function discardOrphanedSession(reason) {
    ...
    if (!sessionId) return;
    console.warn('[pankit] Discarding orphaned session ' + sessionId + ': ' + reason);
  ...
  function injectVariantsFromSource(filePath, sessionId, opts = {}) {
    ...
    fetch(url)
      ...
      .then(html => {
        ...
        const startMark = '<!-- pankit-variants-start ' + sessionId + ' -->';
        const endMark = '<!-- pankit-variants-end ' + sessionId + ' -->';
        ...
        srcWrapper = doc.querySelector('[data-pankit-variants="' + sessionId + '"]');
        if (!srcWrapper) {
          console.warn('[pankit] Variant wrapper not found in source file.');
        ...
        const variants = wrapper.querySelectorAll('[data-pankit-variant]:not([data-pankit-variant="original"])');
        arrivedVariants = variants.length;
        expectedVariants = parseInt(wrapper.dataset.pankitVariantCount || arrivedVariants);
        if (arrivedVariants <= 0) {
          if (state === 'GENERATING') {
            ...
            if (!opts.generationCompleted) {
              console.log('[pankit] Source has scaffold but no variants yet; still generating.');
              return;
            }
            ...
            if (attempt < COMPLETED_SOURCE_FALLBACK_RETRIES) {
              console.log('[pankit] Generation is done but source shows no variants yet; retrying read ('
        ...
        saveSession();
        completeParameterGenerationIfReady();
        console.log('[pankit] Injected ' + arrivedVariants + ' variants from source file.');
      })
      .catch(err => {
        console.error('[pankit] Failed to fetch source:', err);
  ...
  function updateSelectedElement() {
    ...
    if (svelteComponentSession?.sessionId === currentSessionId) {
      const anchor = resolveSvelteComponentAnchor();
      if (anchor && !anchor.__pankitFrozenAnchor) selectedElement = anchor;
      return;
    }
    const wrapper = document.querySelector('[data-pankit-variants="' + currentSessionId + '"]');
    if (!wrapper) return;
  ...
  function readVisibleVariantFromDOM(sessionId) {
    ...
    for (const variant of variants) {
      ...
      const idx = parseInt(variant.dataset.pankitVariant || '0', 10);
      if (idx > 0) return idx;
    }
    return 0;
  }
  ...
  function pickVariantContent(wrapper, index) {
    if (!wrapper) return null;
    const variantDiv = wrapper.querySelector('[data-pankit-variant="' + index + '"]');
  ...
  // single variant (`… > [data-pankit-variant="N"]`); without it, targets
  // every variant via the bare `[data-pankit-variant]` attribute.
  function variantStateSelector(sessionId, num) {
    const wrapper = '[data-pankit-variants="' + sessionId + '"]';
    const variant = num == null
      ? '[data-pankit-variant]'
      : '[data-pankit-variant="' + num + '"]';
  ...
  function updateVariantStateStylesheet(sessionId, num) {
    ...
    const hideOthers = variantStateSelector(sessionId)
      + ':not([data-pankit-variant="' + num + '"]) { ' + VARIANT_HIDE_DECL + ' }';
  ...
  function showOriginalDuringDiscard(sessionId) {
    ...
    const wrapper = '[data-pankit-variants="' + sessionId + '"]';
    styleEl.textContent = wrapper + ' > [data-pankit-variant]:not([data-pankit-variant="original"]) { display:none !important; }\n'
      + wrapper + ' > [data-pankit-variant="original"] { display:block !important; }';
  }
  ...
  function startScrollLock(sessionId, initialTargetY, initialAnchorTop) {
    ...
    scrollLockObserver = new MutationObserver((mutations) => {
      for (const m of mutations) {
        if (m.target?.closest?.('[data-pankit-variants="' + sessionId + '"]')) {
          schedule('mutation-in-wrapper');
          return;
        }
        for (const n of m.addedNodes) {
          if (n.nodeType === 1 && (n.matches?.('[data-pankit-variants="' + sessionId + '"]') || n.querySelector?.('[data-pankit-variants="' + sessionId + '"]'))) {
  ...
  function startVariantObserver(sessionId) {
    ...
    const obs = new MutationObserver((mutations) => {
      ...
      // Only react to mutations that add nodes with data-pankit-variant,
      // or mutations inside the variant wrapper. Ignore our own bar/UI changes.
      let dominated = false;
      for (const m of mutations) {
        if (m.target.closest?.('[data-pankit-variants]')) { dominated = true; break; }
        for (const n of m.addedNodes) {
          if (n.nodeType !== 1) continue;
          // Direct hit: the added node itself is the wrapper or a variant.
          if (n.dataset?.pankitVariants || n.dataset?.pankitVariant) {
          ...
          if (n.querySelector?.('[data-pankit-variants],[data-pankit-variant]')) {
            dominated = true; break;
          }
        }
        if (dominated) break;
      ...
      if (selectedElement && !document.body.contains(selectedElement)) {
        const isInsert = wrapper.dataset.pankitMode === 'insert';
        if (isInsert) {
      ...
      const expected = parseInt(wrapper.dataset.pankitVariantCount || '0');
      ...
      if (arrivedVariants > 0) {
        ...
        recoveryWaitingForAnchor = false;
        hideShaderOverlay();
        if (wrapper.dataset.pankitMode === 'insert') finalizeInsertSession();
        updateSelectedElement();
        showOrUpdateCyclingBar();
        disableInlineEdit();
  ...
  function startScrollTracking() {
    function tick() {
      if (state === 'CONFIGURING' || state === 'GENERATING' || state === 'CYCLING') {
        ...
        if (hiTarget && !hiTarget.hasAttribute?.('data-pankit-insert-placeholder')) {
  ...
  function connectSSE() {
    ...
    evtSource.onmessage = (e) => {
      ...
      switch (msg.type) {
        case 'connected':
          ...
          if (!hasProjectContext) showToast(`No PRODUCT.md found. Variants will be brand-agnostic. Run ${PANKIT_COMMAND} init to generate one.`, 7000);
          console.log('[pankit] Live mode connected.');
        ...
        case 'error':
          ...
          if (awaitingAcceptResult?.id && msg.id === awaitingAcceptResult.id) {
            awaitingAcceptResult = null;
            console.error('[pankit] Accept failed after teardown:', msg.message);
          ...
          if (maybeCompleteSteer(msg)) break;
          console.error('[pankit] Error:', msg.message);
    ...
    evtSource.onerror = () => {
      ...
      if (sseRetries <= SSE_MAX_RETRIES) {
        console.log('[pankit] SSE connection lost. Retry ' + sseRetries + '/' + SSE_MAX_RETRIES + '...');
        return; // EventSource auto-reconnects
      }
      // Server is gone. Clean up gracefully.
      console.log('[pankit] Live server unreachable. Cleaning up UI.');
      evtSource.close();
      evtSource = null;
      handleServerLost();
    };
  ...
  function sendEvent(msg, opts) {
    ...
    function handleFailure(err) {
      if (opts && opts.throwOnError) {
        console.error('[pankit] Failed to send event:', err);
        throw err;
      }
      console.debug('[pankit] Dropped optional live event:', err);
      return null;
    }
    // Token in the query string as well as the body: the URL token is what
    // authorizes the CORS preflight when the page runs on a non-loopback
  ...
  function abandonForeignSession(sessionId) {
    ...
    console.warn('[pankit] The live server has no record of session ' + sessionId + '; clearing stale local state.');
    markSessionHandled();
    cleanup({ instantChrome: true });
  ...
  function handleKeyDown(e) {
    ...
    if (e.target.isContentEditable && isInlineEditActive(e.target)) {
      ...
      e.preventDefault();
      e.stopPropagation();
      const original = e.target.dataset.pankitOriginalText;
  ...
  function shouldUseAncestorCropShaderProxy(el) {
    ...
    const adapter = String(window.__PANKIT_LIVE_ADAPTER__ || '').toLowerCase();
    ...
    const wrapper = el?.closest?.('[data-pankit-variants]');
    return isFrameworkComponentPreviewMode(wrapper?.dataset?.pankitPreview);
  }
    ...
    try {
      ...
      if (shouldUseAncestorCropShaderProxy(el)) {
        try {
          ...
          console.warn('[pankit] Svelte ancestor crop capture failed, falling back to element capture:', err);
        }
      }
  ...
  async function captureAndEmit(el, basePayload, snapshot, rect) {
    ...
    try {
      ...
      console.warn('[pankit] capture failed, proceeding without screenshot:', err);
    }
    ...
    if (blob && hasAnnotations) {
      try {
        ...
        if (uploadRes.ok) {
          ...
          console.warn('[pankit] annotation upload failed:', uploadRes.status);
        }
      } catch (err) {
        console.warn('[pankit] annotation upload failed:', err);
      }
    }
...
void main() {
  ...
  async function showShaderOverlay(el, blob, rect, paper) {
    ...
    try {
      ...
      console.warn('[pankit] shader setup failed:', err);
      canvas.remove();
      return;
    }
    ...
    try {
      ...
      console.warn('[pankit] shader bitmap decode failed:', err);
  ...
  async function handleAccept() {
    ...
    const acceptWrapper = document.querySelector('[data-pankit-variants="' + currentSessionId + '"]');
    ...
    const acceptedIsSvelteComponent = svelteComponentSession?.sessionId === acceptedSessionId
      || isFrameworkComponentPreviewMode(acceptWrapper?.dataset?.pankitPreview);
    ...
    sendEvent(acceptPayload, { throwOnError: true })
      .then(() => {
        ...
        document.documentElement.dataset.pankitAcceptToPickingMs = String(Date.now() - acceptPayload.clientSentAt);
  ...
  function snapshotAcceptedVariantDom(sessionId, variantId) {
    const wrapper = document.querySelector('[data-pankit-variants="' + sessionId + '"]');
    const accepted = wrapper?.querySelector?.('[data-pankit-variant="' + variantId + '"]');
  ...
  function acceptedDomAlreadyClean(pending) {
    ...
    return matches.length > 0
      && matches.every((el) => !el.closest('[data-pankit-variants],[data-pankit-variant],[data-pankit-carbonize]'));
  }
  ...
  function findAcceptedRuntimeWrappers(sessionId) {
    if (!sessionId) return [];
    return [...new Set([
      ...document.querySelectorAll('[data-pankit-variants="' + sessionId + '"]'),
      ...document.querySelectorAll('[data-pankit-carbonize="' + sessionId + '"]'),
    ])];
  }
  ...
  function reloadAfterMissingAcceptedDom(pending) {
    if (acceptedDomAlreadyClean(pending)) return;
    if (pending?.id && document.querySelector('[data-pankit-variants="' + pending.id + '"]')) return;
    location.reload();
  }
  ...
  function commitAcceptedVariantToDom(sessionId, variantId) {
    ...
    const style = wrapper.querySelector('style[data-pankit-css]');
    if (style && !document.querySelector('style[data-pankit-accepted-css="' + sessionId + '"]')) {
      const promotedStyle = style.cloneNode(true);
      promotedStyle.setAttribute('data-pankit-accepted-css', sessionId);
  ...
  function restoreFromActiveSessions(activeSessions, reason) {
    const wrapper = document.querySelector('[data-pankit-variants]');
    if (wrapper && !isFrameworkComponentPreviewMode(wrapper.dataset.pankitPreview)) return false;
  ...
  function recoverMissedGenerationCompletion(activeSessions) {
    ...
    if (!file) return;
    console.log('[pankit] Reconnected after generation completed; recovering variants from source.');
    setTimeout(() => {
  ...
  function loadSession() {
    ...
    if (saved?.appRoot && APP_ROOT && saved.appRoot !== APP_ROOT) {
      console.warn('[pankit] Ignoring saved live session from another project (' + saved.appRoot + ').');
  ...
  function cleanup(options) {
    ...
    if (svelteComponentSession?.sessionId === cleanupSessionId) {
      ...
      const wrapper = document.querySelector('[data-pankit-variants="' + cleanupSessionId + '"]');
      if (wrapper) {
      ...
      setTimeout(function() {
        ...
        const lateWrapper = document.querySelector('[data-pankit-variants="' + cleanupSessionId + '"]');
        if (!lateWrapper) return;
        const orig = lateWrapper.querySelector('[data-pankit-variant="original"]');
        if (orig) {
  ...
  // If a [data-pankit-variants] wrapper exists in the DOM, the agent wrote
  ...
  function resumeSession() {
    ...
    const sessionId = wrapper.dataset.pankitVariants;
    ...
    // wrapper holds a single mount target, not [data-pankit-variant] nodes,
    ...
    if (isFrameworkComponentPreviewMode(wrapper.dataset.pankitPreview)
    ...
    if (isFrameworkComponentPreviewMode(wrapper.dataset.pankitPreview)) {
      ...
      currentSessionId = sessionId;
      expectedVariants = Number(wrapper.dataset.pankitVariantCount)
    ...
    currentSessionId = sessionId;
    expectedVariants = parseInt(wrapper.dataset.pankitVariantCount || '0');
    const variants = wrapper.querySelectorAll('[data-pankit-variant]:not([data-pankit-variant="original"])');
    arrivedVariants = variants.length;
      ...
      if (shaderTarget) {
        (async () => {
          try {
            ...
            console.warn('[pankit] shader resume failed:', err);
          }
        })();
      }
    }
  ...
  const PICK_PREFS_KEY = 'pankit-live-pick';
  const INTERACTION_PREFS_KEY = 'pankit-live-interaction';
  ...
  function detectPageTheme() {
    try {
      // Dev override: set localStorage 'pankit-dev-theme' to 'light' or
      // 'dark' to preview the opposite palette without actually changing the
      // page bg. Used for screenshots and theme QA.
      const override = localStorage.getItem('pankit-dev-theme');
  ...
  function attachSteerFocusGuard() {
    if (window.__PANKIT_STEER_FOCUS_GUARD__) return;
    window.__PANKIT_STEER_FOCUS_GUARD__ = true;
  ...
  function steerFocusDebugEnabled() {
    try { return localStorage.getItem('pankit-steer-debug') === '1'; } catch { return false; }
  }

  function steerFocusLog(reason, extra) {
    if (!steerFocusDebugEnabled()) return;
    console.log('[pankit.steer]', reason, {
      state,
      pickActive,
  ...
  function attachSteerFocusDebug() {
    if (!steerFocusDebugEnabled()) return;
    if (window.__PANKIT_STEER_FOCUS_DEBUG__) return;
    window.__PANKIT_STEER_FOCUS_DEBUG__ = true;
  ...
  function buildSteerProcessingDots() {
    ...
    for (let i = 0; i < 3; i++) {
      wrap.appendChild(el('span', {
        ...
        animation: 'pankit-steer-dot 1.05s ease-in-out ' + (i * 0.14) + 's infinite',
      }));
    }
    return wrap;
  }
  ...
  function startVoice(ctx) {
    ...
    rec.onerror = (event) => {
      ...
      console.warn('[pankit.voice] recognition error:', code);
    ...
    try {
      ...
      console.warn('[pankit.voice] start failed:', err);
      stopVoice({
        suppressSubmit: true,
  ...
  function initPageChat(parent, P) {
    ...
    if (!uiGetById(PREFIX + '-page-chat-style')) {
      ...
      s.textContent =
        '@keyframes pankit-steer-dot { 0%, 70%, 100% { opacity: 0.28; transform: scale(0.82); } 35% { opacity: 1; transform: scale(1); } }' +
        '@keyframes pankit-steer-processing { 0%, 100% { border-color: oklch(70% 0.12 188 / 0.28); box-shadow: 0 0 0 0 oklch(70% 0.12 188 / 0); } 50% { border-color: oklch(82% 0.07 188 / 0.55); box-shadow: 0 0 14px oklch(70% 0.12 188 / 0.18); } }' +
        '@keyframes pankit-voice-pulse { 0%, 100% { opacity: 0.55; } 50% { opacity: 1; } }' +
        '#' + PREFIX + '-page-chat[data-processing="true"] { animation: pankit-steer-processing 1.6s ease-in-out infinite; }' +
        ...
        '#' + PREFIX + '-page-chat-voice[data-listening="true"] svg { animation: pankit-voice-pulse 1.1s ease-in-out infinite; }' +
  ...
  // PanKit mark - same paths as site/components/Header.astro + favicon.svg.
  ...
  function syncAgentPollingUi(connected) {
    ...
    globalBarBrandEl.setAttribute('aria-label', agentStatusMessage
      ? 'PanKit live mode - ' + (agentHasWorkInFlight() ? 'agent is working' : 'agent not polling')
      : 'PanKit live mode');
  ...
  function initGlobalBar() {
    ...
    if (!uiGetById(PREFIX + '-bar-focus-style')) {
      ...
      s.textContent =
        ...
        '}' +
        '@keyframes pankit-agent-dot { 0%, 100% { opacity: 0.45; transform: scale(0.9); } 50% { opacity: 1; transform: scale(1); } }' +
        '#' + PREFIX + '-global-bar-brand[data-agent-connected="false"] [data-agent-dot] { animation: pankit-agent-dot 1.4s ease-in-out infinite; }' +
    ...
    // Brand mark - kinpaku PanKit icon (site header / favicon paths).
    const brand = el('span', {
      display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
    ...
    brand.setAttribute('aria-label', 'PanKit live mode - agent not polling');
  ...
  function updateGlobalBarState() {
    ...
    document.querySelectorAll('.pankit-overlay').forEach(o => {
      o.style.pointerEvents = (pickActive || insertActive) ? 'none' : '';
    });
    syncPageInteractionCursor();
  }

  let detectReady = false; // true once detect script posts 'pankit-ready'
  ...
  function toggleDetect() {
    ...
    if (detectActive) {
      ...
      window.postMessage({ source: 'pankit-command', action: 'remove' }, '*');
  ...
  function loadDetectScript() {
    ...
    s.dataset.pankitExtension = 'true';
  ...
  function onDetectMessage(e) {
    ...
    if (e.data.source === 'pankit-results') {
  ...
  function teardown() {
    ...
    removeVariantStateStylesheet();
    window.__PANKIT_LIVE_INIT__ = false;
    console.log('[pankit] Live mode exited.');
  }

  //
  // Design System Panel - visualizes the project's .pankit/design.json sidecar
  //

  const DESIGN_PREFS_KEY = 'pankit-live-design-panel';
  ...
  let designState = {
    ...
    sidecar: null,          // .pankit/design.json v2 payload (extensions + components + narrative)
    hasMd: false,
    hasSidecar: false,
  ...
  // Neutral panel palette - deliberately NOT PanKit-branded. The panel is
  // a viewer of the project's design system, not an PanKit surface.
  const DP = {
  ...
  function renderDesignBody() {
    ...
    if (designState.present === false) {
      ...
      empty.innerHTML = `<strong>No DESIGN.md yet</strong>Create one by running <code>${PANKIT_COMMAND} document</code> in your terminal, then re-open this panel.`;
  ...
  function renderStaleHint() {
    ...
    box.innerHTML = `
      ...
      <span class="stale-text"><strong>DESIGN.md is newer than .pankit/design.json.</strong> Run <code>${PANKIT_COMMAND} document</code> to refresh the sidecar.</span>
    `;
    return box;
  }
  ...
  function renderParsedMdCta() {
    ...
    box.innerHTML = `<strong>Basic view</strong>This panel reads the tokens in your <code>DESIGN.md</code> frontmatter. Running <code>${PANKIT_COMMAND} document</code> also generates a <code>.pankit/design.json</code> sidecar with your project's actual component snippets (button, input, nav) and tonal ramps, rendered live below the tokens.`;
    return box;
  }
  ...
  function designEmptyMessage() {
    if (designState.hasMd && !designState.hasSidecar) {
      return 'DESIGN.md found, no structured tokens to display. Run ' + PANKIT_COMMAND + ' document to generate the .pankit/design.json sidecar.';
    }
  ...
  function cssSafe(v) {
    ...
    // .pankit/design.json values rendered into inline style strings.
  ...
  function init() {
    ...
    if (!resumeSession()) {
      console.log('[pankit] Live variant mode ready. Hover over elements to pick one.');
      ...
      const scout = new MutationObserver(() => {
        ...
        scout.disconnect();
        if (resumeSession()) {
          console.log('[pankit] Resumed deferred session ' + currentSessionId + ' (post-hydration).');
        }
      });
      scout.observe(document.body, { childList: true, subtree: true });
    } else {
      console.log('[pankit] Resumed active variant session ' + currentSessionId + ' (' + arrivedVariants + '/' + expectedVariants + ' variants).');
    }
