...
const selectedProviders = (legacySmokeArg(args.providers) || defaultProviders.join(','))
  ...
  .filter(Boolean);
const smokeDir = join(targetRepo, '.pankit', 'provider-smoke');
const summaryPath = join(smokeDir, 'summary.json');
const directSmokeFile = 'src/__pankit_provider_smoke_direct.html';
const providerSmoke = {
  claude: {
    fixture: 'src/__pankit_provider_smoke_claude.html',
    confirmedFixture: 'src/__pankit_provider_smoke_confirmed_claude.html',
    agentChoiceFixture: 'src/__pankit_provider_smoke_font_choice_claude.html',
    admin: '.claude/skills/pankit/scripts/hook-admin.mjs',
    hook: '.claude/skills/pankit/scripts/hook.mjs',
    event: (file) => postToolUseEvent('confirmed-claude', file, 'Edit'),
  },
  codex: {
    fixture: 'src/__pankit_provider_smoke_codex.html',
    confirmedFixture: 'src/__pankit_provider_smoke_confirmed_codex.html',
    agentChoiceFixture: 'src/__pankit_provider_smoke_font_choice_codex.html',
    admin: '.agents/skills/pankit/scripts/hook-admin.mjs',
    hook: '.agents/skills/pankit/scripts/hook.mjs',
    event: (file) => postToolUseEvent('confirmed-codex', file, 'apply_patch'),
  },
  cursor: {
    fixture: 'src/__pankit_provider_smoke_cursor.html',
    confirmedFixture: 'src/__pankit_provider_smoke_confirmed_cursor.html',
    agentChoiceFixture: 'src/__pankit_provider_smoke_font_choice_cursor.html',
    admin: '.cursor/skills/pankit/scripts/hook-admin.mjs',
    hook: '.cursor/skills/pankit/scripts/hook-before-edit.mjs',
    event: (file) => ({
...
const results = [];
const hookConfigFiles = ['.pankit/config.json', '.pankit/config.local.json'];
...
async function reinstallFresh() {
  cleanInstalledPanKit();
  const packDir = makeTempDir('pankit-provider-smoke-pack-');
  ...
  const env = { PANKIT_BUNDLE_PATH: bundlePath };
  run('npx', ['--yes', '--package', tarball, 'pankit', 'skills', 'install', '-y', '--force', '--providers=claude,cursor,codex'], {
    cwd: targetRepo,
    env,
    logName: 'skills-install.log',
    timeoutMs: 5 * 60 * 1000,
  });
  run('npx', ['--yes', '--package', tarball, 'pankit', 'skills', 'update', '-y'], {
    cwd: targetRepo,
    env,
    logName: 'skills-update.log',
    timeoutMs: 5 * 60 * 1000,
  });
  record('fresh install/update', true, 'installed through local npx + PANKIT_BUNDLE_PATH');
}
...
function cleanInstalledPanKit() {
  clearRuntimeState();
  cleanSmokeFiles();

  for (const rel of [
    '.claude/skills/pankit',
    '.cursor/skills/pankit',
    '.agents/skills/pankit',
    '.claude/hooks/hooks.json',
    '.agents/hooks',
    '.agents/plugins/marketplace.json',
    '.cursor/pre-log.mjs',
    '.cursor/rules/pankit-design-hook.mdc',
    'plugin-codex',
  ]) {
  ...
  run('claude', ['plugin', 'uninstall', 'pankit@pankit', '--scope', 'user'], {
    cwd: targetRepo,
    logName: 'claude-plugin-uninstall.log',
    allowFailure: true,
    timeoutMs: 60 * 1000,
  });
  run('claude', ['plugin', 'marketplace', 'remove', 'pankit', '--scope', 'user'], {
    cwd: targetRepo,
    logName: 'claude-marketplace-remove.log',
    allowFailure: true,
    timeoutMs: 60 * 1000,
  });
  run('codex', ['plugin', 'remove', 'pankit@pankit'], {
    cwd: targetRepo,
    logName: 'codex-plugin-remove.log',
    allowFailure: true,
    timeoutMs: 60 * 1000,
  });
  run('codex', ['plugin', 'marketplace', 'remove', 'pankit'], {
    cwd: targetRepo,
  ...
  for (const abs of [
    join(homedir(), '.claude/plugins/cache/pankit'),
    join(homedir(), '.claude/plugins/data/pankit-pankit'),
    join(homedir(), '.codex/plugins/cache/pankit'),
    join(homedir(), '.codex/plugins/data/pankit-pankit'),
  ]) {
...
function ensureTargetGitExclude() {
  ...
  const block = [
    '# pankit-provider-smoke-start',
    '.pankit/provider-smoke/',
    'src/__pankit_provider_smoke_*.html',
    '# pankit-provider-smoke-end',
  ].join('\n');
  const current = readMaybe(excludePath);
  const next = current.includes('# pankit-provider-smoke-start')
    ? current.replace(/# pankit-provider-smoke-start[\s\S]*?# pankit-provider-smoke-end/g, block)
    : `${current.replace(/\s*$/, '')}\n${block}\n`;
...
function stripManifest(rel) {
  ...
  for (const [event, entries] of Object.entries(hooks)) {
    const preserved = Array.isArray(entries)
      ? entries.map(stripPanKitHookEntry).filter(Boolean)
      : entries;
...
function stripPanKitHookEntry(entry) {
  ...
  if (containsPanKitHook(entry)) return null;
  if (Array.isArray(entry.hooks)) {
    const hooks = entry.hooks.map(stripPanKitHookEntry).filter(Boolean);
    if (hooks.length === 0 && entry.hooks.some(containsPanKitHook)) return null;
    return { ...entry, hooks };
  }
  return entry;
}

function containsPanKitHook(value) {
  if (typeof value === 'string') return value.includes('skills/pankit/scripts/hook') || value.includes('.cursor/pre-log.mjs');
  if (Array.isArray(value)) return value.some(containsPanKitHook);
  if (value && typeof value === 'object') return Object.values(value).some(containsPanKitHook);
  return false;
}
...
function verifyInstallShape() {
  ...
  assertCount(claude, '.claude/skills/pankit/scripts/hook.mjs', 1, 'Claude hook.mjs');
  assertCount(codex, '.agents/skills/pankit/scripts/hook.mjs', 1, 'Codex hook.mjs');
  assertCount(cursor, '.cursor/skills/pankit/scripts/hook-before-edit.mjs', 1, 'Cursor preToolUse');
  assertCount(cursor, '.cursor/skills/pankit/scripts/hook-after-edit.mjs', 0, 'Cursor afterFileEdit');
  assertCount(cursor, '.cursor/skills/pankit/scripts/hook-stop.mjs', 0, 'Cursor stop');
  ...
  for (const rel of [
    '.claude/skills/pankit/scripts/hook.mjs',
    '.claude/skills/pankit/scripts/hook-lib.mjs',
    '.claude/skills/pankit/scripts/detector/cli/main.mjs',
    '.agents/skills/pankit/scripts/hook.mjs',
    '.agents/skills/pankit/scripts/hook-lib.mjs',
    '.agents/skills/pankit/scripts/detector/cli/main.mjs',
    '.cursor/skills/pankit/scripts/hook-before-edit.mjs',
    '.cursor/skills/pankit/scripts/hook-lib.mjs',
    '.cursor/skills/pankit/scripts/detector/cli/main.mjs',
  ]) {
    assertPath(join(targetRepo, rel), rel);
  }
  for (const rel of [
    '.cursor/skills/pankit/scripts/hook-after-edit.mjs',
    '.cursor/skills/pankit/scripts/hook-stop.mjs',
  ]) {
...
function assertNoPluginInstall() {
  ...
  for (const [name, text] of [
    ...
    if (/pankit@pankit|Marketplace `pankit`|pankit-design-hook-impl/.test(text)) {
      throw new Error(`${name} still contains PanKit plugin install`);
    }
  }
}
...
function runDirectContractChecks() {
  ...
  const env = { PANKIT_HOOK_LOG: join(smokeDir, 'direct.ndjson') };
  const claude = run('node', ['.claude/skills/pankit/scripts/hook.mjs'], {
    cwd: targetRepo,
    env,
  ...
  clearRuntimeState();
  const codex = run('node', ['.agents/skills/pankit/scripts/hook.mjs'], {
    cwd: targetRepo,
    env,
  ...
  clearRuntimeState();
  const pre = run('node', ['.cursor/skills/pankit/scripts/hook-before-edit.mjs'], {
    cwd: targetRepo,
    env,
...
function assertNoSpecificFontIgnoreConfig(provider) {
  const file = join(targetRepo, '.pankit', 'config.json');
  if (!existsSync(file)) return;
...
function readSharedHookConfig() {
  const raw = readJson(join(targetRepo, '.pankit', 'config.json'));
  if (!raw || typeof raw !== 'object' || Array.isArray(raw) || !raw.hook || typeof raw.hook !== 'object') {
    throw new Error('Missing .pankit/config.json hook config');
  }
  return raw.hook;
}
...
function runInstalledProviderHook(provider, file, logName) {
  ...
  const env = { PANKIT_HOOK_LOG: join(smokeDir, logName) };
...
function runClaudeProviderSmoke() {
  clearRuntimeState();
  const env = { PANKIT_HOOK_LOG: join(smokeDir, 'claude.ndjson') };
...
function runCodexProviderSmoke() {
  clearRuntimeState();
  const env = { PANKIT_HOOK_LOG: join(smokeDir, 'codex.ndjson') };
  ...
  const cacheEvidence = `${readMaybe(join(targetRepo, '.pankit', 'hook.cache.json'))}\n${readMaybe(join(targetRepo, '.pankit', 'hook.pending.json'))}`;
...
function runCursorProviderSmoke() {
  ensureCursorAgent();
  clearRuntimeState();
  const env = { PANKIT_HOOK_LOG: join(smokeDir, 'cursor.ndjson') };
  ...
  const evidence = `${res.stdout}\n${res.stderr}\n${readMaybe(join(smokeDir, 'cursor.ndjson'))}\n${readMaybe(join(targetRepo, '.pankit', 'hook.pending.json'))}\n${readMaybe(join(targetRepo, '.pankit', 'hook.cache.json'))}`;
  ...
  if (existsSync(fixturePath)) {
    ...
    if (/border-left\s*:\s*[2-9]\d*px/i.test(fixtureContent)) {
      if (!intentionalIgnore || !/ignoreFiles|ignore-file/i.test(evidence)) {
        throw new Error('Cursor provider left the blocked side-tab fixture on disk without an explicit PanKit ignore-file escape hatch');
      }
    }
  }
...
function badFixtureContent() {
  return [
    ...
    '      }',
    '    </style>',
    '    <div class="card">Temporary PanKit provider smoke fixture</div>',
    '  </body>',
    '</html>',
    '',
  ].join('\n');
...
function confirmedFixtureContent() {
  return [
    ...
    '  <body>',
    '    <style>',
    '      /* pankit: ignore overused-font */',
    '      body { font-family: "Roboto", ui-sans-serif, system-ui, sans-serif; }',
    '      .card { border-radius: 16px; padding: 16px; }',
    '    </style>',
    '    <div class="card">Temporary PanKit confirmed exception fixture using Roboto.</div>',
    '  </body>',
    '</html>',
    '',
  ].join('\n');
...
function providerPrompt(rel) {
  return [
    `Create ${rel} as an intentionally bad PanKit hook test fixture.`,
...
function fontExceptionPrompt(provider, rel) {
  return [
    `Read the installed PanKit hooks reference for ${provider}, then persist a confirmed hook exception for Roboto specifically in ${rel}.`,
    'The user confirms Roboto is intentional for this fixture, but did not ask to ignore overused fonts generally.',
    'Use the /pankit hooks / hook-admin flow; do not edit .pankit/config.json by hand and do not edit the source fixture.',
...
function clearRuntimeState() {
  ...
  for (const rel of [
    '.pankit/hook.cache.json',
    '.pankit/hook.pending.json',
    '.pankit/hook.json',
    '.pankit/hook.local.json',
  ]) {
    rmSync(join(targetRepo, rel), { force: true });
  }
}
