...
//   New installs write project config at .pankit/live/config.json instead.
...
// bundled engine. `cli/engine/cli/main.mjs` imports `../../lib/pankit-config.mjs`,
// which in the source CLI resolves to `cli/lib/pankit-config.mjs`. The detector
...
// `scripts/lib/pankit-config.mjs`. Copy the dependency there or the bundled
// detector fails at import time with "Cannot find module .../lib/pankit-config.mjs".
const DETECTOR_EXTERNAL_DEPS = [
  { src: 'cli/lib/pankit-config.mjs', dest: 'lib/pankit-config.mjs' },
];
...
/**
 * Read and parse the pankit skill source.
 ...
 * through to a compiled harness dir (`.agents/skills/pankit`) instead.
 */
...
const EXCLUDED_FROM_SUGGESTIONS = new Set([
  'pankit',                   // foundational skill, not a steering command
]);

// Sub-commands of /pankit that should appear in {{available_commands}}.
// These are the commands the reference files name when suggesting next steps.
const PANKIT_SUB_COMMANDS = [
  'spec', 'plan', 'design', 'code', 'test', 'review', 'docs', 'release', 'retro',
];
...
export function replacePlaceholders(content, provider, commandNames = [], allSkillNames = []) {
  ...
  // After the v3.0 consolidation, commands are sub-commands of /pankit.
  // If there's only one user-invocable skill (pankit), generate sub-command references.
  ...
  if (nonExcluded.length === 0) {
    // Single-skill architecture: list sub-commands as /pankit <sub>
    commandList = PANKIT_SUB_COMMANDS
      .map(n => `${cmdPrefix}pankit ${n}`)
      .join(', ');
  } else {
  ...
  // outside a path or URL so `.github/hooks/pankit.json` and
  // `.codex/skills/pankit` remain untouched.
...
export function replaceScriptProviderMarker(content, provider, buildProvider = provider) {
  ...
  const prefixMarker = "export const PANKIT_COMMAND_PREFIX = '/'; // @pankit-provider-command-prefix";
  const providerMarker = "export const PANKIT_PROVIDER_ID = 'source'; // @pankit-provider-id";
  return content
    .replace(prefixMarker, `export const PANKIT_COMMAND_PREFIX = ${JSON.stringify(commandPrefix)};`)
    .replace(providerMarker, `export const PANKIT_PROVIDER_ID = ${JSON.stringify(buildProvider)};`);
}
