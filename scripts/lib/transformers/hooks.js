/**
 * Build-pipeline emitters for the PanKit design hook.
 *
 * Two emission targets exist:
 *
 * 1. Project-local install (the `npx pankit skills install` CLI path):
 *      - Claude Code: `.claude/settings.json`   (${CLAUDE_PROJECT_DIR}-relative)
 *      - Codex:       `.codex/hooks.json`
 *      - Cursor:      `.cursor/hooks.json`
 *      - Grok Build:  `.grok/hooks/pankit.json`
 *
 ...
 * root rather than assuming a `.claude/skills/pankit/` layout, so it stays
 * correct wherever Claude Code unpacks the plugin.
 */

export const PANKIT_HOOK_COMMAND_MARKER = 'skills/pankit/scripts/hook.mjs';
...
const CLAUDE_PROJECT_HOOK = '${CLAUDE_PROJECT_DIR}/.claude/skills/pankit/scripts/hook.mjs';
...
const NODE_NOTICE_TEXT = `The pankit design hook is not running: no Node ${NODE_MAJOR_FLOOR} or newer on PATH. `
  + 'Install one, or remove the pankit hook from your harness settings.';
// Claude Code and Codex both read `systemMessage`, so one payload serves both.
// The marker under ~/.pankit holds it to one notice per machine (not per
...
const SYSTEM_MESSAGE_NOTICE = 'D="$HOME/.pankit"; [ -f "$D/node-unsupported" ] || '
  + '{ mkdir -p "$D" 2>/dev/null && : > "$D/node-unsupported" 2>/dev/null && '
  + `printf '%s' '{"systemMessage":"${NODE_NOTICE_TEXT}"}'; }`;
const CLAUDE_PLUGIN_HOOK = '${CLAUDE_PLUGIN_ROOT}/skills/pankit/scripts/hook.mjs';
const CODEX_PLUGIN_HOOK = '${PLUGIN_ROOT}/skills/pankit/scripts/hook.mjs';
...
const codexProjectHook = (skillDir) => `${skillDir}/skills/pankit/scripts/hook.mjs`;
const CURSOR_BEFORE_EDIT_SCRIPT = '.cursor/skills/pankit/scripts/hook-before-edit.mjs';
const GITHUB_PROJECT_HOOK = '$(git rev-parse --show-toplevel)/.github/skills/pankit/scripts/hook.mjs';
...
const GROK_PROJECT_HOOK = '.grok/skills/pankit/scripts/hook.mjs';
