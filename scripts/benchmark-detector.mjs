    ...
    cases.push(await measureCase({
      ...
      run: async (profile) => {
        ...
        try {
          ...
          await page.evaluate(() => { window.__PANKIT_CONFIG__ = { autoScan: false }; });
          await page.evaluate(browserScript);
          const pureStarted = nowMs();
          findings = await page.evaluate(() => {
            const serialized = window.pankitDetect({ decorate: false, serialize: true });
          ...
          const overlayGroupCount = await page.evaluate(() => window.pankitScan().length);
          addEvent(profile, {
            engine: 'browser',
            phase: 'scan',
            ruleId: 'overlay-scan',
