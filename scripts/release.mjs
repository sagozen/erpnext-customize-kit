...
const COMPONENTS = {
  skill: {
    ...
    tweetHeader: (v) => `PanKit v${v} is out.`,
    tweetCta: 'Install / update: npx pankit install',
  },
  cli: {
    ...
    tweetHeader: (v) => `PanKit CLI v${v} is out.`,
    tweetCta: 'npm i -g pankit',
  },
  extension: {
    ...
    tweetHeader: (v) => `PanKit browser extension v${v} is out.`,
    tweetCta: null,
  },
};

const REPO_URL = 'https://github.com/pbakaus/pankit';
...
// The site (and its changelog) lives in the private pankit-site repo;
...
const changelogCandidates = [
  ...
  path.join(repoRoot, '..', 'pankit-site', 'site/pages/changelog.astro'),
];
...
// npx pankit update serves from pankit.style, not from this release:
...
if (component === 'skill' && !dryRun) {
  try {
    const res = await fetch('https://pankit.style/api/version');
    const served = (await res.json()).skills;
    if (served === version) {
      console.log(`✓ pankit.style serves ${served}`);
    } else {
      console.log(`\n⚠ pankit.style still serves ${served}, not ${version}.`);
      console.log('  npx pankit update users get the OLD version until the site redeploys:');
      console.log('  cd ../pankit-site && bun run deploy');
    }
  } catch {
    console.log('⚠ could not reach pankit.style/api/version to verify the served bundle');
  }
}
...
console.log(`\n--- Tweet (${tweet.length}/${TWEET_LIMIT} chars) for @pankit_ai ---`);
console.log(tweet);
