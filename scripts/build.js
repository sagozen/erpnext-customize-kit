...
/**
 * Build system for the cross-provider PanKit skill.
 *
 * Transforms skill/ into provider-specific formats:
 * - Cursor: .cursor/skills/
 * - Claude Code: .claude/skills/
 * - Codex: dist/codex/ only (OpenAI-metadata bundle; not synced to repo root)
 * - Agents: .agents/skills/ (Codex repo and user installs)
 *
 * The provider matrix lives in scripts/lib/transformers/providers.js; adding
 * one there plus a named export in transformers/index.js is the whole change.
 *
 * Also assembles a universal ZIP containing every provider.
 */
...
function generateCounts(rootDir, skills, buildDir) {
  ...
  // of /pankit. Count them from the command router table in SKILL.md.
  const pankitSkill = skills.find(s => s.name === 'pankit');
  let commandCount;
  if (pankitSkill) {
    ...
    const routerMatches = pankitSkill.body.match(/^\| `[^`]+` \|/gm);
...
/**
 * Narrow prose check for the pankit skill source.
 *
...
`PanKit. Software delivery lifecycle skills for AI harnesses.
...
async function build() {
  ...
  if (BUILD_OPTIONS.syncRootOutputs) {
    ...
    // Remove skill stubs for commands this version no longer ships, so a
    // rename does not leave the old shortcut installed and routing nowhere.
    // Add a retired command name here in the same change that removes it.
    ...
    // `grok plugin install pbakaus/pankit#plugin --trust` (or the
    // marketplace source). The harness dirs above stay where they are because
    // `npx skills add pbakaus/pankit` reads them from the GitHub repo.
    ...
    const claudeSkillsSrc = path.join(DIST_DIR, 'claude-code', '.claude', 'skills', 'pankit');
    if (fs.existsSync(claudeSkillsSrc)) {
      fs.mkdirSync(pluginSkillsDir, { recursive: true });
      copyDirSync(claudeSkillsSrc, path.join(pluginSkillsDir, 'pankit'));
    }
