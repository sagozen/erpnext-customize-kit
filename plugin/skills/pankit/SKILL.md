...
version: 0.1.0
user-invocable: true
argument-hint: "[spec|plan|design · code · test|review · docs|release|retro] [target]"
license: Apache 2.0
allowed-tools:
  - Bash(node .claude/skills/pankit/scripts/*)
---
...
1. Run `node <skill-base-dir>/scripts/context.mjs` once per session, where `<skill-base-dir>` is the loaded base directory the runtime reports for this skill; keep cwd at the user's project. That base directory resolves every `node .claude/skills/pankit/scripts/...` command in this skill and its references, and `.claude/skills/pankit/scripts` is the fallback only when the runtime reports no base directory. Pass a named source file as `--target <path>`. Follow its directives and do not rerun it.
...
**Pin / Unpin:** `node .claude/skills/pankit/scripts/pin.mjs <pin|unpin> <command>` creates or removes a standalone `/<command>` shortcut. Report the script's result concisely; relay stderr verbatim on error.
