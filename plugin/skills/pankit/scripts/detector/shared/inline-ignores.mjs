 ...
 * an ignore in `.pankit/config.json`.
 *
 ...
 * line, so the same marker works across every comment style pankit scans —
 * `//`, `/* *\/`, `<!-- -->`, `#`, `{/* *\/}`, `{# #}`. Trailing comment closers
 * are stripped before the rule list is parsed.
 *
 * Syntax (reason optional; eslint `--` or biome `:` separator):
 *
 *   pankit-disable <rule>[, <rule>...] [-- reason]   whole file
 *   pankit-disable-line <rule>...      [-- reason]   the same line
 *   pankit-disable-next-line <rule>... [-- reason]   the following line
 *   pankit-disable                                   bare / `*` = every rule
 *
 * Examples:
 *
 *   <!-- pankit-disable overused-font -- exported brand doc, font is first-party -->
 *   .brand { font-family: Inter; } /* pankit-disable-line overused-font *\/
 *   // pankit-disable-next-line bounce-easing: intentional playful affordance
 *
...
const DIRECTIVE_RE = /pankit-(disable-next-line|disable-line|disable)\b[ \t]*([^\n\r]*)/gi;
...
function parseInlineIgnores(content) {
  ...
  // Case-insensitive to match DIRECTIVE_RE's `i` flag (e.g. `PanKit-Disable`).
  if (!/pankit-disable/i.test(text)) return result;
