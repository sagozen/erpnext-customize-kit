/**
 * Anti-Pattern Browser Detector for PanKit
...
// ─── Scoped ignores: data-pankit-ignore ─────────────────────────────────
//
// An element-scoped waiver that travels with the markup: any element carrying
// `data-pankit-ignore="rule-a rule-b"` (or `*`, or an empty value, for
...
// `pankit-disable` comment directives, which the browser cannot apply (a
// live DOM has no line numbers), and the generalization of the one-off
// `data-pankit-allow-kickers` opt-out.
//
...
function scopedIgnoreActive(el, ruleId) {
  ...
  while (cur && cur.nodeType === 1) {
    const attr = typeof cur.getAttribute === 'function' ? cur.getAttribute('data-pankit-ignore') : null;
    if (attr != null) {
...
function checkElementQualityDOM(el) {
  ...
  const rect = el.getBoundingClientRect();
  const lineMax = (typeof window !== 'undefined' && window.__PANKIT_CONFIG__?.lineLengthMax) || 80;
...
function checkTypography() {
  ...
  for (const el of document.querySelectorAll('p, h1, h2, h3, h4, h5, h6, li, td, th, dd, blockquote, figcaption, a, button, label, span')) {
    // Skip pankit's own elements
    if (el.closest && el.closest('.pankit-overlay, .pankit-label, .pankit-banner, .pankit-tooltip')) continue;
...
if (IS_BROWSER) {
  ...
  const EXTENSION_MODE = (_myScript && _myScript.dataset.pankitExtension === 'true')
    || document.documentElement.dataset.pankitExtension === 'true';
  ...
  styleEl.textContent = `
    @keyframes pankit-reveal {
      from { opacity: 0; }
      to { opacity: 1; }
    }
    .pankit-overlay:not(.pankit-banner) {
      pointer-events: none;
      outline: 2px solid ${OUTLINE_COLOR};
      border-radius: 4px;
      transition: outline-color 0.15s ease;
      animation: pankit-reveal 0.4s cubic-bezier(0.16, 1, 0.3, 1) both;
      animation-play-state: paused;
      border-top-left-radius: 0;
    }
    .pankit-overlay.pankit-visible {
      animation-play-state: running;
    }
    .pankit-overlay.pankit-hover {
      outline-color: ${BRAND_COLOR_HOVER};
      z-index: 100001 !important;
    }
    .pankit-overlay.pankit-hover .pankit-label {
      background: ${BRAND_COLOR_HOVER};
    }
    .pankit-overlay.pankit-spotlight {
      z-index: 100002 !important;
    }
    .pankit-overlay.pankit-spotlight-dimmed {
      opacity: 0.15 !important;
      animation: none !important;
      filter: blur(3px);
    }
    .pankit-spotlight-backdrop {
      position: fixed;
    ...
    .pankit-spotlight-backdrop.pankit-visible {
      opacity: 1;
    }
    .pankit-hidden .pankit-overlay${EXTENSION_MODE ? '' : ':not(.pankit-banner)'} {
  ...
  function getSpotlightBackdrop() {
    if (!spotlightBackdrop) {
      ...
      spotlightBackdrop.className = 'pankit-spotlight-backdrop';
  ...
  function showSpotlight(target) {
    ...
    if (window.__PANKIT_CONFIG__?.spotlightBlur === false) {
    ...
    bd.classList.add('pankit-visible');
  }

  function hideSpotlight() {
    spotlightTarget = null;
    if (spotlightBackdrop) spotlightBackdrop.classList.remove('pankit-visible');
  }
  ...
  function repositionOverlays() {
    for (const o of overlays) {
      if (!o._targetEl || o.classList.contains('pankit-banner')) continue;
  ...
  const visibilityObserver = new IntersectionObserver((entries) => {
    for (const entry of entries) {
      const overlay = entry.target._pankitOverlay;
      if (!overlay) continue;
      if (entry.isIntersecting) {
        ...
        if (!overlay._revealed) {
          ...
          requestAnimationFrame(() => {
            overlay.classList.add('pankit-visible');
  ...
  function detachOverlay(overlay) {
    ...
    if (overlay._targetEl && overlay._targetEl._pankitOverlay === overlay) {
      visibilityObserver.unobserve(overlay._targetEl);
      delete overlay._targetEl._pankitOverlay;
    }
  ...
  document.addEventListener('transitionend', (e) => {
    ...
    for (const o of overlays) {
      if (!o._targetEl || o.classList.contains('pankit-banner') || o.style.display === 'none') continue;
  ...
  const highlight = function(el, findings) {
    ...
    const onMouseEnter = () => {
      isHovered = true;
      outline.classList.add('pankit-hover');
    ...
    const onMouseLeave = () => {
      isHovered = false;
      outline.classList.remove('pankit-hover');
  ...
  const showPageBanner = function(findings) {
    ...
    banner.className = 'pankit-overlay pankit-banner';
    ...
    if (!EXTENSION_MODE) {
      ...
      toggle.addEventListener('click', () => {
        ...
        document.body.classList.toggle('pankit-hidden', !overlaysVisible);
  ...
  function buildSelectorSegment(el) {
    ...
    if (el.classList && el.classList.length > 0) {
      const classes = [...el.classList]
        .filter(c => !c.startsWith('pankit-') && !isLikelyHashedClass(c))
        .slice(0, 2);
      if (classes.length > 0) {
  ...
  function collectVisualContrastCandidates(options = {}) {
    ...
    for (const el of document.querySelectorAll('*')) {
      ...
      if (el.closest('.pankit-overlay, .pankit-label, .pankit-banner, .pankit-tooltip')) continue;
      if (el.closest('[id^="pankit-live-"]')) continue;
  ...
  async function sampleVisualBackgroundAtPoint(el, point, textColor, depth = 0) {
    ...
    for (const node of nodes) {
      ...
      if (node.closest?.('.pankit-overlay, .pankit-label, .pankit-banner, .pankit-tooltip')) continue;
  ...
  const printSummary = function(allFindings) {
    if (allFindings.length === 0) {
      console.log('%c[pankit] No anti-patterns found.', 'color: #22c55e; font-weight: bold');
      return;
    }
    console.group(
      `%c[pankit] ${allFindings.length} anti-pattern${allFindings.length === 1 ? '' : 's'} found`,
  ...
  function addBrowserFindings(groupMap, el, findings) {
    ...
    // Element-scoped waivers: a data-pankit-ignore ancestor suppresses
  ...
  function browserDesignSystemConfig() {
    const raw = window.__PANKIT_CONFIG__?.designSystem;
  ...
  function collectBrowserFindings() {
    ...
    const _disabled = EXTENSION_MODE ? (window.__PANKIT_CONFIG__?.disabledRules || []) : [];
    ...
    for (const el of document.querySelectorAll('*')) {
      // Skip pankit's own elements and any descendants (overlays, labels, banner, nav buttons)
      if (el.closest('.pankit-overlay, .pankit-label, .pankit-banner, .pankit-tooltip')) continue;
      ...
      // Skip the pankit live-mode overlay (highlight, tooltip, bar, picker, toast).
    ...
    // Clone the document and strip pankit-live overlay nodes before the
    ...
    for (const node of docClone.querySelectorAll('[id^="pankit-live-"]')) {
      node.remove();
    }
    ...
    // ground truth in the browser), and a match under a data-pankit-ignore
  ...
  function visualContrastMode(options = {}) {
    const explicit = typeof options.visualContrast === 'boolean'
      ...
      : typeof window.__PANKIT_CONFIG__?.visualContrast === 'boolean'
        ? window.__PANKIT_CONFIG__.visualContrast
        : null;
  ...
  function visualContrastOptions(options = {}) {
    const config = window.__PANKIT_CONFIG__ || {};
  ...
  function reportVisualContrastError(err, detail = {}) {
    window.dispatchEvent(new CustomEvent('pankit-visual-contrast-error', {
      detail: {
        ...detail,
        message: err?.message || String(err),
      },
    ...
    if (EXTENSION_MODE) {
      ...
      console.warn('[pankit] visual contrast scan failed', err);
    }
  }
        ...
        waitForVisualPaint()
          ...
          .then(result => {
            ...
            if (added) {
              ...
              window.dispatchEvent(new CustomEvent('pankit-visual-contrast-resolved', {
                detail: {
  ...
  if (EXTENSION_MODE) {
    ...
    window.addEventListener('message', (e) => {
      if (e.source !== window || !e.data || e.data.source !== 'pankit-command') return;
      if (e.data.action === 'scan') {
        if (e.data.config) window.__PANKIT_CONFIG__ = e.data.config;
        try {
          scan(e.data.config || {});
        } catch (err) {
          postExtensionError(err);
      ...
      if (e.data.action === 'toggle-overlays') {
        const visible = !document.body.classList.contains('pankit-hidden');
        document.body.classList.toggle('pankit-hidden', visible);
        window.postMessage({ source: 'pankit-overlays-toggled', visible: !visible }, '*');
      }
      if (e.data.action === 'remove') {
        clearOverlays();
        styleEl.remove();
        if (spotlightBackdrop) { spotlightBackdrop.remove(); spotlightBackdrop = null; }
        document.body.classList.remove('pankit-hidden');
      }
      if (e.data.action === 'highlight') {
        try {
          ...
          if (target) {
            ...
            for (const o of overlays) {
              if (o.classList.contains('pankit-banner')) continue;
              const isMatch = o._targetEl === target;
              o.classList.toggle('pankit-spotlight', isMatch);
              o.classList.toggle('pankit-spotlight-dimmed', !isMatch);
              if (isMatch) {
                // Force the matching overlay visible immediately, don't wait for IntersectionObserver
                o.style.display = '';
                o.style.animation = 'none';
                o.classList.add('pankit-visible');
                o._revealed = true;
                positionOverlay(o);
              }
            }
      ...
      if (e.data.action === 'unhighlight') {
        hideSpotlight();
        for (const o of overlays) {
          o.classList.remove('pankit-spotlight');
          o.classList.remove('pankit-spotlight-dimmed');
        }
      }
    });
    window.postMessage({ source: 'pankit-ready' }, '*');
  } else {
    if (window.__PANKIT_CONFIG__?.autoScan !== false) {
      const runAutoScan = () => {
        try {
          scan();
        } catch (err) {
          console.warn('[pankit] scan failed', err);
        }
      };
  ...
  window.pankitDetect = detect;
  window.pankitDetectAsync = detectAsync;
  window.pankitScan = scan;
  window.pankitScanAsync = scanAsync;
  ...
  window.pankitMeasureHiddenText = measureHiddenTextDOM;
  window.pankitCollectVisualContrastCandidates = collectVisualContrastCandidates;
  window.pankitAnalyzeVisualContrast = analyzeVisualContrast;
  window.pankitGetLastVisualContrastAnalyses = () => lastVisualContrastAnalyses.slice();
}
