Report and repair drift between this project's PanKit artifacts and what the installed version reads: PRODUCT.md, DESIGN.md and its `.pankit/design.json` sidecar, `.pankit/config.json`, persisted surface briefs, and the design hook.
...
- **Tool version.** The installed skill is older than the published one. `context.mjs` reports that at boot as `UPDATE_AVAILABLE` and `npx pankit update` fixes it. Not this command's job.
- **Schema drift.** An artifact was written by an older PanKit: fields nothing reads, fields now expected, files in retired locations. Mechanical, and this command repairs most of it.
...
```
node .claude/skills/pankit/scripts/doctor.mjs --json
```
...
- **`auto`** carries no decision. Run `node .claude/skills/pankit/scripts/doctor.mjs --fix` once to apply these, then report what it moved in one line. Do not ask permission first, and do not ask about them afterward.
...
`context.mjs` reports the cheap subset of these findings at session start, throttled to once a week per project. Set `"stalenessCheck": false` in `.pankit/config.json` to silence that, or `PANKIT_NO_STALENESS_CHECK=1` for one session. This command still works with the check disabled, and that is the combination to suggest for a user who wants the report only when they ask for it.
