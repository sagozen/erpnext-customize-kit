...
function writeFixture(root, { plugin, marketplace, subtreePlugin, codexPlugin, skill } = {}) {
  ...
  if (plugin !== undefined) {
    write('.claude-plugin/plugin.json', JSON.stringify({ name: 'pankit', version: plugin }, null, 2));
  }
  if (marketplace !== undefined) {
    write('.claude-plugin/marketplace.json', JSON.stringify({ plugins: [{ name: 'pankit', version: marketplace }] }, null, 2));
  }
  if (subtreePlugin !== undefined) {
    write('plugin/.claude-plugin/plugin.json', JSON.stringify({ name: 'pankit', version: subtreePlugin, skills: './skills/' }, null, 2));
  }
  if (codexPlugin !== undefined) {
    write('dist/openai/pankit/.codex-plugin/plugin.json', JSON.stringify({ name: 'pankit', version: codexPlugin, skills: './skills/' }, null, 2));
  }
  if (skill !== undefined) {
    write('plugin/skills/pankit/SKILL.md', skillMd(skill));
  }
}
...
describe('collectPluginVersions', () => {
  let root;
  beforeEach(() => {
    root = fs.mkdtempSync(path.join(os.tmpdir(), 'pankit-ver-'));
  });
  afterEach(() => {
  ...
  test('flags a stale Codex plugin manifest', () => {
    ...
    const { mismatches } = collectPluginVersions(root);
    expect(mismatches).toEqual([
      { relPath: 'dist/openai/pankit/.codex-plugin/plugin.json', found: '3.6.0', expected: '3.7.1' },
    ]);
  });
  ...
  test('flags a stale bundled SKILL.md frontmatter version', () => {
    ...
    const { mismatches } = collectPluginVersions(root);
    expect(mismatches).toEqual([
      { relPath: 'plugin/skills/pankit/SKILL.md', found: '3.1.1', expected: '3.7.1' },
    ]);
  });
  ...
  test('flags a root plugin.json that exists but has no version field', () => {
    fs.mkdirSync(path.join(root, '.claude-plugin'), { recursive: true });
    fs.writeFileSync(path.join(root, '.claude-plugin/plugin.json'), JSON.stringify({ name: 'pankit' }));
    const { source, errors } = collectPluginVersions(root);
...
describe('collectPluginVersions with CRLF line endings', () => {
  let root;
  beforeEach(() => {
    root = fs.mkdtempSync(path.join(os.tmpdir(), 'pankit-ver-crlf-'));
  });
  afterEach(() => {
    fs.rmSync(root, { recursive: true, force: true });
  });
  ...
  test('a CRLF-saved SKILL.md at the right version is not a false mismatch', () => {
    ...
    const skillPath = path.join(root, 'plugin/skills/pankit/SKILL.md');
