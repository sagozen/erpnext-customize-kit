...
describe('pankit ignores CLI', () => {
  ...
  function readConfig(name = 'config.json') {
    return JSON.parse(readFileSync(join(root, '.pankit', name), 'utf-8'));
  }
  ...
  test('rejects broad wildcard value ignores', () => {
    ...
    expect(existsSync(join(root, '.pankit', 'config.json'))).toBe(false);
  });

  test('removes an existing broad wildcard value ignore', () => {
    mkdirSync(join(root, '.pankit'), { recursive: true });
    writeFileSync(join(root, '.pankit', 'config.json'), JSON.stringify({
      detector: {
