...
beforeEach(() => {
  scratch = fs.mkdtempSync(path.join(os.tmpdir(), 'pankit-doctor-'));
});
...
describe('checkDesignCoverage', () => {
  ...
  it('allows Components to be absent from a marked seed document', () => {
    for (const command of ['/pankit', '$pankit']) {
      const design = [
...
describe('checkDetectorIgnores', () => {
  ...
  it('skips validation entirely when the registry is unavailable', () => {
    write('.pankit/config.json', JSON.stringify({ detector: { ignoreRules: ['whatever'] } }));
    assert.deepEqual(checkDetectorIgnores({ projectRoot: scratch, knownRuleIds: null }), []);
  });

  it('accepts the wildcard rule', () => {
    write('.pankit/config.json', JSON.stringify({ detector: { ignoreRules: ['*'] } }));
    assert.deepEqual(
...
describe('checkHookInstallation', () => {
  it('flags an installed hook whose script path does not resolve', () => {
    write('.claude/settings.json', JSON.stringify({
      hooks: { Stop: [{ hooks: [{ command: 'node .claude/skills/pankit/scripts/hook.mjs' }] }] },
    }));
  ...
  it('stays quiet when the hook script is present', () => {
    write('.claude/skills/pankit/scripts/hook.mjs', '// hook\n');
    write('.claude/settings.json', JSON.stringify({
      hooks: { Stop: [{ hooks: [{ command: 'node .claude/skills/pankit/scripts/hook.mjs' }] }] },
    }));
    assert.deepEqual(
  ...
  it('flags an installed manifest contradicted by hook.enabled false', () => {
    ...
    write('.pankit/config.json', JSON.stringify({ hook: { enabled: false } }));
    const findings = checkHookInstallation({
      projectRoot: scratch, repoRoot: scratch, providerId: 'claude-code',
    });
  ...
  // The manifest `pankit hooks on` actually writes, verbatim: a
  ...
  const claudeManifest = () => ({
    hooks: {
      PostToolUse: [{
        ...
        hooks: [{ type: 'command', command: 'node "${CLAUDE_PROJECT_DIR}/.claude/skills/pankit/scripts/hook.mjs"' }],
      }],
      Stop: [{ hooks: [{ type: 'command', command: 'node "${CLAUDE_PROJECT_DIR}/.claude/skills/pankit/scripts/hook.mjs"' }] }],
    },
  });
  ...
  it('handles the #399 guarded project-relative form', () => {
    const p = '"${CLAUDE_PROJECT_DIR}/.claude/skills/pankit/scripts/hook.mjs"';
  ...
  it('handles the #399 guarded absolute form (user-level installs)', () => {
    const abs = path.join(scratch, '.claude', 'skills', 'pankit', 'scripts', 'hook.mjs');
  ...
  it('never reports missing for the GitHub $(git rev-parse) form', () => {
    ...
    write('.github/hooks/pankit.json', JSON.stringify({
      hooks: { postToolUse: [{ bash: 'node "$(git rev-parse --show-toplevel)/.github/skills/pankit/scripts/hook.mjs"' }] },
    }));
    assert.deepEqual(
  ...
  it('never reports missing for plugin-root placeholders the doctor cannot map', () => {
    for (const token of ['${CLAUDE_PLUGIN_ROOT}', '${PLUGIN_ROOT}', '${GROK_PLUGIN_ROOT}']) {
      ...
      write('.claude/settings.json', JSON.stringify({
        hooks: { Stop: [{ hooks: [{ command: `node "${token}/skills/pankit/scripts/hook.mjs"` }] }] },
      }));
      assert.deepEqual(
        checkHookInstallation({ projectRoot: scratch, repoRoot: scratch, providerId: 'claude-code' }),
        [],
...
describe('doctor CLI', () => {
  function run(args = [], cwd = scratch) {
    return spawnSync(process.execPath, [DOCTOR_PATH, ...args], {
      cwd,
      encoding: 'utf8',
      env: { ...process.env, PANKIT_NO_UPDATE_CHECK: '1' },
    });
  }
  ...
  it('keeps boot and deep findings in their established artifact order', () => {
    ...
    write('.pankit/config.json', JSON.stringify({ unknownSetting: true }));
  ...
  it('applies only the automatic migrations under --fix', () => {
    write('PRODUCT.md', CURRENT_PRODUCT.replace('<!-- pankit:product-schema 1 -->\n\n', ''));
    write('DESIGN.json', JSON.stringify({ schemaVersion: 2 }));
    const res = run(['--fix']);
    assert.equal(res.status, 0, res.stderr);
    assert.match(res.stdout, /Moved DESIGN\.json to \.pankit\/design\.json/);
    assert.match(res.stdout, /Stamped PRODUCT\.md/);
    assert.ok(fs.existsSync(path.join(scratch, '.pankit', 'design.json')));
    assert.ok(!fs.existsSync(path.join(scratch, 'DESIGN.json')));
    assert.match(fs.readFileSync(path.join(scratch, 'PRODUCT.md'), 'utf8'), /pankit:product-schema 1/);
  });
  ...
  it('leaves a legacy sidecar alone when the canonical one already exists', () => {
    ...
    write('.pankit/design.json', JSON.stringify({ schemaVersion: 2, title: 'current' }));
    const res = run(['--fix', '--json']);
    assert.equal(res.status, 0, res.stderr);
    const report = JSON.parse(res.stdout);
    assert.equal(report.fixes.applied.length, 0);
    assert.equal(
      JSON.parse(fs.readFileSync(path.join(scratch, '.pankit', 'design.json'), 'utf8')).title,
      'current',
    );
    assert.ok(fs.existsSync(path.join(scratch, 'DESIGN.json')));
  });
