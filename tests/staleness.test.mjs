/**
 * Tests for staleness detection of PanKit's own project artifacts.
...
beforeEach(() => {
  scratch = fs.mkdtempSync(path.join(os.tmpdir(), 'pankit-stale-'));
  savedCacheEnv = process.env.PANKIT_STALENESS_CACHE;
  process.env.PANKIT_STALENESS_CACHE = path.join(scratch, 'notice.json');
});

afterEach(() => {
  if (savedCacheEnv === undefined) delete process.env.PANKIT_STALENESS_CACHE;
  else process.env.PANKIT_STALENESS_CACHE = savedCacheEnv;
...
describe('checkDesignSidecar', () => {
  ...
  it('accepts a current sidecar in the canonical location', () => {
    write('.pankit/design.json', JSON.stringify({ schemaVersion: 2 }));
    assert.deepEqual(checkDesignSidecar({ sidecarCandidates: candidates(), projectRoot: scratch }), []);
  });

  it('flags an outdated schema version as a routable repair', () => {
    write('.pankit/design.json', JSON.stringify({ schemaVersion: 1 }));
  ...
  it('treats a missing schemaVersion as an outdated sidecar', () => {
    write('.pankit/design.json', JSON.stringify({ title: 'x' }));
    assert.deepEqual(
  ...
  it('flags a retired location as an automatic migration', () => {
    ...
    assert.match(findings[0].fix, /\.pankit\/design\.json/);
  });

  it('flags a sidecar older than the DESIGN.md it extends', () => {
    const sidecar = write('.pankit/design.json', JSON.stringify({ schemaVersion: 2 }));
...
describe('checkConfig', () => {
  it('accepts a config using only recognized keys', () => {
    write('.pankit/config.json', JSON.stringify({
      updateCheck: true,
      projectRoots: ['apps/*'],
  ...
  it('flags unrecognized top-level keys', () => {
    write('.pankit/config.json', JSON.stringify({ updateChek: true }));
  ...
  it('flags unrecognized detector keys, the singular-typo case', () => {
    write('.pankit/config.json', JSON.stringify({ detector: { ignoreRule: ['side-tab'] } }));
    const findings = checkConfig({ projectRoot: scratch, repoRoot: scratch });
  ...
  it('checks the local config too', () => {
    write('.pankit/config.local.json', JSON.stringify({ bogus: 1 }));
    const findings = checkConfig({ projectRoot: scratch, repoRoot: scratch });
    assert.deepEqual(ids(findings), ['config-unknown-keys']);
  ...
  it('does not check the hook subtree, which has many writers', () => {
    write('.pankit/config.json', JSON.stringify({ hook: { somethingRuntime: true } }));
    assert.deepEqual(checkConfig({ projectRoot: scratch, repoRoot: scratch }), []);
  });

  it('ignores a malformed config rather than reporting it as drift', () => {
    write('.pankit/config.json', '{ not json');
    assert.deepEqual(checkConfig({ projectRoot: scratch, repoRoot: scratch }), []);
  });

  it('accepts both build-path values', () => {
    for (const value of ['comp', 'code']) {
      write('.pankit/config.json', JSON.stringify({ buildPath: value }));
      assert.deepEqual(checkConfig({ projectRoot: scratch, repoRoot: scratch }), []);
    }
  });
...
describe('checkBuildPathUnset', () => {
  ...
  it('flags a project that has done direction work and recorded nothing', () => {
    fs.mkdirSync(path.join(scratch, '.pankit', 'surfaces'), { recursive: true });
  ...
  it('accepts decision mocks as the same evidence', () => {
    fs.mkdirSync(path.join(scratch, '.pankit', 'mocks', 'decision'), { recursive: true });
  ...
  it('stays silent once a value is recorded, in either config file', () => {
    ...
    fs.rmSync(path.join(scratch, '.pankit', 'config.json'));
    write('.pankit/config.local.json', JSON.stringify({ buildPath: 'comp' }));
    assert.deepEqual(checkBuildPathUnset(args()), []);
  });

  it('defers to the invalid-value finding rather than reporting the key twice', () => {
    fs.mkdirSync(path.join(scratch, '.pankit', 'surfaces'), { recursive: true });
    write('.pankit/config.json', JSON.stringify({ buildPath: 'comp-first' }));
    assert.deepEqual(checkBuildPathUnset(args()), []);
  });
});
...
describe('checkSurfaceBriefs', () => {
  it('flags a brief whose primary target no longer exists', () => {
    const findings = checkSurfaceBriefs({
      projectRoot: scratch,
      candidates: [{ path: '.pankit/surfaces/pricing.md', primaryTarget: 'src/Pricing.tsx' }],
    });
...
describe('staleness notices', () => {
  ...
  it('keys throttling per project', () => {
    const other = fs.mkdtempSync(path.join(os.tmpdir(), 'pankit-stale-other-'));
    try {
  ...
  it('honors stalenessCheck false in config, local overriding shared', () => {
    ...
    write('.pankit/config.json', JSON.stringify({ stalenessCheck: false }));
    assert.equal(stalenessCheckDisabled([scratch]), true);
    write('.pankit/config.local.json', JSON.stringify({ stalenessCheck: true }));
    assert.equal(stalenessCheckDisabled([scratch]), false);
  });

  it('honors the environment opt-out', () => {
    process.env.PANKIT_NO_STALENESS_CHECK = '1';
    try {
      assert.equal(stalenessCheckDisabled([scratch]), true);
    } finally {
      delete process.env.PANKIT_NO_STALENESS_CHECK;
    }
  });
});
...
describe('context.mjs CONTEXT_STALE directive', () => {
  function run(env = {}) {
    return spawnSync(process.execPath, [SCRIPT_PATH], {
      ...
      env: {
        ...process.env,
        PANKIT_NO_UPDATE_CHECK: '1',
        PANKIT_STALENESS_CACHE: path.join(scratch, 'notice.json'),
        ...env,
      },
    });
  }
  ...
  it('respects PANKIT_NO_STALENESS_CHECK', () => {
    write('PRODUCT.md', '# Product\n\n## Register\n\nbrand\n');
    const res = run({ PANKIT_NO_STALENESS_CHECK: '1' });
    assert.equal(res.status, 0, res.stderr);
    assert.doesNotMatch(res.stdout, /CONTEXT_STALE:/);
  });
  ...
  it('checks sidecar and config drift with no PRODUCT.md present', () => {
    write('.pankit/design.json', JSON.stringify({ schemaVersion: 1 }));
    write('.pankit/config.json', JSON.stringify({ detector: { ignoreRule: [] } }));
    const res = run();
    assert.equal(res.status, 0, res.stderr);
  ...
  it('flags projectRoots globs that match nothing in a monorepo', () => {
    ...
    write('.pankit/config.json', JSON.stringify({ projectRoots: ['services/*'] }));
    write('PRODUCT.md', CURRENT_PRODUCT);
    const res = run();
    assert.equal(res.status, 0, res.stderr);
