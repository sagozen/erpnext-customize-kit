/**
 * Tests for project-local PanKit path resolution.
 * Run with: node --test tests/pankit-paths.test.mjs
 */
...
} from '../skill/scripts/lib/pankit-paths.mjs';

describe('pankit project paths', () => {
  let tmp;

  beforeEach(() => {
    tmp = mkdtempSync(join(tmpdir(), 'pankit-paths-'));
  });
  ...
  it('resolves the generated design sidecar under .pankit', () => {
    assert.equal(getDesignSidecarPath(tmp), join(tmp, '.pankit', 'design.json'));
  ...
  it('uses .pankit/live/config.json as the default live config path', () => {
  ...
  it('falls back to legacy scripts/config.json when no new live config exists', () => {
    const scriptsDir = join(tmp, 'skills', 'pankit', 'scripts');
  ...
  it('lets PANKIT_LIVE_CONFIG override both new and legacy config locations', () => {
    const override = join(tmp, 'custom-live-config.json');
    mkdirSync(join(tmp, '.pankit', 'live'), { recursive: true });
    writeFileSync(getLiveConfigPath(tmp), '{"source":"new"}');
    writeFileSync(override, '{"source":"override"}');

    assert.equal(
      resolveLiveConfigPath({ cwd: tmp, scriptsDir: join(tmp, 'scripts'), env: { PANKIT_LIVE_CONFIG: 'custom-live-config.json' } }),
      override,
    );
  });

  it('places live server, session, and annotation state under .pankit/live', () => {
    assert.equal(getLiveServerPath(tmp), join(tmp, '.pankit', 'live', 'server.json'));
    assert.equal(getLiveSessionsDir(tmp), join(tmp, '.pankit', 'live', 'sessions'));
    assert.equal(getLiveAnnotationsDir(tmp), join(tmp, '.pankit', 'live', 'annotations'));
  });

  it('places .pankit state under the active monorepo child project', () => {
    ...
    assert.equal(getDesignSidecarPath(tmp, options), join(projectRoot, '.pankit', 'design.json'));
    assert.equal(getLiveConfigPath(tmp, options), join(projectRoot, '.pankit', 'live', 'config.json'));
    assert.equal(getLiveServerPath(tmp, options), join(projectRoot, '.pankit', 'live', 'server.json'));
    assert.equal(getLiveSessionsDir(tmp, options), join(projectRoot, '.pankit', 'live', 'sessions'));
    assert.equal(getLiveAnnotationsDir(tmp, options), join(projectRoot, '.pankit', 'live', 'annotations'));
    assert.equal(getCritiqueDir(tmp, options), join(projectRoot, '.pankit', 'critique'));
    assert.equal(getLegacyLiveServerPath(tmp, options), join(projectRoot, '.pankit-live.json'));
  ...
  it('does not let a root live config shadow a child project live config path', () => {
    ...
    mkdirSync(join(tmp, '.pankit', 'live'), { recursive: true });
    writeFileSync(join(tmp, '.pankit', 'live', 'config.json'), '{"source":"root"}');

    assert.equal(
      resolveLiveConfigPath({ cwd: tmp, scriptsDir: join(tmp, 'scripts'), env: {}, targetPath: 'apps/admin/src/App.jsx' }),
      join(tmp, 'apps', 'admin', '.pankit', 'live', 'config.json'),
    );
  });
