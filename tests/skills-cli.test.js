/**
 * End-to-end tests for `pankit skills` subcommands.
 *
 ...
 * `bun run test:cli-remote-e2e` (PANKIT_CLI_REMOTE_E2E=1), skipping
 * gracefully when pankit.style is unreachable.
 */
...
function createFakeSkills(root, skills = ['audit', 'polish', 'pankit'], providers = ['.claude']) {
  for (const provider of providers) {
    for (const skill of skills) {
      ...
      writeFileSync(join(skillDir, 'SKILL.md'), [
        ...
        'Use the pankit skill for setup.',
      ].join('\n'));
    }
  }
}
...
function createFakeLinkSource(root, providers = ['.claude']) {
  for (const provider of providers) {
    writeSkill(join(root, '.pankit', 'dist', 'universal'), provider, 'pankit');
  }
}
...
function createFakeUniversalBundle(root, providers = ['.claude', '.agents', '.cursor']) {
  ...
  for (const provider of providers) {
    const skillDir = join(bundleRoot, provider, 'skills', 'pankit');
  ...
  if (providers.includes('.claude')) {
    ...
    writeFileSync(join(bundleRoot, '.claude', 'settings.json'), JSON.stringify({
      ...
      hooks: { PostToolUse: [{ matcher: 'Edit', hooks: [{ type: 'command', command: 'node ".claude/skills/pankit/scripts/hook.mjs"' }] }] },
    }, null, 2));
    writeFileSync(join(bundleRoot, '.claude', 'agents', 'pankit-finish-reviewer.md'),
      '---\nname: pankit-finish-reviewer\ndescription: Reviews a finished build.\n---\nClaude reviewer body.\n');
  }
  if (providers.includes('.cursor')) {
    ...
    writeFileSync(join(bundleRoot, '.cursor', 'hooks.json'), JSON.stringify({
      version: 1,
      hooks: { preToolUse: [{ command: 'node ".cursor/skills/pankit/scripts/hook-before-edit.mjs"' }] },
    }, null, 2));
  }
  if (providers.includes('.agents')) {
    ...
    writeFileSync(join(bundleRoot, '.codex', 'hooks.json'), JSON.stringify({
      hooks: { PostToolUse: [{ matcher: 'apply_patch', hooks: [{ type: 'command', command: 'node ".codex/skills/pankit/scripts/hook.mjs"' }] }] },
    }, null, 2));
  }
  if (providers.includes('.grok')) {
    mkdirSync(join(bundleRoot, '.grok', 'hooks'), { recursive: true });
    writeFileSync(join(bundleRoot, '.grok', 'hooks', 'pankit.json'), JSON.stringify({
      hooks: { PostToolUse: [{ matcher: 'Edit|Write|MultiEdit', hooks: [{ type: 'command', command: 'node ".grok/skills/pankit/scripts/hook.mjs"' }] }] },
    }, null, 2));
  }
  ...
  if (providers.includes('.github')) {
    ...
    writeFileSync(join(bundleRoot, '.github', 'agents', 'pankit-finish-reviewer.agent.md'),
      '---\nname: pankit-finish-reviewer\ndescription: Reviews a finished build.\n---\nCopilot reviewer body.\n');
    writeFileSync(join(bundleRoot, '.github', 'agents', 'pankit-asset-producer.agent.md'),
      '---\nname: pankit-asset-producer\ndescription: Produces assets.\n---\nCopilot producer body.\n');
  }
  if (providers.includes('.cursor')) {
    mkdirSync(join(bundleRoot, '.cursor', 'agents'), { recursive: true });
    writeFileSync(join(bundleRoot, '.cursor', 'agents', 'pankit-finish-reviewer.md'),
 ...
 * skill lives at `<prefix>pankit`. Optionally drop in a third-party skill
...
function createPrefixedInstall(root, { prefix = 'i-', providers = ['.claude'], foreign = null } = {}) {
  for (const provider of providers) {
    writeSkill(root, provider, `${prefix}pankit`);
...
// Remote e2e blocks (real bundle downloads from pankit.style) run only
// under `bun run test:cli-remote-e2e` (PANKIT_CLI_REMOTE_E2E=1). The default
...
const WANT_CLI_REMOTE_E2E = process.env.PANKIT_CLI_REMOTE_E2E === '1';
let bundleReachable = false;
if (WANT_CLI_REMOTE_E2E) {
  try {
    execSync('curl -sfIL --max-time 10 https://pankit.style/api/download/bundle/universal -o /dev/null', { stdio: 'pipe' });
...
describe('copyProviderSkills: symlink handling', () => {
  test('preserves an external shared-skills symlink and writes through it (#295)', () => {
    ...
    expect(existsSync(join(shared, 'pankit', 'SKILL.md'))).toBe(true);
  ...
  test('still converts an in-project cross-provider link to a real dir', () => {
    ...
    expect(existsSync(join(skillsPath, 'pankit', 'SKILL.md'))).toBe(true);
    rmSync(tmp, { recursive: true, force: true });
  });
...
describe('copyProviderAgents: Claude, Copilot, and Cursor subagents', () => {
  test('Claude project and user scopes use .claude/agents, with project copies taking precedence', () => {
    ...
    writeFileSync(join(home, '.claude', 'agents', 'pankit-finish-reviewer.md'), 'stale copy\n');
    ...
    expect(readFileSync(join(tmp, '.claude', 'agents', 'pankit-finish-reviewer.md'), 'utf8'))
      .toContain('Claude reviewer body.');
    expect(readFileSync(join(home, '.claude', 'agents', 'pankit-finish-reviewer.md'), 'utf8'))
      .toContain('Claude reviewer body.');
  ...
  test('Claude install and update backfill bundled agents beside an unchanged skill', () => {
    ...
    const env = { ...process.env, HOME: home, PANKIT_BUNDLE_PATH: bundleRoot };
    const agentPath = join(tmp, '.claude', 'agents', 'pankit-finish-reviewer.md');
  ...
  test('project scope places agents at .github/agents/ and .cursor/agents/', () => {
    ...
    expect(existsSync(join(tmp, '.github', 'agents', 'pankit-finish-reviewer.agent.md'))).toBe(true);
    expect(existsSync(join(tmp, '.github', 'agents', 'pankit-asset-producer.agent.md'))).toBe(true);
    expect(existsSync(join(tmp, '.cursor', 'agents', 'pankit-finish-reviewer.md'))).toBe(true);
  ...
  test('user scope places Copilot agents at ~/.copilot/agents (not ~/.github) and Cursor agents at ~/.cursor/agents, overwriting stale copies', () => {
    ...
    writeFileSync(join(home, '.copilot', 'agents', 'pankit-finish-reviewer.agent.md'), 'stale copy\n');

    copyProviderAgents(bundle, home, ['.github', '.cursor'], { scope: 'user' });

    const copilotAgent = readFileSync(join(home, '.copilot', 'agents', 'pankit-finish-reviewer.agent.md'), 'utf8');
    expect(copilotAgent).toContain('Copilot reviewer body.');
    expect(existsSync(join(home, '.cursor', 'agents', 'pankit-finish-reviewer.md'))).toBe(true);
  ...
  test('skills check accepts current Copilot user agents in a home-rooted checkout', () => {
    ...
    expect(existsSync(join(home, '.copilot', 'agents', 'pankit-finish-reviewer.agent.md'))).toBe(true);
    expect(existsSync(join(home, '.github', 'agents'))).toBe(false);
  ...
  test('inferred home-rooted updates refresh stale or missing Copilot user agents', () => {
    ...
    const bundleRoot = createFakeUniversalBundle(home, ['.github']);
    const env = { ...process.env, HOME: home, PANKIT_BUNDLE_PATH: bundleRoot };
    const userAgent = join(home, '.copilot', 'agents', 'pankit-finish-reviewer.agent.md');
    const projectAgent = join(home, '.github', 'agents', 'pankit-finish-reviewer.agent.md');
  ...
  test('project scope reports user-level Copilot agents that shadow the installed ones; Cursor never does (project wins there)', () => {
    ...
    const bundle = createFakeUniversalBundle(tmp, ['.github', '.cursor']);
    mkdirSync(join(home, '.copilot', 'agents'), { recursive: true });
    writeFileSync(join(home, '.copilot', 'agents', 'pankit-finish-reviewer.agent.md'), 'user-level copy\n');
    mkdirSync(join(home, '.cursor', 'agents'), { recursive: true });
    writeFileSync(join(home, '.cursor', 'agents', 'pankit-finish-reviewer.md'), 'user-level copy\n');
    ...
    expect(github.shadowed).toEqual(['pankit-finish-reviewer.agent.md']);
  ...
  test('fresh install lays agents down alongside skills and reports them', () => {
    ...
    const output = run('skills install -y --no-hooks --providers=github,cursor', {
      cwd: tmp,
      env: { ...process.env, HOME: home, PANKIT_BUNDLE_PATH: bundleRoot },
    });

    expect(output).toContain('Installed pankit into: .github, .cursor (project)');
...
describe('skills install: already-installed detection', () => {
  test('detects pankit sentinel and bails', () => {
    ...
    writeFileSync(join(tmp, '.claude', 'settings.local.json'), JSON.stringify({
      hooks: { PostToolUse: [{ matcher: 'Edit|Write|MultiEdit', hooks: [
        { type: 'command', command: 'node ".claude/skills/pankit/scripts/hook.mjs"' },
      ] }] },
    }));

    const output = run('skills install -y', {
      cwd: tmp,
      env: { ...process.env, PANKIT_BUNDLE_PATH: bundleRoot },
    });
  ...
  test('already-installed projects keep working when the update check is offline', () => {
    ...
    execSync('git init', { cwd: tmp });
    createFakeSkills(tmp, ['pankit'], ['.claude']);
    writeFileSync(join(tmp, '.claude', 'settings.local.json'), JSON.stringify({
      hooks: { PostToolUse: [{ matcher: 'Edit|Write|MultiEdit', hooks: [
        { type: 'command', command: 'node ".claude/skills/pankit/scripts/hook.mjs"' },
      ] }] },
    ...
    const output = run('skills install -y --providers=claude', {
      cwd: tmp,
      env: { ...process.env, PANKIT_BUNDLE_PATH: join(tmp, 'missing-bundle') },
    });

    expect(output).toContain('already installed');
    expect(output).toContain('Could not check for skill updates');
    expect(output).toContain('Existing skills were left unchanged.');
    expect(existsSync(join(tmp, '.claude', 'skills', 'pankit', 'SKILL.md'))).toBe(true);

    rmSync(tmp, { recursive: true, force: true });
  }, 15000);

  test('detects prefixed i-pankit', () => {
    const tmp = mkdtempSync(join(tmpdir(), 'imp-test-'));
    execSync('git init', { cwd: tmp });

    const skillDir = join(tmp, '.cursor', 'skills', 'i-pankit');
    mkdirSync(skillDir, { recursive: true });
    writeFileSync(join(skillDir, 'SKILL.md'), '---\nname: i-pankit\n---\n');
  ...
  test('repairs the hook when settings.local.json exists without the PanKit marker', () => {
    ...
    expect(JSON.stringify(merged)).toContain('skills/pankit/scripts/hook.mjs');
  ...
  test('a permissions entry mentioning the hook path is not mistaken for an installed hook', () => {
    ...
    writeFileSync(join(tmp, '.claude', 'settings.local.json'), JSON.stringify({
      permissions: { allow: ['Bash(node .claude/skills/pankit/scripts/hook.mjs:*)'] },
    }, null, 2));
    ...
    expect(merged.permissions.allow[0]).toContain('skills/pankit/scripts/hook.mjs');
...
describe('skills link: submodule installs', () => {
  test('creates relative skill symlinks from dist/universal', () => {
    ...
    const output = run('skills link --source=.pankit --providers=claude,cursor -y', { cwd: tmp });
    expect(output).toContain('Linked pankit into: .claude, .cursor');

    for (const provider of ['.claude', '.cursor']) {
      const dest = join(tmp, provider, 'skills', 'pankit');
      const src = join(tmp, '.pankit', 'dist', 'universal', provider, 'skills', 'pankit');
  ...
  test('is idempotent when links already point at the same source', () => {
    ...
    run('skills link --source=.pankit --providers=claude -y', { cwd: tmp });
    const before = readlinkSync(join(tmp, '.claude', 'skills', 'pankit'));
    const output = run('skills link --source=.pankit --providers=claude -y', { cwd: tmp });

    expect(output).toContain('already linked');
    expect(readlinkSync(join(tmp, '.claude', 'skills', 'pankit'))).toBe(before);
  ...
  test('does not overwrite an existing real skill unless forced', () => {
    ...
    execSync('git init', { cwd: tmp });
    createFakeLinkSource(tmp);
    writeSkill(tmp, '.claude', 'pankit');

    expect(() => run('skills link --source=.pankit --providers=claude -y', { cwd: tmp })).toThrow();
    const dest = join(tmp, '.claude', 'skills', 'pankit');
    expect(lstatSync(dest).isSymbolicLink()).toBe(false);

    const output = run('skills link --source=.pankit --providers=claude -y --force', { cwd: tmp });
  ...
  test('maps codex and rovo-dev provider aliases to their install folders', () => {
    ...
    run('skills link --source=.pankit --providers=codex,rovo-dev -y', { cwd: tmp });

    expect(lstatSync(join(tmp, '.agents', 'skills', 'pankit')).isSymbolicLink()).toBe(true);
    expect(lstatSync(join(tmp, '.rovodev', 'skills', 'pankit')).isSymbolicLink()).toBe(true);
  ...
  test('maps grok and grok-build provider aliases to .grok', () => {
    ...
    run('skills link --source=.pankit --providers=grok -y', { cwd: tmp });
    expect(lstatSync(join(tmp, '.grok', 'skills', 'pankit')).isSymbolicLink()).toBe(true);
    rmSync(tmp, { recursive: true, force: true });
    ...
    run('skills link --source=.pankit --providers=grok-build -y', { cwd: tmp2 });
    expect(lstatSync(join(tmp2, '.grok', 'skills', 'pankit')).isSymbolicLink()).toBe(true);
  ...
  test('plain install leaves linked installs on the submodule path', () => {
    ...
    const linkedDest = join(tmp, '.claude', 'skills', 'pankit');
    const before = readlinkSync(linkedDest);
    const copiedDest = join(tmp, '.cursor', 'skills', 'pankit');
  ...
  test('deduplicates providers that share one skills directory', () => {
    ...
    run('skills link --source=.pankit --providers=claude,codex -y', { cwd: tmp });

    const dest = join(tmp, '.agents', 'skills', 'pankit');
    const src = join(tmp, '.pankit', 'dist', 'universal', '.claude', 'skills', 'pankit');
    expect(lstatSync(dest).isSymbolicLink()).toBe(true);
    expect(realpathSync(dest)).toBe(realpathSync(src));
...
// when each command was its own skill). migrateUnprefixPanKit retires any
// old `<prefix>pankit` install back to the canonical `pankit`, so an
...
describe('skills: unprefix migration', () => {
  test('renames i-pankit back to pankit across every provider', () => {
    const tmp = mkdtempSync(join(tmpdir(), 'imp-test-mig-'));
    createPrefixedInstall(tmp, { prefix: 'i-', providers: ['.claude', '.cursor'] });

    const migrated = migrateUnprefixPanKit(tmp);
    expect(migrated).toBe(2); // one skill x two providers

    for (const provider of ['.claude', '.cursor']) {
      const skills = readdirSync(join(tmp, provider, 'skills'));
      expect(skills).toContain('pankit');
      expect(skills).not.toContain('i-pankit');
    }

    rmSync(tmp, { recursive: true, force: true });
  });

  test('migrates a custom prefix too (x-pankit)', () => {
    const tmp = mkdtempSync(join(tmpdir(), 'imp-test-mig-x-'));
    createPrefixedInstall(tmp, { prefix: 'x-' });

    expect(migrateUnprefixPanKit(tmp)).toBe(1);
    expect(readdirSync(join(tmp, '.claude', 'skills'))).toContain('pankit');
  ...
  test('REGRESSION: never touches third-party skills, even ones starting with i-', () => {
    ...
    // A foreign skill that shares the i- prefix but is NOT pankit.
    createPrefixedInstall(tmp, { prefix: 'i-', foreign: 'i-cool-skill' });

    const migrated = migrateUnprefixPanKit(tmp);
    expect(migrated).toBe(1); // only i-pankit
  ...
  test('leaves a clean pankit install alone (no-op)', () => {
    const tmp = mkdtempSync(join(tmpdir(), 'imp-test-mig-clean-'));
    createFakeSkills(tmp, ['pankit'], ['.claude']);

    expect(migrateUnprefixPanKit(tmp)).toBe(0);
    expect(readdirSync(join(tmp, '.claude', 'skills'))).toContain('pankit');
  ...
  test('leaves unrelated legacy skill names alone', () => {
    ...
    createFakeSkills(tmp, ['teach-pankit'], ['.claude']);

    expect(migrateUnprefixPanKit(tmp)).toBe(0);
    expect(readdirSync(join(tmp, '.claude', 'skills'))).toContain('teach-pankit');
...
describe('skills install/update: local universal bundle e2e', () => {
  test('root help advertises top-level skills commands', () => {
    ...
    expect(output).toContain('install                          Install pankit skills');
    expect(output).toContain('update                           Update skills to the latest version');
    expect(output).toContain('pankit skills <command>       Legacy namespace; still supported.');
    expect(output).not.toContain('skills install                   Install pankit skills');
  });
  ...
  test('top-level install aliases the legacy skills install command', () => {
    ...
    expect(output).toContain('Installed pankit into: .claude (project)');
    expect(existsSync(join(tmp, '.claude', 'skills', 'pankit', 'SKILL.md'))).toBe(true);
  ...
  test('install completion says /pankit init runs in the agent chat, not the terminal (#472)', () => {
    ...
    expect(output).toContain("type /pankit init in your AI coding agent's chat (not in this terminal)");

    rmSync(tmp, { recursive: true, force: true });
    rmSync(home, { recursive: true, force: true });
  }, 15000);

  test('`pankit init` in the shell points at the agent chat instead of "Unknown command" (#472)', () => {
    ...
    expect(stderr).toContain("Type /pankit init in your AI coding agent's chat");
  ...
  test('installs provider-specific skills into a fresh project', () => {
    ...
    for (const provider of ['.claude', '.agents', '.cursor']) {
      const skillDir = join(tmp, provider, 'skills', 'pankit');
    ...
    expect(codexHooks).toContain('.agents/skills/pankit/scripts/hook.mjs');
    expect(codexHooks).not.toContain('.codex/skills/pankit/scripts/hook.mjs');
  ...
  test('installs Antigravity skills into .agent/ with --providers=antigravity', () => {
    ...
    const skillDir = join(tmp, '.agent', 'skills', 'pankit');
    expect(existsSync(join(skillDir, 'SKILL.md'))).toBe(true);
  ...
  test('interactive install explains home detections and can target the project root', () => {
    ...
    for (const provider of ['.claude', '.agents', '.cursor', '.gemini']) {
      expect(existsSync(join(tmp, provider, 'skills', 'pankit', 'SKILL.md'))).toBe(true);
      expect(existsSync(join(home, provider, 'skills', 'pankit', 'SKILL.md'))).toBe(false);
    }
  ...
  test('interactive install can add providers beyond detected harnesses', () => {
    ...
    expect(output).toContain('Installed pankit into: .claude, .agents (project)');
    expect(existsSync(join(tmp, '.claude', 'skills', 'pankit', 'SKILL.md'))).toBe(true);
    expect(existsSync(join(tmp, '.agents', 'skills', 'pankit', 'SKILL.md'))).toBe(true);
  ...
  test('interactive install defaults config-only home detections to project scope', () => {
    ...
    expect(output).toContain('Installed pankit into: .claude, .agents, .cursor, .gemini (project)');
    for (const provider of ['.claude', '.agents', '.cursor', '.gemini']) {
      expect(existsSync(join(tmp, provider, 'skills', 'pankit', 'SKILL.md'))).toBe(true);
      expect(existsSync(join(home, provider, 'skills', 'pankit', 'SKILL.md'))).toBe(false);
    }
  ...
  test('interactive install defaults home detections with real skills to user scope', () => {
    ...
    expect(output).toContain('Installed pankit into: .claude, .agents, .cursor, .gemini (global)');
    for (const provider of ['.claude', '.agents', '.cursor', '.gemini']) {
      expect(existsSync(join(home, provider, 'skills', 'pankit', 'SKILL.md'))).toBe(true);
      expect(existsSync(join(tmp, provider, 'skills', 'pankit', 'SKILL.md'))).toBe(false);
    }
  ...
  test('Codex system/runtime-only skills do not count as real user skills', () => {
    ...
    expect(output).toContain('Installed pankit into: .agents (project)');
    expect(existsSync(join(tmp, '.agents', 'skills', 'pankit', 'SKILL.md'))).toBe(true);
    expect(existsSync(join(home, '.agents', 'skills', 'pankit', 'SKILL.md'))).toBe(false);
  ...
  test('--scope=global installs skills globally and project hooks point there', () => {
    ...
    expect(output).toContain('Installed pankit into: .claude, .agents, .cursor, .grok (global)');
    for (const provider of ['.claude', '.agents', '.cursor', '.grok']) {
      expect(existsSync(join(home, provider, 'skills', 'pankit', 'SKILL.md'))).toBe(true);
      expect(existsSync(join(tmp, provider, 'skills', 'pankit', 'SKILL.md'))).toBe(false);
    }
    expect(readFileSync(join(tmp, '.claude', 'settings.local.json'), 'utf8')).toContain(join(home, '.claude', 'skills', 'pankit', 'scripts', 'hook.mjs'));
    expect(readFileSync(join(tmp, '.codex', 'hooks.json'), 'utf8')).toContain(join(home, '.agents', 'skills', 'pankit', 'scripts', 'hook.mjs'));
    expect(readFileSync(join(tmp, '.cursor', 'hooks.json'), 'utf8')).toContain(join(home, '.cursor', 'skills', 'pankit', 'scripts', 'hook-before-edit.mjs'));
    const grokHooks = readFileSync(join(tmp, '.grok', 'hooks', 'pankit.json'), 'utf8');
    expect(grokHooks).toContain(join(home, '.grok', 'skills', 'pankit', 'scripts', 'hook.mjs'));
    expect(grokHooks).not.toContain('".grok/skills/pankit/scripts/hook.mjs"');
  ...
  test('global install detects ~/.pi and writes Pi skills to ~/.pi/agent/skills', () => {
    ...
    expect(output).toContain('Installed pankit into: .pi (global)');
    expect(existsSync(join(home, '.pi', 'agent', 'skills', 'pankit', 'SKILL.md'))).toBe(true);
    expect(existsSync(join(home, '.pi', 'skills', 'pankit'))).toBe(false);
    expect(existsSync(join(tmp, '.pi', 'skills', 'pankit', 'SKILL.md'))).toBe(false);
  ...
  test('global install writes OpenCode skills to ~/.config/opencode/skills (#406)', () => {
    ...
    expect(output).toContain('Installed pankit into: .opencode (global)');
    expect(existsSync(join(home, '.config', 'opencode', 'skills', 'pankit', 'SKILL.md'))).toBe(true);
    expect(existsSync(join(home, '.opencode', 'skills', 'pankit'))).toBe(false);
  ...
  test('OpenCode global dir honors OPENCODE_CONFIG_DIR and XDG_CONFIG_HOME (#406)', () => {
    ...
    execSync('git init', { cwd: tmp });
    const bundleRoot = createFakeUniversalBundle(tmp, ['.opencode']);
    const baseEnv = { ...process.env, HOME: home, PANKIT_BUNDLE_PATH: bundleRoot };
    ...
    expect(existsSync(join(home, 'occfg', 'skills', 'pankit', 'SKILL.md'))).toBe(true);

    run('skills install -y --providers=opencode --scope=global --no-hooks', {
      cwd: tmp,
      env: { ...baseEnv, XDG_CONFIG_HOME: join(home, 'xdg') },
    });
    expect(existsSync(join(home, 'xdg', 'opencode', 'skills', 'pankit', 'SKILL.md'))).toBe(true);
    expect(existsSync(join(home, '.opencode', 'skills', 'pankit'))).toBe(false);
  ...
  test('global OpenCode install migrates a legacy ~/.opencode/skills copy, sparing siblings (#406)', () => {
    ...
    execSync('git init', { cwd: tmp });
    writeSkill(home, '.opencode', 'pankit');
  ...
  test('OpenCode migration never follows a symlinked legacy skills dir (#406)', () => {
    ...
    writeSkill(join(home, '.config'), 'agents', 'pankit');
    mkdirSync(join(home, '.opencode'), { recursive: true });
    ...
    expect(existsSync(join(home, '.config', 'agents', 'skills', 'pankit', 'SKILL.md'))).toBe(true);
  ...
  test('OpenCode migration leaves a home-rooted repo project install alone (#406)', () => {
    ...
    expect(existsSync(join(home, '.config', 'opencode', 'skills', 'pankit', 'SKILL.md'))).toBe(true);
    expect(existsSync(join(home, '.opencode', 'skills', 'pankit', 'SKILL.md'))).toBe(true);
  ...
  test('project-scope install keeps Pi skills in .pi/skills even for a home-rooted repo', () => {
    ...
    execSync('git init', { cwd: home });
    writeSkill(join(home, '.pi'), 'agent', 'pankit');
    ...
    expect(output).toContain('Installed pankit into: .pi (project)');
    expect(existsSync(join(home, '.pi', 'skills', 'pankit', 'SKILL.md'))).toBe(true);
    // The pre-existing global copy is untouched, not refreshed in place.
    expect(readFileSync(join(home, '.pi', 'agent', 'skills', 'pankit', 'SKILL.md'), 'utf8')).toContain('name: pankit');
    expect(readFileSync(join(home, '.pi', 'agent', 'skills', 'pankit', 'SKILL.md'), 'utf8')).not.toContain('Local deterministic bundle');
    ...
    expect(readFileSync(join(home, '.pi', 'skills', 'pankit', 'SKILL.md'), 'utf8')).toContain('Local deterministic bundle');
    expect(readFileSync(join(home, '.pi', 'agent', 'skills', 'pankit', 'SKILL.md'), 'utf8')).toContain('Local deterministic bundle');
  ...
  test('recorded consent "declined" skips the hook (no prompt, no --no-hooks needed)', () => {
    ...
    execSync('git init', { cwd: tmp });
    const bundleRoot = createFakeUniversalBundle(tmp, ['.claude']);
    mkdirSync(join(tmp, '.pankit'), { recursive: true });
    writeFileSync(join(tmp, '.pankit', 'config.local.json'),
    ...
    expect(output).toContain('Installed pankit into: .claude');
    expect(output).not.toContain('Installed hooks into');
    expect(existsSync(join(tmp, '.claude', 'settings.local.json'))).toBe(false);
  ...
  test('--no-hooks records no consent decision (one-off skip)', () => {
    ...
    expect(existsSync(join(tmp, '.pankit', 'config.local.json'))).toBe(false);
  ...
  test('updates stale copied skills from the local bundle', () => {
    ...
    const skillDir = join(tmp, '.claude', 'skills', 'pankit');
    mkdirSync(skillDir, { recursive: true });
    writeFileSync(join(skillDir, 'SKILL.md'), '---\nname: pankit\nstale: true\n---\nOld content.\n');
  ...
  test('skills update refreshes script-only bundle changes when SKILL.md is unchanged', () => {
    ...
    const scriptPath = join(tmp, '.claude', 'skills', 'pankit', 'scripts', 'context.mjs');
  ...
  test('plain install refreshes an already-installed stale skill', () => {
    ...
    writeFileSync(
      ...
      readFileSync(join(bundleRoot, '.claude', 'skills', 'pankit', 'SKILL.md'), 'utf8')
    );
  ...
  test('plain install only refreshes selected copied providers', () => {
    ...
    expect(output).toContain('already installed');
    expect(output).toContain('Updated');
    expect(readFileSync(join(tmp, '.claude', 'skills', 'pankit', 'SKILL.md'), 'utf8')).toContain('version: 9.9.9-local');
    expect(readFileSync(join(tmp, '.cursor', 'skills', 'pankit', 'SKILL.md'), 'utf8')).toContain('stale: .cursor');
    expect(readFileSync(join(tmp, '.cursor', 'skills', 'pankit', 'scripts', 'context.mjs'), 'utf8')).toBe('console.log("old .cursor script");\n');
  ...
  test('explicit --providers installs a missing provider without --force (#500)', () => {
    ...
    expect(output).toContain('Installed pankit into: .cursor');
    expect(readFileSync(join(tmp, '.cursor', 'skills', 'pankit', 'SKILL.md'), 'utf8')).toContain('version: 9.9.9-local');
  ...
  test('--force reinstall over an old prefixed install lands on canonical pankit', () => {
    const tmp = mkdtempSync(join(tmpdir(), 'imp-test-local-force-'));
    execSync('git init', { cwd: tmp });
    const bundleRoot = createFakeUniversalBundle(tmp, ['.claude']);
    const prefixed = join(tmp, '.claude', 'skills', 'i-pankit');
...
describe('hook manifest merge helpers', () => {
  test('mergeHookManifests refreshes fresh description and version while preserving third-party hooks', () => {
    const merged = mergeHookManifests(
      {
        ...
        hooks: {
          preToolUse: [
            ...
            { command: 'node .cursor/skills/pankit/scripts/hook-before-edit.mjs' },
          ],
        },
      },
      {
    ...
    expect(merged.hooks.preToolUse.map((entry) => entry.command)).toEqual([
      ...
      'node .cursor/skills/pankit/scripts/hook-before-edit.mjs',
    ]);
  });

  test('mergeHookManifests replaces legacy Windows-path Claude hooks (#604)', () => {
    const legacyPath = 'C:\\Users\\alice\\.claude\\skills\\pankit\\scripts\\hook.mjs';
...
function createProjectDirBundle(root) {
  ...
  const skillDir = join(bundleRoot, '.claude', 'skills', 'pankit', 'scripts');
  mkdirSync(skillDir, { recursive: true });
  writeFileSync(join(bundleRoot, '.claude', 'skills', 'pankit', 'SKILL.md'),
    '---\nname: pankit\nversion: 9.9.9-local\n---\nbundle\n');
  mkdirSync(join(bundleRoot, '.claude'), { recursive: true });
  writeFileSync(join(bundleRoot, '.claude', 'settings.json'), JSON.stringify({
    hooks: {
      PostToolUse: [{ matcher: 'Edit|Write|MultiEdit', hooks: [
        { type: 'command', command: 'node "${CLAUDE_PROJECT_DIR}/.claude/skills/pankit/scripts/hook.mjs"' },
      ] }],
      Stop: [{ hooks: [
        { type: 'command', command: 'node "${CLAUDE_PROJECT_DIR}/.claude/skills/pankit/scripts/hook.mjs"' },
      ] }],
...
describe('copyProviderHooks: hook command path resolution (#399)', () => {
  test('project-scope hook keeps ${CLAUDE_PROJECT_DIR} and adds a missing-file guard', () => {
    ...
    for (const command of commands) {
      expect(command).toContain('${CLAUDE_PROJECT_DIR}/.claude/skills/pankit/scripts/hook.mjs');
  ...
  test('project hook pointing at a global skill uses the absolute skill path', () => {
    ...
    const commands = claudeHookCommands(join(tmp, '.claude', 'settings.local.json'));
    const absolute = join(skillHome, '.claude', 'skills', 'pankit', 'scripts', 'hook.mjs');
    for (const command of commands) {
...
describe('skills update: names the resolved target and honors scope (#399)', () => {
  test('user-level update writes an absolute, guarded hook to ~/.claude', () => {
    ...
    expect(existsSync(join(home, '.claude', 'skills', 'pankit', 'SKILL.md'))).toBe(true);
    ...
    expect(raw).toContain(join(home, '.claude', 'skills', 'pankit', 'scripts', 'hook.mjs'));
    expect(raw).not.toContain('${CLAUDE_PROJECT_DIR}');
    expect(raw).toContain('[ ! -f ');
    // The project dir was never touched.
    expect(existsSync(join(tmp, '.claude', 'skills', 'pankit'))).toBe(false);

    rmSync(tmp, { recursive: true, force: true });
    rmSync(home, { recursive: true, force: true });
  }, 20000);

  test('does not vendor pankit into a repo that only tracks OTHER skills', () => {
    ...
    // The repo tracks a first-party, NON-pankit skill under .claude/skills.
    writeSkill(tmp, '.claude', 'house-brand');
    // A real user-level pankit install exists.
    run('skills install -y --providers=claude --scope=global --no-hooks', { cwd: tmp, env });
...
describeRemote('skills update: refreshes from the production universal bundle', () => {
  ...
  beforeAll(() => {
    ...
    // Stale pankit skill that the update should overwrite with fresh,
    // compiled content. v3.0 ships a single `pankit` skill (with
  ...
  test('downloads the bundle and refreshes the pankit skill', () => {
    const output = run('skills update -y', { cwd: tmp });
    expect(output).toContain('Updated');

    // The skill now carries fresh, compiled content (no 'stale: true').
    const content = readFileSync(join(tmp, '.claude', 'skills', 'pankit', 'SKILL.md'), 'utf8');
    expect(content).not.toContain('stale: true');
  ...
  test('refreshed skill ships its compiled scripts directory', () => {
    ...
    expect(existsSync(join(tmp, '.claude', 'skills', 'pankit', 'scripts'))).toBe(true);
  });
});
...
describeRemote('skills install: production universal bundle download', () => {
  ...
  test('--force reinstall over an old prefixed install lands on canonical pankit', () => {
    ...
    // retire i-pankit so we are left with the canonical name only.
    const prefixed = join(tmp, '.claude', 'skills', 'i-pankit');
    mkdirSync(prefixed, { recursive: true });
    writeFileSync(join(prefixed, 'SKILL.md'), '---\nname: i-pankit\n---\n');
...
describe('hermesGlobalHome resolver (PR #521)', () => {
  ...
  test('end-to-end: --scope=user --providers=hermes with HERMES_HOME=profile lands in the active profile', () => {
    ...
    expect(existsSync(join(profileDir, 'skills', 'pankit', 'SKILL.md'))).toBe(true);
    expect(existsSync(join(home, '.hermes', 'skills', 'pankit', 'SKILL.md'))).toBe(false);
