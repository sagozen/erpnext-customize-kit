...
describe('release.mjs guards', () => {
  ...
  before(() => {
    root = fs.mkdtempSync(path.join(os.tmpdir(), 'pankit-release-'));
    ...
    write('.claude-plugin/plugin.json', JSON.stringify({ name: 'pankit', version: '1.2.3' }));
    write('.claude-plugin/marketplace.json', JSON.stringify({ plugins: [{ name: 'pankit', version: '1.2.3' }] }));
    write('package.json', JSON.stringify({ name: 'pankit', version: '9.9.9' }));
  ...
  it('renders a tweet within the 280-char limit with the release URL', () => {
    ...
    assert.match(tweetMatch[2], /PanKit v1\.2\.3 is out\./);
  ...
  it('refuses when plugin.json and marketplace.json disagree', () => {
    write('.claude-plugin/marketplace.json', JSON.stringify({ plugins: [{ name: 'pankit', version: '1.0.0' }] }));
    git(workDir, 'add', '-A');
