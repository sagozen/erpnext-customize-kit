 ...
 * 0-byte universal.zip and every `npx pankit install` failed with
 ...
 * made `npx pankit install` exit 0 after writing only a fraction of the
 * bundle.
 */
...
function makeUniversalTree(distDir) {
  const skillDir = join(distDir, 'universal', 'skills', 'pankit');
...
function makeLargeUniversalTree(distDir, { providers = ['.claude', '.cursor', '.agents', '.gemini', '.github', '.kiro'], scriptCount = 8, extraSkills = ['audit', 'polish'] } = {}) {
  ...
  for (const provider of providers) {
    // The pankit skill ships a scripts/ dir with many files, like the real
    // bundle. This is where most entries live.
    const scriptsDir = join(distDir, 'universal', provider, 'skills', 'pankit', 'scripts');
    mkdirSync(scriptsDir, { recursive: true });
    writeFileSync(join(distDir, 'universal', provider, 'skills', 'pankit', 'SKILL.md'), `---\nname: pankit\n---\nprovider ${provider}\n`);
    files += 1;
...
describe('release bundle zip writer', () => {
  it('createAllZips produces a non-empty universal.zip that unpacks to the skill tree', async () => {
    ...
    const skillMd = join(out, 'skills', 'pankit', 'SKILL.md');
    assert.ok(existsSync(skillMd), 'unpacked bundle is missing skills/pankit/SKILL.md');
    assert.match(readFileSync(skillMd, 'utf8'), /name: pankit/);
  ...
  it('REGRESSION: extracts every entry of a many-file bundle (no silent partial extraction on Node v24.16.0+)', async () => {
    ...
    // because nothing else keeps the event loop alive -- `npx pankit install`
