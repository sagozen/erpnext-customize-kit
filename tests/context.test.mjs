 ...
 *   3. PANKIT_CONTEXT_DIR env var as a power-user escape hatch (only
...
beforeEach(() => {
  scratch = fs.mkdtempSync(path.join(os.tmpdir(), 'pankit-loadctx-'));
  savedEnv = process.env.PANKIT_CONTEXT_DIR;
  delete process.env.PANKIT_CONTEXT_DIR;
});

afterEach(() => {
  if (savedEnv === undefined) delete process.env.PANKIT_CONTEXT_DIR;
  else process.env.PANKIT_CONTEXT_DIR = savedEnv;
  ...
  if (providerId) {
    ...
    fs.writeFileSync(
      ...
      fs.readFileSync(providerPath, 'utf8')
        .replace("PANKIT_PROVIDER_ID = 'source'", `PANKIT_PROVIDER_ID = '${providerId}'`),
    );
  }
...
describe('resolveContextDir', () => {
  ...
  it('uses PANKIT_CONTEXT_DIR as a fallback when defaults are empty (relative path)', () => {
    write('design/PRODUCT.md');
    process.env.PANKIT_CONTEXT_DIR = 'design';
    assert.equal(resolveContextDir(scratch), path.join(scratch, 'design'));
  });

  it('uses PANKIT_CONTEXT_DIR as a fallback when defaults are empty (absolute path)', () => {
    const elsewhere = fs.mkdtempSync(path.join(os.tmpdir(), 'pankit-elsewhere-'));
    try {
      process.env.PANKIT_CONTEXT_DIR = elsewhere;
  ...
  it('default paths win over PANKIT_CONTEXT_DIR (lazy escape hatch)', () => {
  ...
  it('ignores empty PANKIT_CONTEXT_DIR', () => {
    write('PRODUCT.md');
    process.env.PANKIT_CONTEXT_DIR = '   ';
    assert.equal(resolveContextDir(scratch), scratch);
  });
...
describe('loadContext (monorepo project context)', () => {
  ...
  it('does not discover dependency or generated directories as workspace candidates', () => {
    ...
    const res = spawnSync(process.execPath, [SCRIPT_PATH], {
      cwd: scratch,
      encoding: 'utf8',
      env: { ...process.env, PANKIT_NO_UPDATE_CHECK: '1', PANKIT_NO_STALENESS_CHECK: '1' },
    });
...
describe('loadContext (pankit projectRoots config)', () => {
  function writeSkinsConfig(extra = {}) {
    write('.pankit/config.json', JSON.stringify({ projectRoots: ['docs/design/skins/*'], ...extra }, null, 2));
    write('PRODUCT.md', '# Root product\n');
    write('DESIGN.md', '# Root design\n');
  }
  ...
  it('extends shared projectRoots with config.local.json', () => {
    write('.pankit/config.json', JSON.stringify({ projectRoots: ['docs/design/skins/*'] }, null, 2));
    write('.pankit/config.local.json', JSON.stringify({ projectRoots: ['experiments/*'] }, null, 2));
    write('PRODUCT.md', '# Root product\n');
    write('DESIGN.md', '# Root design\n');
  ...
  it('does not treat an .pankit config without projectRoots as a monorepo', () => {
    write('.pankit/config.json', JSON.stringify({ hook: { consent: 'accepted' } }, null, 2));
    write('PRODUCT.md', '# Root product\n');
    write('docs/design/skins/marble/DESIGN.md', '# Marble design\n');
  ...
  // projectRoots pattern (positive or negated) is governed by the pankit
  ...
  describe('composition with package-manager workspaces', () => {
    ...
    it('lets an pankit negation exclude a package-manager workspace', () => {
      write('package.json', JSON.stringify({ private: true, workspaces: ['apps/*'] }, null, 2));
      write('.pankit/config.json', JSON.stringify({ projectRoots: ['!apps/internal'] }, null, 2));
      write('PRODUCT.md', '# Root product\n');
      write('apps/dashboard/PRODUCT.md', '# Dashboard product\n');
    ...
    it('gives a broad pankit pattern the boundary over a deeper package workspace', () => {
      write('package.json', JSON.stringify({ private: true, workspaces: ['apps/web/packages/ui'] }, null, 2));
      write('.pankit/config.json', JSON.stringify({ projectRoots: ['apps/*'] }, null, 2));
      write('PRODUCT.md', '# Root product\n');
    ...
    it('falls through to package workspaces for paths pankit does not match', () => {
      write('package.json', JSON.stringify({ private: true, workspaces: ['apps/*'] }, null, 2));
      write('.pankit/config.json', JSON.stringify({ projectRoots: ['docs/design/skins/*'] }, null, 2));
      write('PRODUCT.md', '# Root product\n');
      write('apps/dashboard/PRODUCT.md', '# Dashboard product\n');
    ...
    it('resolves other workspaces normally when pankit config only negates', () => {
      write('package.json', JSON.stringify({ private: true, workspaces: ['apps/*'] }, null, 2));
      write('.pankit/config.json', JSON.stringify({ projectRoots: ['!apps/internal'] }, null, 2));
      write('PRODUCT.md', '# Root product\n');
      write('apps/dashboard/PRODUCT.md', '# Dashboard product\n');
...
describe('loadContext (PANKIT_CONTEXT_DIR escape hatch)', () => {
  ...
  it('reports a missing override directory as no-context, not as a crash', () => {
    process.env.PANKIT_CONTEXT_DIR = 'no/such/dir';
    const ctx = loadContext(scratch);
...
describe('context.mjs CLI', () => {
  it('emits NO_PRODUCT_MD directive when no PRODUCT.md is found', async () => {
    const { spawnSync } = await import('node:child_process');
    const res = spawnSync(process.execPath, [SCRIPT_PATH], { cwd: scratch, encoding: 'utf8', env: { ...process.env, PANKIT_NO_UPDATE_CHECK: '1' } });
    assert.equal(res.status, 0);
  ...
  describe('BUILD_PATH_DEFAULT', () => {
    ...
    it('reports a recorded preference from the shared config', () => {
      write('PRODUCT.md', '# Acme\n');
      write('.pankit/config.json', JSON.stringify({ buildPath: 'code' }));
      assert.match(run().stdout, /BUILD_PATH_DEFAULT: code \(from \.pankit\/config\.json\)/);
    });

    it('lets the gitignored local config win over the committed one', () => {
      write('PRODUCT.md', '# Acme\n');
      write('.pankit/config.json', JSON.stringify({ buildPath: 'comp' }));
      write('.pankit/config.local.json', JSON.stringify({ buildPath: 'code' }));
      assert.match(run().stdout, /BUILD_PATH_DEFAULT: code \(from \.pankit\/config\.local\.json\)/);
    });
    ...
    it('stays silent on a value nothing reads rather than guessing at it', () => {
      write('PRODUCT.md', '# Acme\n');
      write('.pankit/config.json', JSON.stringify({ buildPath: 'code-first' }));
      assert.equal(run().stdout.includes('BUILD_PATH_DEFAULT'), false);
    });
    ...
    describe('in a monorepo', () => {
      ...
      it('lets the workspace override the repo root, nearest root first', () => {
        writeWorkspace();
        write('.pankit/config.json', JSON.stringify({ buildPath: 'code' }));
        write('apps/dashboard/.pankit/config.json', JSON.stringify({ buildPath: 'comp' }));
      ...
      it('does not let the invoking workspace lend its value to the target', () => {
        ...
        write('.pankit/config.json', JSON.stringify({ buildPath: 'code' }));
        write('apps/marketing/.pankit/config.json', JSON.stringify({ buildPath: 'comp' }));
  ...
  it('keeps the manual-detector directive out of early context when the current provider hook is active', () => {
    const scripts = path.join(scratch, 'bundle', 'skills', 'pankit', 'scripts');
    ...
    fs.writeFileSync(path.join(project, '.codex', 'hooks.json'), JSON.stringify({
      hooks: { Stop: [{ hooks: [{ command: 'node .agents/skills/pankit/scripts/hook.mjs' }] }] },
    }));
    ...
    fs.mkdirSync(path.join(project, '.pankit'), { recursive: true });
    fs.writeFileSync(path.join(project, '.pankit', 'config.json'), JSON.stringify({ hook: { enabled: false } }));
  ...
  it('loads the only persisted surface brief as current task context', () => {
    write('PRODUCT.md', '# Acme product\n');
    write('DESIGN.md', '# Acme design\n');
    write('.pankit/surfaces/src-pages-pricing-astro.md', `---
version: 1
...
`);
    ...
    assert.equal(res.status, 0);
    assert.match(res.stdout, /# SURFACE BRIEF \(\.pankit\/surfaces\/src-pages-pricing-astro\.md\)/);
...
`);
    write('.pankit/surfaces/src-pages-home-astro.md', `---
version: 1
...
`);
  ...
  // The native platform axis survives from the design-focused original: the
  // loader still recognizes ios/android/adaptive, but this repository ships no
  // reference/ios.md or reference/android.md. Absent files must degrade to
  // silence rather than crash the boot, which is what these pin.
  it('recognizes an ios project and degrades quietly with no ios reference on disk', async () => {
    write('PRODUCT.md', '# Acme\n\n## Platform\n\nios\n');
    const { spawnSync } = await import('node:child_process');
    const res = spawnSync(process.execPath, [SCRIPT_PATH], { cwd: scratch, encoding: 'utf8', env: { ...process.env, PANKIT_NO_UPDATE_CHECK: '1' } });
    assert.equal(res.status, 0);
    assert.doesNotMatch(res.stdout, /# NATIVE PLATFORM REFERENCE/);
    assert.doesNotMatch(res.stdout, /NEXT STEP:.*reference\/ios\.md/);
  });

  it('recognizes an adaptive project and degrades quietly with no native references on disk', async () => {
    write('PRODUCT.md', '# Acme\n\n## Platform\n\nadaptive\n');
    const { spawnSync } = await import('node:child_process');
    const res = spawnSync(process.execPath, [SCRIPT_PATH], { cwd: scratch, encoding: 'utf8', env: { ...process.env, PANKIT_NO_UPDATE_CHECK: '1' } });
    assert.equal(res.status, 0);
  ...
  it('recognizes an android project and degrades quietly with no android reference on disk', async () => {
    write('PRODUCT.md', '# Acme\n\n## Platform\n\nandroid\n');
    const { spawnSync } = await import('node:child_process');
    const res = spawnSync(process.execPath, [SCRIPT_PATH], { cwd: scratch, encoding: 'utf8', env: { ...process.env, PANKIT_NO_UPDATE_CHECK: '1' } });
    assert.equal(res.status, 0);
...
describe('context.mjs update check', () => {
  ...
  function setup(cacheObj, { disable = false, host } = {}) {
    ...
    const env = {
      ...process.env,
      PANKIT_UPDATE_CACHE: cachePath(),
      PANKIT_NO_UPDATE_CHECK: disable ? '1' : '',
      ...
      PANKIT_NO_STALENESS_CHECK: '1',
      ...(host ? { PANKIT_UPDATE_HOST: host } : {}),
    };
  ...
  it('appends UPDATE_AVAILABLE when the cached latest version is newer', () => {
    ...
    assert.equal(res.status, 0);
    assert.match(res.stdout, /UPDATE_AVAILABLE: A newer PanKit skill is available/);
    assert.match(res.stdout, /installed v1\.0\.0, latest v2\.0\.0/);
    assert.match(res.stdout, /npx pankit update/);
  ...
  it('forbids running the update in the same turn, on any answer', () => {
    ...
    assert.match(stdout, /Do not run `npx pankit update` in this turn, whatever the user answers/);
  ...
  it('respects PANKIT_NO_UPDATE_CHECK', () => {
