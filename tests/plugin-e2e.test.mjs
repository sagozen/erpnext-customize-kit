...
const MARKETPLACE_NAME = 'pankit-e2e';
const PLUGIN_REF = `pankit@${MARKETPLACE_NAME}`;
...
describe('committed plugin subtree loads in a real Claude Code', { skip }, () => {
  ...
  before(() => {
    workDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pankit-plugin-e2e-'));
    ...
    fs.cpSync(PLUGIN_DIR, path.join(marketplaceDir, 'pankit'), { recursive: true });
    fs.writeFileSync(
      path.join(marketplaceDir, '.claude-plugin', 'marketplace.json'),
      JSON.stringify({
        name: MARKETPLACE_NAME,
        owner: { name: 'pankit plugin E2E' },
        plugins: [
          { name: 'pankit', source: './pankit', description: 'committed ./plugin subtree' },
        ],
      }, null, 2),
    );
  ...
  it('parses and loads the pankit skill', () => {
    assert.equal(componentCount('Skills'), 1);
    assert.match(detailsOutput, /Skills\s+\(1\)\s+pankit/);
  });
