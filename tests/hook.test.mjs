/**
 * Unit tests for the PanKit design hook.
...
import { normalizeIgnoreValueEntries as normalizeIgnoreValueEntriesCli } from '../cli/lib/pankit-config.mjs';
import { detectHtml, detectText } from '../cli/engine/detect-antipatterns.mjs';

function mkTmp() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'pankit-hook-'));
}
...
describe('readConfig()', () => {
  ...
  it('parses hook runtime and legacy hook detector filters', () => {
    fs.mkdirSync(path.join(cwd, '.pankit'), { recursive: true });
...
describe('ensureHookGitExcludes()', () => {
  ...
  it('adds hook runtime files to local git info exclude, not tracked .gitignore', () => {
    ...
    assert.match(exclude, /\.pankit\/hook\.cache\.json/);
    assert.match(exclude, /\.pankit\/hook\.pending\.json/);
    assert.match(exclude, /\.pankit\/config\.local\.json/);

    const second = ensureHookGitExcludes(cwd);
    assert.equal(second.changed, false);
    const rewritten = fs.readFileSync(path.join(cwd, '.git', 'info', 'exclude'), 'utf-8');
    assert.equal((rewritten.match(/pankit-hook-ignore-start/g) || []).length, 1);
  });
});
...
describe('hook-admin.mjs', () => {
  ...
  it('refuses an empty --file glob instead of silently writing a project-wide ignore', () => {
    ...
    assert.equal(fs.existsSync(path.join(cwd, '.pankit', 'config.json')), false, 'nothing may be written');
  });
  ...
  it('matches an on-disk scope whose glob order differs from the sorted argv form', () => {
    ...
    fs.mkdirSync(path.join(cwd, '.pankit'), { recursive: true });
    fs.writeFileSync(path.join(cwd, '.pankit', 'config.json'), JSON.stringify({
    ...
    const cfg = JSON.parse(fs.readFileSync(path.join(cwd, '.pankit', 'config.json'), 'utf-8'));
  ...
  it('ignore-value --local writes private config and status reports local ignores', () => {
    ...
    assert.match(status, /local file:\s+\.pankit\/config\.local\.json/);
  ...
  // hook-lib.mjs (skill, ships into harness dirs) and cli/lib/pankit-config.mjs
  ...
  it('both config normalizers emit identical entries', () => {
    ...
    assert.equal(
      ...
      'skill/scripts/hook-lib.mjs and cli/lib/pankit-config.mjs must agree, key order included',
    );
  ...
  it('a /pankit hooks edit preserves sibling hook fields (consent, quiet)', () => {
    fs.mkdirSync(path.join(cwd, '.pankit'), { recursive: true });
  ...
  it('hooks on accepts declined consent and installs missing provider manifests', () => {
    ...
    for (const provider of ['.claude', '.agents', '.cursor', '.github']) {
      fs.mkdirSync(path.join(cwd, provider, 'skills', 'pankit', 'scripts'), { recursive: true });
    }
    ...
    fs.writeFileSync(path.join(cwd, '.claude', 'settings.local.json'), JSON.stringify({
      hooks: {
        PostToolUse: [
          ...
          { matcher: 'Edit', hooks: [{ type: 'command', command: 'node ".claude/skills/pankit/scripts/hook.mjs"' }] },
        ],
      },
    }));
    ...
    // pankit entry must have been stripped, not accumulated.
    assert.equal(claude.split('skills/pankit/scripts/hook.mjs').length - 1, 2);
    assert.match(claude, /"Stop"/);
    const claudeManifest = JSON.parse(claude);
    const pankitGroup = claudeManifest.hooks.PostToolUse.find((group) =>
      group.hooks?.some((hook) => hook.command?.includes('skills/pankit/scripts/hook.mjs')));
    assert.ok(pankitGroup, 'repaired Claude settings should contain the PanKit PostToolUse group');
    assert.equal(pankitGroup.matcher, 'Edit|Write');

    const codex = fs.readFileSync(path.join(cwd, '.codex', 'hooks.json'), 'utf-8');
    assert.match(codex, /\.agents\/skills\/pankit\/scripts\/hook\.mjs/);
    const cursor = fs.readFileSync(path.join(cwd, '.cursor', 'hooks.json'), 'utf-8');
    assert.match(cursor, /\.cursor\/skills\/pankit\/scripts\/hook-before-edit\.mjs/);
    const github = JSON.parse(fs.readFileSync(path.join(cwd, '.github', 'hooks', 'pankit.json'), 'utf-8'));
    assert.equal(github.hooks.postToolUse[0].matcher, 'edit|create|apply_patch');
    assert.match(github.hooks.postToolUse[0].bash, /\.github\/skills\/pankit\/scripts\/hook\.mjs/);
  });
...
describe('renderTemplate()', () => {
  it('starts with the versioned envelope and caps to maxFindings', () => {
    ...
    assert.match(text, /\.\.\. and 7 more \(see \/pankit audit\)\./);
  ...
  it('emits a directive footer (triage branches + executable self-serve ignore + honest provenance)', () => {
    ...
    assert.match(text, /Full suppression ladder: \/pankit hooks/);
  });
  ...
  it('does not suggest ignore-value for rules that cannot be value-filtered', () => {
    ...
    assert.doesNotMatch(text, /\/pankit hooks ignore-value side-tab Inter/);
  });
...
describe('writeAuditLog()', () => {
  ...
  it('appends NDJSON when PANKIT_HOOK_LOG is set', () => {
    const log = path.join(cwd, 'audit.ndjson');
    writeAuditLog({ PANKIT_HOOK_LOG: log }, { event: 'PostToolUse', emitted: true });
    writeAuditLog({ PANKIT_HOOK_LOG: log }, { event: 'PostToolUse', emitted: false });
  ...
  it('is a no-op when PANKIT_HOOK_LOG is unset', () => {
  ...
  it('prefers the env var over config hook.auditLog', () => {
    ...
    writeAuditLog({ PANKIT_HOOK_LOG: envLog }, { event: 'PostToolUse' }, cwd);
  ...
  it('resolves config auditLog from entry.cwd (the event project root), not the fallback cwd', () => {
    ...
    fs.mkdirSync(path.join(projectDir, '.pankit'), { recursive: true });
    fs.writeFileSync(path.join(projectDir, '.pankit', 'config.json'),
...
`);
  ...
  it('emits a clean ack when the file has zero findings', async () => {
    ...
    assert.match(r.stdout, /keep following the project design system and the pankit skill guidance/);
    assert.equal(r.audit.emitted, true);
  ...
  it('PANKIT_HOOK_QUIET=1 suppresses clean and pending acks, keeps findings emission', async () => {
    ...
    const rClean = await runHook({
      ...
      env: { PANKIT_HOOK_QUIET: '1' }, cwd, detector: detClean,
    });
    ...
    const rFindings = await runHook({
      ...
      env: { PANKIT_HOOK_QUIET: '1' }, cwd, detector: detFindings,
    });
  ...
  it('re-entrancy guard short-circuits when PANKIT_HOOK_DEPTH is set', async () => {
    const file = writeFixture('src/Card.tsx', 'noop');
    const det = fakeDetector([finding('side-tab', 1)]);
    const r = await runHook({
      stdinJson: JSON.stringify(eventFor(file)),
      env: { PANKIT_HOOK_DEPTH: '1' },
      cwd,
      detector: det,
    });
    assert.equal(r.stdout, '');
  ...
  it('PANKIT_HOOK_DISABLED kill switch', async () => {
    const file = writeFixture('src/Card.tsx', 'noop');
    const det = fakeDetector([finding('side-tab', 1)]);
    for (const v of ['1', 'true', 'yes', 'on', 'TRUE']) {
      const r = await runHook({
        stdinJson: JSON.stringify(eventFor(file)),
        env: { PANKIT_HOOK_DISABLED: v },
        cwd,
        detector: det,
      });
  ...
  it('adds a non-blocking note when DESIGN.md is newer than the sidecar', async () => {
    ...
    assert.match(r.stdout, /No deterministic design-quality issues found/);
    assert.match(r.stdout, /DESIGN\.md is newer than \.pankit\/design\.json/);
    assert.match(r.stdout, /\/pankit document/);
  });
  ...
  it('rejects files outside the project, like harness scratchpads', async () => {
    ...
    try {
      ...
      assert.ok(!fs.existsSync(path.join(cwd, '.pankit')), 'out-of-project edit must not dirty the cache');
    } finally {
      fs.rmSync(scratch, { recursive: true, force: true });
    }
  });
  ...
  it('emits suppressionNotice text on the threshold-crossing edit', async () => {
    ...
    assert.match(r.stdout, /Run \/pankit audit to revisit/);
  });
  ...
  it('honors an inline pankit-disable comment so the hook scans the file clean', async () => {
    // The hook runs the same engine as `npx pankit detect`, so an in-file
    ...
    const waived = writeFixture('src/Waived.tsx',
      'const css = "font-family: Inter"; // pankit-disable-line overused-font');
...
describe('runHook() — cache write gating (issues #344, #305)', () => {
  ...
  // `.pankit/` footprint: skipped files never dirty the cache, and a
  // dirty cache is only persisted when there are fresh findings or the
  // project already opted in (an `.pankit/` dir exists).
  let cwd;
  beforeEach(() => { cwd = mkTmp(); });
  afterEach(() => fs.rmSync(cwd, { recursive: true, force: true }));
  ...
  it('non-UI edit (.md) does not create .pankit/', async () => {
    ...
    assert.equal(r.audit.skipped, 'extension');
    assert.ok(!fs.existsSync(path.join(cwd, '.pankit')), '.pankit should not exist');
  });

  it('clean UI edit in a project with no footprint does not create .pankit/, still acks', async () => {
  ...
  it('detector-missing path does not create .pankit/', async () => {
    const file = write('src/Card.tsx', 'noop');
    const r = await runHook({
      stdinJson: JSON.stringify(eventFor(file)),
      env: {}, cwd, detector: {},
  ...
  it('fresh findings create the cache, and dedup works on the next run', async () => {
    ...
    const first = await runHook({ stdinJson: JSON.stringify(eventFor(file)), env: {}, cwd, detector: det });
    assert.match(first.stdout, /Design hook findings requiring review/);
    assert.ok(fs.existsSync(path.join(cwd, '.pankit', 'hook.cache.json')), 'cache should exist');
  ...
  it('clean UI edit in an opted-in project (existing .pankit/) still persists editCount', async () => {
    fs.mkdirSync(path.join(cwd, '.pankit'), { recursive: true });
    const file = write('src/Card.tsx', 'noop');
  ...
  it('umbrella launch keys the cache to the edited file\'s project root', async () => {
    // cwd is the umbrella: no .git / package.json / .pankit of its own.
    ...
    assert.ok(fs.existsSync(path.join(child, '.pankit', 'hook.cache.json')), 'cache should land in the child project');
    assert.ok(!fs.existsSync(path.join(cwd, '.pankit')), 'umbrella root should stay clean');
  });
});
...
describe('runHook() — session-scoped notices', () => {
  ...
  it('mentions the DESIGN.md staleness note once per session', async () => {
    ...
    const r1 = await runHook({ stdinJson: event(a), env: {}, cwd, detector: det });
    assert.match(r1.stdout, /DESIGN\.md is newer than \.pankit\/design\.json/);
...
describe('runHook() — clean-ack noise', () => {
  ...
  it('does not spend the clean ack while quiet mode is suppressing output', async () => {
    ...
    const quiet = await runHook({ stdinJson: event(file), env: { PANKIT_HOOK_QUIET: '1' }, cwd, detector: det });
...
describe('resolveCacheCwd()', () => {
  ...
  it('keeps the session cwd when it already looks like a project root', () => {
    for (const marker of ['.git', '.pankit']) {
...
describe('suppressionNotice()', () => {
  it('starts with envelope and mentions /pankit audit', () => {
    ...
    assert.match(text, /\/pankit audit/);
  });
});
...
describe('renderCleanAck() / renderPendingAck()', () => {
  it('renderCleanAck stays short and ends with the steer line', () => {
    ...
    assert.match(text, /^\[pankit@1\] Design hook scanned src\/App\.jsx\. No deterministic design-quality issues found\./);
    assert.match(text, /keep following the project design system and the pankit skill guidance/);
  ...
  it('renderPendingAck quotes up to 3 known findings and counts the rest', () => {
    ...
    assert.match(text, /^\[pankit@1\] Design hook scanned src\/SlopCard\.jsx\./);
...
describe('resolveHarness() / normalizeHookEvent()', () => {
  it('routes explicit env and Cursor conversation_id to cursor harness', () => {
    assert.equal(resolveHarness({ PANKIT_HOOK_HARNESS: 'cursor' }), 'cursor');
    assert.equal(resolveHarness({ PANKIT_HOOK_HARNESS: 'codex' }), 'codex');
  ...
  it('prefers explicit harness and Cursor detection over the Codex turn_id', () => {
    assert.equal(resolveHarness({ PANKIT_HOOK_HARNESS: 'claude' }, { turn_id: 'turn-1' }), 'claude');
    assert.equal(resolveHarness({ PANKIT_HOOK_HARNESS: 'grok' }, { turn_id: 'turn-1' }), 'grok');
  ...
  it('routes a GitHub Copilot postToolUse event (toolName/toolArgs) to the github harness', () => {
    ...
    assert.equal(resolveHarness({ PANKIT_HOOK_HARNESS: 'github' }), 'github');
  ...
  it('routes a Grok Build envelope (toolName/toolInput, no toolArgs) to grok, not github (#646)', () => {
    ...
    assert.equal(resolveHarness({ PANKIT_HOOK_HARNESS: 'grok' }), 'grok');
...
describe('expandScanTargets()', () => {
  ...
  it('does not follow imports from traversal-looking primary targets', () => {
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'pankit-hook-outside-'));
    try {
...
describe('Cursor hook scripts', () => {
  ...
  it('preToolUse denies proposed writes with detector findings before they land', () => {
    ...
    const out = execFileSync(process.execPath, [path.join('skill', 'scripts', 'hook-before-edit.mjs')], {
      ...
      env: { ...process.env, PANKIT_HOOK_LOG: logPath },
      encoding: 'utf-8',
    });
  ...
  it('preToolUse delivers the stale-sidecar note within a 500-char budget (PR #508)', () => {
    ...
    const sidecarPath = path.join(cwd, '.pankit', 'design.json');
...
`);
    ...
    const out = execFileSync(process.execPath, [path.join('skill', 'scripts', 'hook-before-edit.mjs')], {
      ...
      env: { ...process.env, PANKIT_HOOK_LOG: '' },
      encoding: 'utf-8',
    });
  ...
  it('preToolUse honors truthy PANKIT_HOOK_DISABLED values before stdin parsing', () => {
    const logPath = path.join(cwd, 'hook.ndjson');
    ...
    const out = execFileSync(process.execPath, [path.join('skill', 'scripts', 'hook-before-edit.mjs')], {
      ...
      env: {
        ...process.env,
        PANKIT_HOOK_DISABLED: 'true',
        PANKIT_HOOK_LOG: logPath,
      },
      encoding: 'utf-8',
    });
...
describe('runHook() — emission enrichment', () => {
  ...
  it('returns emission.kind fresh with findings on new hits', async () => {
    ...
    const r = await runHook({
      ...
      env: { PANKIT_HOOK_HARNESS: 'claude' },
      cwd,
...
describe('runStopHook()', () => {
  ...
  it('skips out-of-project files even when an older cache still lists them', async () => {
    ...
    const scratch = fs.mkdtempSync(path.join(os.tmpdir(), 'pankit-scratch-'));
    try {
      const outside = path.join(scratch, 'landing.html');
  ...
  it('honors kill switches and the re-entrancy guard', async () => {
    const disabled = await runStopHook({
      ...
      env: { PANKIT_HOOK_DISABLED: '1' }, cwd,
    });
