...
describe('hook manifest builders', () => {
  it('builds Claude project settings for the real detector hook', () => {
    ...
    expectCommand(handler.command, '.claude/skills/pankit/scripts/hook.mjs');
    ...
    expectCommand(stop.command, '.claude/skills/pankit/scripts/hook.mjs');
  });
  ...
  it('builds Codex project-local hooks for the real detector hook', () => {
    ...
    assert.equal(handler.timeout, 5);
    assert.equal(handler.statusMessage, 'Checking UI changes');
    expectCommand(handler.command, '.codex/skills/pankit/scripts/hook.mjs');
    ...
    const stop = manifest.hooks.Stop[0].hooks[0];
    assert.equal(stop.timeout, 30);
    expectCommand(stop.command, '.codex/skills/pankit/scripts/hook.mjs');
  });
  ...
  it('derives the Codex hook payload path from the install dir', () => {
    ...
    expectCommand(codexDir.hooks.PostToolUse[0].hooks[0].command, '.codex/skills/pankit/scripts/hook.mjs');
    expectCommand(codexDir.hooks.Stop[0].hooks[0].command, '.codex/skills/pankit/scripts/hook.mjs');

    const agentsDir = buildCodexHooksManifest('.agents');
    expectCommand(agentsDir.hooks.PostToolUse[0].hooks[0].command, '.agents/skills/pankit/scripts/hook.mjs');
    expectCommand(agentsDir.hooks.Stop[0].hooks[0].command, '.agents/skills/pankit/scripts/hook.mjs');
    ...
    expectCommand(
      ...
      '.codex/skills/pankit/scripts/hook.mjs',
    );
  });
  ...
  it('builds one Cursor pre-write blocking hook', () => {
    ...
    assert.equal(manifest.hooks.sessionStart, undefined);
    expectCommand(beforeEdit.command, '.cursor/skills/pankit/scripts/hook-before-edit.mjs');
  ...
  it('builds GitHub Copilot repo-level hooks for the real detector hook', () => {
    ...
    expectCommand(entry.bash, '.github/skills/pankit/scripts/hook.mjs');
  ...
  it('builds Grok Build project hooks for the real detector hook', () => {
    ...
    assert.equal(handler.timeout, 5);
    assert.equal(handler.statusMessage, 'Checking UI changes');
    expectCommand(handler.command, '.grok/skills/pankit/scripts/hook.mjs');
    assert.ok(!handler.command.includes('${CLAUDE_PROJECT_DIR}'));
    ...
    assert.equal(stop.timeout, 30);
    assert.equal(stop.statusMessage, 'Design deep pass');
    expectCommand(stop.command, '.grok/skills/pankit/scripts/hook.mjs');
  });
...
describe('generated hook artifacts in repo', () => {
  ...
  it('Claude project settings reference hook.mjs in .claude/skills', () => {
    ...
    expectCommand(handler.command, '.claude/skills/pankit/scripts/hook.mjs');
    assert.ok(fs.existsSync(path.join(REPO_ROOT, '.claude/skills/pankit/scripts/hook.mjs')));
    assert.ok(fs.existsSync(path.join(REPO_ROOT, '.claude/skills/pankit/scripts/hook-lib.mjs')));
    assert.ok(fs.existsSync(path.join(REPO_ROOT, '.claude/skills/pankit/scripts/detector/detect-antipatterns.mjs')));
  });
  ...
  it('Cursor project hooks reference only the pre-write runtime in .cursor/skills', () => {
    ...
    assert.equal(Object.keys(manifest.hooks).length, 1);
    expectCommand(beforeEdit.command, '.cursor/skills/pankit/scripts/hook-before-edit.mjs');
    assert.ok(fs.existsSync(path.join(REPO_ROOT, '.cursor/skills/pankit/scripts/hook-before-edit.mjs')));
    assert.equal(fs.existsSync(path.join(REPO_ROOT, '.cursor/skills/pankit/scripts/hook-after-edit.mjs')), false);
    assert.equal(fs.existsSync(path.join(REPO_ROOT, '.cursor/skills/pankit/scripts/hook-stop.mjs')), false);
    assert.ok(fs.existsSync(path.join(REPO_ROOT, '.cursor/skills/pankit/scripts/hook-lib.mjs')));
    assert.ok(fs.existsSync(path.join(REPO_ROOT, '.cursor/skills/pankit/scripts/detector/detect-antipatterns.mjs')));
  });
  ...
  it('Codex project hooks reference hook.mjs in the .codex skill payload', () => {
    ...
    assert.ok(fs.existsSync(path.join(REPO_ROOT, '.agents/skills/pankit/SKILL.md')));
    assert.ok(fs.existsSync(path.join(REPO_ROOT, '.agents/skills/pankit/scripts/hook.mjs')));
    assert.ok(fs.existsSync(path.join(REPO_ROOT, '.agents/skills/pankit/scripts/hook-lib.mjs')));
    assert.ok(fs.existsSync(path.join(REPO_ROOT, '.agents/skills/pankit/scripts/detector/detect-antipatterns.mjs')));
  });


  it('does not generate probe scripts into provider skill payloads', () => {
    for (const providerDir of ['.claude', '.cursor', '.agents', 'plugin']) {
      const probe = path.join(REPO_ROOT, providerDir, 'skills', 'pankit', 'scripts', 'hook-probe.mjs');
  ...
  it('packages the Claude design hook in the plugin via plugin-root paths', () => {
    ...
    expectCommand(handler.command, 'skills/pankit/scripts/hook.mjs');
    ...
    const stop = manifest.hooks.Stop[0].hooks[0];
    assert.equal(stop.timeout, 30);
    expectCommand(stop.command, 'skills/pankit/scripts/hook.mjs');
    ...
    assert.ok(fs.existsSync(path.join(REPO_ROOT, 'plugin/skills/pankit/scripts/hook.mjs')));
    assert.ok(fs.existsSync(path.join(REPO_ROOT, 'plugin/skills/pankit/scripts/hook-lib.mjs')));
  });

  it('keeps the marketplace hook repair matcher aligned with Claude Code', () => {
    const hookAdmin = fs.readFileSync(
      path.join(REPO_ROOT, 'plugin/skills/pankit/scripts/hook-admin.mjs'),
      'utf8',
    );
  ...
  it('generated hook runtime can import the bundled detector', async () => {
    for (const scriptDir of [
      '.claude/skills/pankit/scripts',
      '.cursor/skills/pankit/scripts',
      '.agents/skills/pankit/scripts',
      'plugin/skills/pankit/scripts',
    ]) {
