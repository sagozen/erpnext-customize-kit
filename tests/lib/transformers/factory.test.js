...
describe('createTransformer factory', () => {
  ...
  test('should render the explicit script provider marker without rewriting executable code', () => {
    ...
    const skills = [{
      ...
      scripts: [{
        ...
        content: [
          ...
          'const command = `${PANKIT_COMMAND_PREFIX}pankit polish`;',
          'const hint = "Run /pankit audit";',
          'const hook = ".github/hooks/pankit.json";',
    ...
    const script = fs.readFileSync(
      path.join(TEST_DIR, 'codex/.test/skills/pankit/scripts/example.mjs'),
      'utf-8',
    );
    expect(script).toContain('PANKIT_COMMAND_PREFIX = "$"');
    expect(script).toContain('PANKIT_PROVIDER_ID = "codex"');
    expect(script).toContain('`${PANKIT_COMMAND_PREFIX}pankit polish`');
    expect(script).toContain('"Run /pankit audit"');
    expect(script).toContain('".github/hooks/pankit.json"');
    expect(script).toContain('"/src/lib/pankit/__runtime.js"');
    expect(script).toContain('/pankit\\b/gi');
  });
