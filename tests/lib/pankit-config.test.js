...
} from '../../cli/lib/pankit-config.mjs';

describe('cli/lib/pankit-config', () => {
  let root;
  ...
  test('setHookConsent preserves unrelated keys in config.local.json', () => {
    mkdirSync(join(root, '.pankit'), { recursive: true });
  ...
  test('writing consent gitignores config.local.json via .git/info/exclude', () => {
    ...
    expect(exclude).toContain('.pankit/config.local.json');
    ...
    expect((again.match(/pankit-config-ignore-start/g) || []).length).toBe(1);
  ...
  test('writeDetectionConfig local ignores do not create an implicit design-system override', () => {
    ...
    expect(readFileSync(join(root, '.git', 'info', 'exclude'), 'utf-8')).toContain('.pankit/config.local.json');
  });
