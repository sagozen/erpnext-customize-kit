...
Audit the code.`;
  ...
  test('should read skill with reference files', () => {
    const skillContent = `---
name: pankit
description: PanKit design skill
---

PanKit design instructions.`;
  ...
  test('should fall back to "pankit" when frontmatter has no name', () => {
    const skillDir = path.join(testRootDir, 'skill');
    ensureDir(skillDir);
    ...
    expect(skills).toHaveLength(1);
    expect(skills[0].name).toBe('pankit');
  });
...
describe('replacePlaceholders', () => {
  ...
  test('should exclude pankit from {{available_commands}}', () => {
    const result = replacePlaceholders('Commands: {{available_commands}}', 'claude-code', ['audit', 'pankit', 'polish']);
    expect(result).toBe('Commands: /audit, /polish');
  });


  test('lists /pankit sub-commands for {{available_commands}} when no command names are passed', () => {
    // Single-skill architecture: with no command names, the list falls back
    // to the PANKIT_SUB_COMMANDS sub-commands rendered as `/pankit <sub>`.
    const result = replacePlaceholders('Commands: {{available_commands}}', 'claude-code', []);
    expect(result.startsWith('Commands: /pankit ')).toBe(true);
    expect(result).toContain('/pankit spec');
    expect(result).toContain('/pankit review');
  });
  ...
  test('should replace Codex command invocations without rewriting paths', () => {
    const source = [
      'Run /pankit audit.',
      'Use `/pankit polish` next.',
      '.github/hooks/pankit.json',
      '.codex/skills/pankit/scripts/context.mjs',
      'https://example.com/pankit',
    ].join('\n');

    const result = replacePlaceholders(source, 'codex', [], ['pankit']);

    expect(result).toContain('Run $pankit audit.');
    expect(result).toContain('Use `$pankit polish` next.');
    expect(result).toContain('.github/hooks/pankit.json');
    expect(result).toContain('.codex/skills/pankit/scripts/context.mjs');
    expect(result).toContain('https://example.com/pankit');
  });
});
...
describe('replaceScriptProviderMarker', () => {
  test('renders only the explicit provider declarations', () => {
    const source = [
      ...
      'const regex = /pankit\\b/gi;',
      "const runtime = '/src/lib/pankit/__runtime.js';",
      "const text = 'Run /pankit audit';",
    ].join('\n');

    const result = replaceScriptProviderMarker(source, 'codex', 'agents');

    expect(result).toContain('export const PANKIT_COMMAND_PREFIX = "$";');
    expect(result).toContain('export const PANKIT_PROVIDER_ID = "agents";');
    expect(result).toContain('const regex = /pankit\\b/gi;');
    expect(result).toContain("const runtime = '/src/lib/pankit/__runtime.js';");
    expect(result).toContain("const text = 'Run /pankit audit';");
  });
});
