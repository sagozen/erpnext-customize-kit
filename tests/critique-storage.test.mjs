...
describe('slugFromTarget', () => {
  ...
  it('normalizes URL casing and trailing slash', () => {
    assert.equal(
      slugFromTarget('https://PanKit.Style/docs/audit/', { cwd }),
      'pankit-style-docs-audit',
    );
  });
...
describe('writeSnapshot + readLatestSnapshot', () => {
  ...
  it('picks the newest snapshot across target slugs', () => {
    ...
    writeFileSync(join(cwd, '.pankit', 'critique', 'ignore.md'), '# Critique ignores\n');
    writeFileSync(join(cwd, '.pankit', 'critique', '9999-not-a-snapshot.md'), '# Draft\n');
  ...
  it('caller-supplied meta cannot override computed timestamp or slug', () => {
    // Defends against a corrupt PANKIT_CRITIQUE_META blob (parsed from
