...
Use this packet for the initial public submission of PanKit. The release build creates the upload at `dist/openai-plugin.zip`.

## Submission type

Choose **Skills only**. PanKit has no MCP server. The package includes one skill, its referenced scripts and files, a branded icon, and a `PostToolUse` lifecycle hook.

The hook runs locally after supported file edits, checks changed UI files for known anti-patterns, and returns findings to the active coding-agent session. It does not call an PanKit service. Codex asks each user to review and trust the hook before it can run.
...
- Long description: Create, critique, and refine frontend interfaces with your coding agent. PanKit provides 23 focused design commands, live browser iteration for exploring visual directions, and automatic checks that flag common design anti-patterns as you work.
- Category: Creativity
- Website: https://pankit.style
- Support: https://github.com/pbakaus/pankit/issues
- Privacy policy: https://pankit.style/privacy
- Terms: https://github.com/pbakaus/pankit/blob/main/LICENSE
- Logo: `dist/openai/pankit/assets/icon.png`
...
3. start pankit live so I can explore bolder or more delightful variants.
...
- Expected behavior: Invoke PanKit, load the audit and brand references, inspect the existing HTML, CSS, PRODUCT.md, and DESIGN.md, then run the bundled detector where applicable.
...
- Fixture data: The public `demos/landing-demo/` folder in `pbakaus/pankit`.

### 2. Final polish pass

- User prompt: `polish demos/landing-demo/index.html and fix the rough edges without changing the brand.`
- Expected behavior: Invoke PanKit, load the polish and brand references, preserve the existing design tokens, inspect the page in a browser when available, and make scoped edits.
...
- Expected behavior: Do not invoke PanKit. Explain that the plugin is scoped to frontend interface work and continue with suitable general coding help if available.
...
- Expected behavior: Do not use PanKit as the asset generator. Route to image generation when available, or explain that the plugin handles interface design rather than standalone raster artwork.
...
Initial submission of PanKit, a frontend design skill for ChatGPT and Codex. It packages one skill with 23 design workflows, referenced scripts and guidance, a local anti-pattern detector, and an optional `PostToolUse` hook. No MCP server, authentication, demo account, or private network is required. The only network request made by the skill is the documented optional daily version check to `https://pankit.style/api/version`.
...
- Upload `dist/openai-plugin.zip` and confirm the scanner recognizes `skills/pankit/SKILL.md` and `hooks/hooks.json`.
