...
> **This repository currently ships four providers** (Claude Code, Cursor, Codex,
> and Codex repo skills). The rest of this file is the landscape reference kept
> so a provider can be added back to `providers.js` without re-researching it.
>
...
> PanKit's *emission targets*, not the support landscape (see its note).
...
- Codex CLI uses a separate `agents/openai.yaml` sidecar for skill metadata (icons, branding, MCP tools, invocation control). Codex also auto-discovers subagents bundled inside an installed skill's `agents/` folder (TOML), which is how PanKit ships its asset-producer. Standalone custom agents can still live under `.codex/agents/` or `~/.codex/agents/`, but PanKit no longer installs anything there.
...
## Hook surface used by PanKit

| Harness | Edit hook | Startup hook | Manifest location | Notes |
|---------|:---------:|:------------:|-------------------|-------|
| Claude Code | Yes (`PostToolUse`) | No | `.claude/settings.json` | Project-local settings entry installed by `npx pankit skills install/update`. Runs `.claude/skills/pankit/scripts/hook.mjs`. |
| Codex CLI | Yes (`PostToolUse`) | No | `.codex/hooks.json` | Project-local manifest installed with the `.agents/skills/pankit` payload. Runs `.agents/skills/pankit/scripts/hook.mjs` from the git root. Requires normal `/hooks` trust approval. |
| Cursor | Yes (`preToolUse`) | No | `.cursor/hooks.json` | Project-level manifest installed with `.cursor/skills/pankit`. Runs `hook-before-edit.mjs` to block bad proposed writes before they land. Reloads on save; restart Cursor if hooks do not pick up. |
| Grok Build | Yes (`PostToolUse`) | No | `.grok/hooks/pankit.json` | Project-local manifest installed with `.grok/skills/pankit`. Claude-compatible matchers (`Edit\|Write\|MultiEdit`) alias to Grok `search_replace`. PostToolUse runs the scan and warms the session cache; Grok ignores that stdout. Stop `additionalContext` is the user-visible pass. Ignore Grok's observe-only Stop with `reason: "shutdown"`. Requires `/hooks-trust` or `--trust`. Plugin installs use `plugin/hooks/hooks.json` with `${CLAUDE_PLUGIN_ROOT}` (aliased to `GROK_PLUGIN_ROOT`). |
...
## Native Subagent Directory Structure (PanKit emission targets)

> **Scope:** this table is **where PanKit emits native subagent files**, not a
...
> GitHub Copilot, and Google Antigravity ship them too. PanKit only writes
