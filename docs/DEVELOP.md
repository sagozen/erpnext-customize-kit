...
bun run test:cli-remote-e2e   # Opt-in: remote CLI install smoke test
```

`scripts/test-suites.mjs` owns the suite definitions, including which file changes select which suite in CI. Adding a test file without registering it there means CI can never select it by editing it; `tests/test-suites.test.mjs` pins that invariant.

`tests/skill-reference.test.mjs` is the structural guard for the router: it checks the command table against the reference files and `command-metadata.json` in both directions. Run it after any command change.
...
6. **Test across providers**: Verify the compiled output in `dist/` for each target, not just the source.
...
pankit/
  ...
  cli/                             # Standalone `pankit` CLI (npm package)
  site/                            # pankit.style (Astro)
