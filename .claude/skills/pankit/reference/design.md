...
5. Define links, child ownership, revision snapshots, lifecycle, roles, units/precision, validation and retry identity. For touched document boundaries, name the producer, consumer and authoritative owner of each shared value; specify conversion and historical-version behavior once. Curtain Measurement and similar names are proposed custom DocTypes, not built-ins.
