...
import {
  ...
  PANKIT_COMMAND_PREFIX,
  readLiveServerInfo,
  removeLiveServerInfo,
  resolveDesignSidecarPath,
  writeLiveServerInfo,
} from './lib/pankit-paths.mjs';
...
// that outlives an `pankit document` run (or a context file created after
...
const DEBUG_MANUAL_EDIT_EVENTS = /^(1|true|yes)$/i.test(process.env.PANKIT_LIVE_DEBUG_EVENTS || '');
...
function loadBrowserScripts() {
  ...
  const detectPaths = [
    ...
    path.join(__dirname, '..', '..', '..', '..', 'cli', 'engine', 'detect-antipatterns-browser.js'),
    path.join(process.cwd(), 'node_modules', 'pankit', 'cli', 'engine', 'detect-antipatterns-browser.js'),
  ];
  let detectScript = '';
  ...
  return (req, res) => {
    ...
    if (p === '/live.js') {
      ...
      const body = assembleLiveBrowserScript({
        ...
        commandPrefix: PANKIT_COMMAND_PREFIX,
        appRoot: process.cwd(),
        parts,
      });
      res.writeHead(200, {
    ...
    //   /design-system.json    returns both parsed DESIGN.md and .pankit/design.json
    ...
    //                          - sidecar: .pankit/design.json contents when present.
    ...
    if (p === '/design-system.json' || p === '/design-system/raw') {
      ...
      if (jsonStat) {
        try {
          ...
          response.sidecarError = 'Failed to parse .pankit/design.json: ' + err.message;
        }
      }
...
function sessionFileMetadataFromPollReply(file) {
  ...
  if (!metadataFile.includes('.pankit/live/previews/')
      && !metadataFile.includes('node_modules/.pankit-live/')
      && !metadataFile.includes('src/lib/pankit/')
      && !metadataFile.includes('/.pankit-live/')) return base;
...
function cleanupSvelteComponentSessionsBeforeExit() {
  try {
    ...
    console.warn('[pankit] Svelte component session cleanup failed:', err.message);
  }
}
...
function sweepOrphanSvelteComponentSessionsOnStartup() {
  try {
    ...
    if (result.removed.length > 0 || result.removedRoot) {
      console.log('[pankit] swept orphaned Svelte component sessions:', JSON.stringify(result));
    }
  } catch (err) {
    console.warn('[pankit] Svelte component session sweep failed:', err.message);
  }
}
...
function sweepStaleAcceptReceiptsOnStartup() {
  try {
    ...
    if (removed > 0) console.log(`[pankit] removed ${removed} accept receipt(s) older than 14 days`);
  } catch (err) {
    console.warn('[pankit] accept receipt retention sweep failed:', err.message);
  }
}
...
function applyLegacyDeferredAcceptsOnStartup() {
  try {
    ...
    if (result.applied > 0 || result.failed > 0) {
      console.log('[pankit] applied legacy deferred Svelte component accepts:', JSON.stringify(result));
    }
  } catch (err) {
    console.warn('[pankit] legacy deferred Svelte component accept apply failed:', err.message);
  }
}
...
httpServer.listen(state.port, '127.0.0.1', () => {
  ...
  console.log(`\nPanKit live server running on ${url}`);
