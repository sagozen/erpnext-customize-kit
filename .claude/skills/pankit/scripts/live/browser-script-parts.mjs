...
export function assembleLiveBrowserScript({
  ...
  const prelude =
    `window.__PANKIT_TOKEN__ = '${token}';\n` +
    `window.__PANKIT_PORT__ = ${port};\n` +
    ...
    `window.__PANKIT_APP_ROOT__ = ${JSON.stringify(appRoot)};\n` +
    `window.__PANKIT_COMMAND_PREFIX__ = ${JSON.stringify(commandPrefix)};\n` +
    ...
    `window.__PANKIT_VOCAB__ = ${JSON.stringify(vocabulary)};\n` +
    ...
    // repo's tests, the pankit-site Live UI lab) import the module directly,
    // which is what keeps the two from drifting.
    `window.__PANKIT_LIVE_UI_SURFACES__ = ${JSON.stringify(uiSurfaces)};\n` +
    `window.__PANKIT_LIVE_MOUNT_CONTRACT__ = ${JSON.stringify(mountContract)};\n`;

  const body = parts.map((part) => {
    const file = part.file || path.basename(part.path || '');
    return `// --- pankit live script part: ${part.name} (${file}) ---\n${part.source}`;
  }).join('\n');
