...
export const NUXT_PLUGIN_MARKER = 'pankit-live-nuxt-plugin';
export const NUXT_PLUGIN_NAME = 'pankit-live.client.ts';
...
const liveSelector = 'script[data-pankit-live-nuxt]';
...
export function applyNuxtLiveAdapter({ cwd = process.cwd(), port, token, project = detectNuxtProject(cwd) }) {
  ...
  if (existing !== null && !existing.includes(NUXT_PLUGIN_MARKER)) {
    return {
      ...
      hint: `${project.pluginFile} already exists and is not managed by PanKit Live`,
    };
  }
...
export function removeNuxtLiveAdapter({ cwd = process.cwd(), project = detectNuxtProject(cwd) }) {
  ...
  if (!content.includes(NUXT_PLUGIN_MARKER)) {
    return {
      ...
      removed: false,
      error: 'nuxt_plugin_conflict',
      hint: `${project.pluginFile} is not managed by PanKit Live`,
    };
  }
