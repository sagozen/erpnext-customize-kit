 ...
 *
 * On first live run, the agent generates `.pankit/live/config.json`
...
import { resolveLiveConfigPath } from './lib/pankit-paths.mjs';
import {
...
const IGNORE_MARKER_OPEN = '# pankit-live-ignore-start';
const IGNORE_MARKER_CLOSE = '# pankit-live-ignore-end';

export const LIVE_IGNORE_PATTERNS = Object.freeze([
  '.pankit/hook.cache.json',
  '.pankit/hook.pending.json',
  '.pankit/config.local.json',
  '.pankit/live/server.json',
  '.pankit/live/roots.json',
  '.pankit/live/app-root.json',
  '.pankit/live/inject-journal.json',
  '.pankit/live/sessions/',
  '.pankit/live/previews/',
  '.pankit/live/annotations/',
  '.pankit/live/artifacts/',
  '.pankit/live/accept-receipts/',
  '.pankit/live/locks/',
  '.pankit/live/cache/',
  '.pankit/live/manual-edit-apply-transaction.json',
  '.pankit/live/manual-edit-events.jsonl',
  '.pankit/live/manual-edit-evidence/',
  '.pankit/live/pending-manual-edits.json',
  '.pankit/live/deferred-svelte-component-accepts.json',
  '.pankit-live.json',
  '.pankit-live/',
  'app/.pankit-live/',
  'src/.pankit-live/',
  'node_modules/.pankit-live/',
  'src/lib/pankit/PanKitLiveRoot.svelte',
  'src/lib/pankit/__runtime.js',
  'src/lib/pankit/[0-9a-f]*/',
  'plugins/pankit-live.client.ts',
  'app/plugins/pankit-live.client.ts',
  'src/plugins/pankit-live.client.ts',
]);
...
Reads configuration from .pankit/live/config.json.

Modes:
  --port PORT   Insert script tag pointing at http://localhost:PORT/live.js
  --remove      Remove the script tag (if present)
  --check       Print whether .pankit/live/config.json exists and its content
...
Output (JSON):
  ...
  if (!token) {
    try {
      const info = JSON.parse(fs.readFileSync(path.join(cwd, '.pankit', 'live', 'server.json'), 'utf-8'));
  ...
  try {
    const rootsManifest = JSON.parse(fs.readFileSync(path.join(cwd, '.pankit', 'live', 'roots.json'), 'utf-8'));
