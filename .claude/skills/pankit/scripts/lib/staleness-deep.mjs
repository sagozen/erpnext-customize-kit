 ...
 *
 * The boot tier answers "did an older PanKit write this". This tier also
...
// Retired live-mode state locations. pankit-paths still reads these as
// fallbacks; reporting them is what eventually lets the fallbacks go.
const LEGACY_LIVE_PATHS = ['.pankit-live.json', '.pankit-live'];
...
const SEED_DESIGN_MARKERS = ['/', '$'].map((prefix) =>
  '<!-- SEED: established with the user before implementation; '
    + `re-run ${prefix}pankit document once there's code to capture the actual tokens and components. -->`
);
...
export function checkDetectorIgnores({ projectRoot, knownRuleIds = null }) {
  ...
  for (const name of ['config.json', 'config.local.json']) {
    const filePath = path.join(projectRoot, '.pankit', name);
...
const HOOK_MARKER = /skills\/pankit\/scripts\/hook(?:-before-edit)?\.mjs/;
...
function hookScriptTokenFrom(command) {
  ...
  const quoted = str.match(/"([^"]*skills\/pankit\/scripts\/hook(?:-before-edit)?\.mjs)"/);
  ...
  const singleQuoted = str.match(/'([^']*skills\/pankit\/scripts\/hook(?:-before-edit)?\.mjs)'/);
  if (singleQuoted) return singleQuoted[1];
  const bare = str.match(/([^\s"'|&;()]*skills\/pankit\/scripts\/hook(?:-before-edit)?\.mjs)/);
    ...
    for (const rel of manifests) {
      ...
      if (broken.length) {
        findings.push(finding({
          ...
          fix: `Reinstall with \`pankit hooks on\`, which rewrites the manifest against the skill's current location.`,
        }));
      }
    }
  }
      ...
      for (const name of ['config.json', 'config.local.json']) {
        ...
        if (raw?.hook && raw.hook.enabled === false) {
          findings.push(finding({
            ...
            artifact: 'config.json',
            filePath: toRelative(path.join(root, '.pankit', name), projectRoot || root),
            severity: 'mention',
            ...
            fix: 'Ask which was intended: `pankit hooks on` to enable, or `pankit hooks off` to uninstall '
...
export function checkLegacyLiveState({ projectRoot }) {
  ...
  return [finding({
    ...
    summary: `Live-mode state sits in retired location(s): ${present.map((rel) => `\`${rel}\``).join(', ')}. `
      + 'Current live mode writes under `.pankit/live/`.',
