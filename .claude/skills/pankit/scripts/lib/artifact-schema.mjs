/**
 * Schema versions for the artifacts PanKit writes, plus the readers and
...
/** `.pankit/design.json`, as documented in reference/document.md Step 4b. */
...
/**
 * Headings PanKit used to read and no longer does, with the reason. The
...
const PRODUCT_STAMP_RE = /^[ \t]*<!--[ \t]*pankit:product-schema[ \t]+(\d+)[ \t]*-->[ \t]*$/im;
...
export function productStampLine(version = PRODUCT_SCHEMA_VERSION) {
  return `<!-- pankit:product-schema ${version} -->`;
}
