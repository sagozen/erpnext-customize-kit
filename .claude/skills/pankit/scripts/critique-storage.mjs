 ...
 *   .pankit/critique/<timestamp>__<slug>.md
...
import { getCritiqueDir } from './lib/pankit-paths.mjs';
...
export function writeSnapshot({ slug, meta, body, cwd = process.cwd(), now = new Date() }) {
  ...
  // PANKIT_CRITIQUE_META env var) could clobber them, leaving the
...
function main(argv) {
  ...
  switch (cmd) {
    ...
    case 'write': {
      ...
      let meta = {};
      const metaArg = process.env.PANKIT_CRITIQUE_META;
      if (metaArg) {
