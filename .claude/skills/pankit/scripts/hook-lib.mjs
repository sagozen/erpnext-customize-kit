/**
 * Shared library for the PanKit design hook.
 *
...
export const ENVELOPE_PREFIX = '[pankit@1]';
...
// behavior with `.pankit/config.json` → `hook: { "perEditRules": "all" }`.
...
// A project opts back in with `.pankit/config.json`:
...
export const HOOK_LOCAL_IGNORE_PATTERNS = Object.freeze([
  ...
  '.pankit/config.local.json',
]);

const HOOK_IGNORE_MARKER_OPEN = '# pankit-hook-ignore-start';
const HOOK_IGNORE_MARKER_CLOSE = '# pankit-hook-ignore-end';
...
export function getConfigPath(cwd) {
  return path.join(cwd, '.pankit', 'config.json');
}

export function getLocalConfigPath(cwd) {
  return path.join(cwd, '.pankit', 'config.local.json');
}

export function getCachePath(cwd) {
  return path.join(cwd, '.pankit', 'hook.cache.json');
}

export function getPendingPath(cwd) {
  return path.join(cwd, '.pankit', 'hook.pending.json');
}
...
function looksLikeProjectRoot(dir) {
  return ['.git', 'package.json', '.pankit'].some((marker) => {
    try { return fs.existsSync(path.join(dir, marker)); } catch { return false; }
  });
}

// Where `.pankit/` (cache + config) lives for this event. Normally the
...
// .pankit), key to the edited file's nearest project root instead, so a
...
export function suppressionNotice(filePath) {
  return `${ENVELOPE_PREFIX} Suppressing further design hints on ${filePath}. More than ${EDIT_COUNT_THRESHOLD} edits in this session reached. Run ${PANKIT_COMMAND} audit to revisit.`;
}
...
export function renderTemplate(findings, filePath, config, opts = {}) {
  ...
  const more = remaining > 0
    ? `... and ${remaining} more (see ${PANKIT_COMMAND} audit).`
    : null;
...
function renderGroupedTemplate(groups, config, opts = {}) {
  ...
  for (const group of realGroups) {
    ...
    if (hidden > 0) {
      lines.push(`- ... ${hidden} more in ${display} (see ${PANKIT_COMMAND} audit).`);
    }
  }
...
function clampGroupedToBudget(header, lines, footer, maxChars) {
  const assemble = (linesArr, omitted, footerText) => [
    header,
    ...linesArr,
    ...(omitted ? [`... and more (see ${PANKIT_COMMAND} audit).`] : []),
    '',
    footerText,
  ].join('\n');
...
function clampToBudget(header, lines, more, footer, maxChars) {
  ...
  for (const footerText of footerFallbacks(footer)) {
    ...
    while (assembled.length > maxChars && working.length > 1) {
      working.pop();
      moreText = `... and more (see ${PANKIT_COMMAND} audit).`;
...
export function resolveHarness(env = {}, event = null) {
  const explicit = env?.PANKIT_HOOK_HARNESS;
  ...
  // contract without rewriting the hook command to set PANKIT_HOOK_HARNESS.
...
export function writeAuditLog(env, entry, cwd = process.cwd()) {
  ...
  let target = env?.PANKIT_HOOK_LOG;
...
// silence-on-clean can set `PANKIT_HOOK_QUIET=1` — runHook checks that
...
const STEER_LINE = 'That does not mean the design is good: keep following the project design system and the pankit skill guidance.';
...
const DESIGN_STALE_NOTE = `${ENVELOPE_PREFIX} DESIGN.md is newer than .pankit/design.json. Run ${PANKIT_COMMAND} document to refresh the design-system sidecar.`;
...
function directiveFooter(opts = {}) {
  ...
  return [
    ...
    `Self-serve ends at ignore-value: \`ignore-file\` and \`ignore-rule\` need the user's explicit approval, and never add an ignore to push a blocked write through. Full suppression ladder: ${PANKIT_COMMAND} hooks.`,
  ].join('\n');
}
...
export async function runHook({ stdinJson, env = {}, cwd = process.cwd(), now = Date.now, detector } = {}) {
  ...
  try {
    // Re-entrancy guard.
    if (depthIsSet(env.PANKIT_HOOK_DEPTH) || depthIsSet(env.CLAUDE_HOOK_DEPTH)) {
      return result({ reentrant: true, durationMs: 0 });
    }

    if (truthy(env.PANKIT_HOOK_DISABLED)) {
    ...
    let skippedBytes = 0;
    const quietMode = truthy(env.PANKIT_HOOK_QUIET) || config.quiet === true;
    ...
    if (freshGroups.length > 0) {
      ...
      // `.pankit/`: dedup, suppression, and the notice flags need it.
    ...
    // `.pankit/` dir marks a project that opted in. A non-UI edit, or a
    // clean UI edit in a project with no PanKit footprint, must be a
    // no-op on disk (issues #344, #305).
    if (deferredTotal > 0 || (cacheDirty && fs.existsSync(path.join(projectCwd, '.pankit')))) {
      persistCache(projectCwd, cache);
    }
