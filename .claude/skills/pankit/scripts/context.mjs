 ...
 *      `.pankit/config.json` declares `projectRoots`)
 *   4. $PANKIT_CONTEXT_DIR (absolute or cwd-relative) — power-user
...
import { PANKIT_COMMAND, PANKIT_PROVIDER_ID } from './lib/provider.mjs';
...
// can offer `npx pankit update`. Everything here is best-effort and
// silent on failure: a network problem, sandbox, or missing cache must never
// block context output or print an error.

const UPDATE_HOST = (process.env.PANKIT_UPDATE_HOST || 'https://pankit.style').replace(/\/$/, '');
const UPDATE_CACHE_PATH =
  process.env.PANKIT_UPDATE_CACHE || path.join(os.homedir(), '.pankit', 'update-check.json');
...
function resolveEnvContextDir(cwd) {
  const envDir = process.env.PANKIT_CONTEXT_DIR;
...
// outside both the project and repo roots (PANKIT_CONTEXT_DIR override).
...
// itself. PanKit `projectRoots` patterns govern every path they match:
...
// deeper folder would silently resolve there. Paths the PanKit group does
...
function isSelectableCandidate(repoRoot, rel, patternGroups) {
  ...
  const [pankitPatterns, packagePatterns] = patternGroups;
  if (isExcludedByWorkspacePattern(relSegments, pankitPatterns)) return false;
  for (const pattern of pankitPatterns) {
...
// `projectRoots` globs in .pankit config, then package-manager workspace
// declarations. A path matched by any PanKit pattern — positive or
// negated — is governed by the PanKit group alone; package-manager
// patterns only apply to paths the PanKit group does not match. Within a
// group, negations win over positives.
function readProjectPatternGroups(repoRoot) {
  return [
    readPanKitProjectRoots(repoRoot),
    [
...
function readPanKitProjectRoots(repoRoot) {
  const patterns = [];
  for (const name of ['config.json', 'config.local.json']) {
    const cfg = readJson(path.join(repoRoot, '.pankit', name));
...
function buildUpdateDirective(localVersion, latestVersion) {
  return (
    `UPDATE_AVAILABLE: A newer PanKit skill is available ` +
    `(installed v${localVersion}, latest v${latestVersion}). ` +
    `Mention it once, in this form: "A newer PanKit (v${latestVersion}) is available. ` +
    `Update now? It runs \`npx pankit update\`." ` +
    `Do not run \`npx pankit update\` in this turn, whatever the user answers: it rewrites the skill files ` +
 ...
 * the agent never nags. Opt out entirely with PANKIT_NO_UPDATE_CHECK=1.
 */
...
function updateCheckDisabledByConfig(cwd = process.cwd()) {
  ...
  for (const name of ['config.json', 'config.local.json']) {
    try {
      const raw = JSON.parse(fs.readFileSync(path.join(cwd, '.pankit', name), 'utf-8'));
...
async function computeUpdateDirective(now = Date.now()) {
  try {
    if (process.env.PANKIT_NO_UPDATE_CHECK) return null;
  ...
  if (!ctx.hasProduct) {
    ...
    const parts = ctx.hasVisualImplementation
      ? [
          ...
          `offer \`${PANKIT_COMMAND} init\` as a follow-up.`,
      ...
      : [
          ...
          `context and offer \`${PANKIT_COMMAND} init\` as a suggestion (do not block).`,
...
const HOOK_MANIFESTS_BY_PROVIDER = Object.freeze({
  ...
  github: ['.github/hooks/pankit.json'],
  grok: ['.grok/hooks/pankit.json'],
});
...
function valueHasHookMarker(value) {
  if (typeof value === 'string') {
    return value.includes('skills/pankit/scripts/hook.mjs')
      || value.includes('skills/pankit/scripts/hook-before-edit.mjs');
  }
...
function hookEnabledAt(root) {
  if (truthyEnv(process.env.PANKIT_HOOK_DISABLED)) return false;
  let enabled = true;
  for (const name of ['.pankit/config.json', '.pankit/config.local.json']) {
...
function automaticHookMode(ctx) {
  ...
  const manifests = HOOK_MANIFESTS_BY_PROVIDER[PANKIT_PROVIDER_ID] || [];
  ...
  for (const root of roots) {
    for (const rel of manifests) {
      ...
      if (raw?.hooks && valueHasHookMarker(raw.hooks)) {
        return STOP_REVIEW_PROVIDERS.has(PANKIT_PROVIDER_ID) ? 'stop' : 'per-edit';
      }
    }
  }
  return 'none';
...
function readBuildPathAt(root) {
  ...
  let source = null;
  for (const name of ['config.json', 'config.local.json']) {
    const raw = readJson(path.join(root, '.pankit', name));
...
function appendAutonomyCounterDirective(parts) {
  parts.push([
    ...
    "PanKit's interview and decision steps stay live: probe once with the structured question tool or the decision page.",
...
function appendDetectorFallback(parts, ctx) {
  ...
  const scriptsPath = path.dirname(fileURLToPath(import.meta.url));
  parts.push([
    'MANUAL_DETECTOR_REQUIRED: No automatic PanKit design hook is active this session.',
...
// Tier 1 staleness: schema drift in PanKit's own project files, measured
...
function projectRootsDiagnostic(ctx, options) {
  ...
  const patterns = readPanKitProjectRoots(ctx.repoRoot);
...
function buildTargetSelectionDirective(selection) {
  return (
    ...
    'Ask the user which app PanKit should use, then rerun PanKit helper commands from that child app cwd using this same scripts directory. ' +
