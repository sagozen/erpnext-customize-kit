...
async function captureVisualContrastCandidate(page, candidate, viewport) {
  ...
  const token = `pankit-contrast-${Date.now()}-${Math.random().toString(36).slice(2)}`;
  const applied = await page.evaluate(({ selector, token, backgroundClipText }) => {
    ...
    if (!el) return false;
    let style = document.getElementById('pankit-visual-contrast-hide-style');
    if (!style) {
      style = document.createElement('style');
      style.id = 'pankit-visual-contrast-hide-style';
      style.textContent = [
        '[data-pankit-visual-contrast-target] {',
        '  color: transparent !important;',
        '  -webkit-text-fill-color: transparent !important;',
        '  text-shadow: none !important;',
        '}',
        '[data-pankit-visual-contrast-target][data-pankit-bgclip-text="true"] {',
        '  background-image: none !important;',
        '}',
      ].join('\n');
      document.head.appendChild(style);
    }
    el.setAttribute('data-pankit-visual-contrast-target', token);
    if (backgroundClipText) el.setAttribute('data-pankit-bgclip-text', 'true');
    return true;
  }, {
  ...
  try {
    ...
    await page.evaluate(({ selector }) => {
      try {
        ...
        if (el) {
          el.removeAttribute('data-pankit-visual-contrast-target');
          el.removeAttribute('data-pankit-bgclip-text');
        }
      } catch {
