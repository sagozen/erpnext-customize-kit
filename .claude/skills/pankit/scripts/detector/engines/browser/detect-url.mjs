...
async function measureContentHiddenAfterReveal(page) {
  ...
  return page.evaluate(() => {
    if (typeof window.pankitMeasureHiddenText !== 'function') return null;
    return window.pankitMeasureHiddenText();
  });
}
...
async function runVisualContrastFallback(page, serializedGroups, options, profile, target) {
  ...
  if (options?.visualContrastBrowser !== false) {
    const browserFindings = await profileFindingsAsync(profile, {
      ...
      browserAnalyses = await page.evaluate(async ({ maxCandidates, scrollOffscreen }) => {
        if (typeof window.pankitAnalyzeVisualContrast !== 'function') return [];
        return window.pankitAnalyzeVisualContrast({ maxCandidates, scrollOffscreen });
  ...
  if (candidates.length === 0) {
    candidates = await profileStepAsync(profile, {
      ...
      if (typeof window.pankitCollectVisualContrastCandidates !== 'function') return [];
      return window.pankitCollectVisualContrastCandidates({ maxCandidates });
    }, { maxCandidates }));
  }
  ...
  try {
    ...
    results = await profileFindingsAsync(profile, {
      ...
      serializedGroups = await page.evaluate(() => {
        if (!window.pankitDetect) return [];
        return window.pankitDetect({ decorate: false, serialize: true });
      });
