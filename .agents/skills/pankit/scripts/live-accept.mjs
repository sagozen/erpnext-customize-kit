...
import { getLiveDir, safeSessionId } from './lib/pankit-paths.mjs';
 ...
 * <div data-pankit-carbonize> so the slot keeps a single root node.
 */
function buildCarbonizeReplacement({
  ...
  const pushCarbonizeBody = (bodyIndent) => {
    ...
    lines.push(bodyIndent + commentSyntax.open + ' pankit-carbonize-start ' + id + ' ' + commentSyntax.close);
    lines.push(bodyIndent + '<style data-pankit-css="' + id + '">' + (isJsx ? '{`' : ''));
    ...
    if (paramValues && Object.keys(paramValues).length > 0) {
      lines.push(
        bodyIndent + commentSyntax.open + ' pankit-param-values ' + id + ': ' + JSON.stringify(paramValues) + ' ' + commentSyntax.close,
      );
    }
    lines.push(bodyIndent + commentSyntax.open + ' pankit-carbonize-end ' + id + ' ' + commentSyntax.close);
    lines.push(bodyIndent + '<div data-pankit-variant="' + variantNum + '" ' + variantStyleAttr + '>');
  ...
  if (isJsx) {
    ...
    lines.push(indent + '<div data-pankit-carbonize="' + id + '" ' + wrapperStyle + '>');
...
function buildAcceptedWrappedSource(id, variantNum, lines, targetFile, paramValues) {
  ...
  const hasHelperAttrs = variantText.includes('data-pankit-variant');
...
function readSourceShadowPreviewMeta(content, id) {
  ...
  const wrapperRe = new RegExp('<[^>]+data-pankit-variants=(["\'])' + escaped + '\\1[^>]*>');
  const match = String(content || '').match(wrapperRe);
  if (!match) return null;
  const tag = match[0];
  if (readHtmlAttr(tag, 'data-pankit-preview') !== 'source-shadow') return null;
  const sourceFile = readHtmlAttr(tag, 'data-pankit-source-file');
  const sourceStartLine = Number(readHtmlAttr(tag, 'data-pankit-source-start'));
  const sourceEndLine = Number(readHtmlAttr(tag, 'data-pankit-source-end'));
...
function findMarkerBlock(id, lines) {
  let start = -1;
  let end = -1;
  const startPattern = 'pankit-variants-start ' + id;
  const endPattern = 'pankit-variants-end ' + id;
 ...
 * the `<div data-pankit-variants="ID">` outer wrapper so the picked
 ...
 * (with `data-pankit-variants="ID"`) and its matching `</div>`. We
...
function expandReplaceRange(block, lines, isJsx) {
  ...
  // Walk back for the wrapper `<div data-pankit-variants="..."` opener.
...
function isVariantEndMarkerLine(line, id) {
  return new RegExp('pankit-variants-end\\s+' + escapeRegExp(id) + '(?:\\s|--|\\*/|$)').test(line);
}

function hasVariantWrapperAttr(line, id) {
  const escaped = escapeRegExp(id);
  return new RegExp(`data-pankit-variants\\s*=\\s*(?:"${escaped}"|'${escaped}'|\\{["']${escaped}["']\\})`).test(line);
}
 ...
 *   - CSS `@scope ([data-pankit-variant="N"])` strings that look like the
...
function extractOriginal(lines, block) {
  ...
  const inner = extractInnerByAttr(text, 'data-pankit-variant="original"');
...
function extractVariant(lines, block, variantNum) {
  const text = stripStyleAndJoin(lines, block);
  const inner = extractInnerByAttr(text, 'data-pankit-variant="' + variantNum + '"');
 ...
 *
 * Handles three shapes of `<style data-pankit-css="ID" ...>`:
...
function extractCss(lines, block, id) {
  const styleAttr = 'data-pankit-css="' + id + '"';
  let inStyle = false;
  const content = [];
...
function findSessionFile(id, cwd) {
  const result = findSourceFile({
    query: 'pankit-variants-start ' + id,
    cwd,
