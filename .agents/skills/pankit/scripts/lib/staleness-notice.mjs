...
function cachePath() {
  return process.env.PANKIT_STALENESS_CACHE
    || path.join(os.homedir(), '.pankit', 'staleness-check.json');
}
 ...
 * the file accumulate one entry per directory PanKit has ever booted in
...
/**
 * Opt out with PANKIT_NO_STALENESS_CHECK=1 or `"stalenessCheck": false` in
 * .pankit/config.json. Local config overrides shared, matching how
 * updateCheck resolves.
 */
export function stalenessCheckDisabled(roots = [process.cwd()]) {
  if (process.env.PANKIT_NO_STALENESS_CHECK) return true;
  let value;
...
export function buildStalenessDirective(findings) {
  ...
  const lines = [
    ...
    "PanKit's own project files have drifted from what this version reads. "
