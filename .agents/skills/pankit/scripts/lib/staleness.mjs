/**
 * Staleness detection for PanKit's own project artifacts: PRODUCT.md,
 * DESIGN.md and its `.pankit/design.json` sidecar, `.pankit/config.json`,
 ...
 *   2. Schema drift. An artifact was written by an older PanKit: fields it
...
const DIRECTION_WORK_PATHS = Object.freeze([
  path.join('.pankit', 'surfaces'),
  path.join('.pankit', 'mocks', 'decision'),
]);
 ...
 * pankit-paths (which resolves the project root) and context.mjs (which
 * cannot import pankit-paths without a cycle) share one definition of
 * where the retired locations are.
 */
export function designSidecarCandidatesFor(projectRoot, contextDir = projectRoot) {
  const candidates = [
    path.join(projectRoot, '.pankit', 'design.json'),
 ...
 *
 * `sidecarCandidates` comes from pankit-paths' resolver so this module
...
export function checkConfig({ projectRoot, repoRoot }) {
  ...
  for (const root of roots) {
    for (const name of ['config.json', 'config.local.json']) {
      const filePath = path.join(root, '.pankit', name);
...
export function checkBuildPathUnset({ projectRoot, repoRoot, product }) {
  ...
  return [finding({
    ...
    fix: 'Only when image generation exists in your tool surface, offer the choice once: '
      ...
      + 'Write the answer to `.pankit/config.json` as `"buildPath": "comp"` or `"buildPath": "code"`, '
 ...
 */
export function checkProjectRoots({ patterns = [], candidates = [], configuredIn = '.pankit/config.json' }) {
