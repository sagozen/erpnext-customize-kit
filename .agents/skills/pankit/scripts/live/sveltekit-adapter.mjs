...
export const SVELTE_LIVE_ROOT_COMPONENT = 'src/lib/pankit/PanKitLiveRoot.svelte';
export const SVELTE_LAYOUT_MARKER_OPEN = '<!-- pankit-live-svelte-start -->';
export const SVELTE_LAYOUT_MARKER_CLOSE = '<!-- pankit-live-svelte-end -->';
export const SVELTE_ROOT_IMPORT = "import PanKitLiveRoot from '$lib/pankit/PanKitLiveRoot.svelte';";
...
const SVELTE_ROOT_IMPORT_LINE_RE = /^[ \t]*import PanKitLiveRoot from '\$lib\/pankit\/PanKitLiveRoot\.svelte(?:\?[^']*)?';[ \t]*\r?\n?/gm;
...
export function svelteRootImportLine(rev) {
  ...
  return "import PanKitLiveRoot from '$lib/pankit/PanKitLiveRoot.svelte?pankit-live=" + rev + "';";
}
...
export function patchSvelteLayout(content, { rev = null } = {}) {
  ...
  if (!out.includes(importLine)) {
    ...
    // indentation; only a layout with no pankit import gets an insert.
    let replaced = false;
...
export function buildSvelteLiveRootComponent(port, token) {
  ...
  const HOST_ID = 'pankit-live-root';

  onMount(() => {
    let host = document.querySelector('pankit-live-root#' + HOST_ID) || document.getElementById(HOST_ID);
    if (!host) {
      host = document.createElement('pankit-live-root');
      host.id = HOST_ID;
      document.body.appendChild(host);
    }

    host.dataset.pankitLiveAdapter = 'sveltekit';
    ...
    if (!root.querySelector('style[data-pankit-live-reset]')) {
      const reset = document.createElement('style');
      reset.dataset.pankitLiveReset = 'true';
      reset.textContent = ':host, :host *, * { box-sizing: border-box; }';
      root.appendChild(reset);
    }

    window.__PANKIT_LIVE_ADAPTER__ = 'sveltekit';
    window.__PANKIT_LIVE_UI_ROOT__ = root;
    window.__PANKIT_LIVE_CHROME_MOUNT__ = {
      adapter: 'sveltekit',
      version: 1,
      host,
      root,
    ...
    script.src = LIVE_URL;
    script.async = true;
    script.dataset.pankitLiveScript = 'true';
    script.onerror = () => console.error(
      '[pankit] live.js failed to load from ' + LIVE_URL
    ...
    return () => {
      script.remove();
      if (window.__PANKIT_LIVE_UI_ROOT__ === root) delete window.__PANKIT_LIVE_UI_ROOT__;
      if (window.__PANKIT_LIVE_CHROME_MOUNT__?.root === root) delete window.__PANKIT_LIVE_CHROME_MOUNT__;
      if (window.__PANKIT_LIVE_ADAPTER__ === 'sveltekit') delete window.__PANKIT_LIVE_ADAPTER__;
    };
  });
</script>
`;
