 ...
 *
 * Variants are real .svelte components under node_modules/.pankit-live/<session-id>/.
...
// node_modules, so an .pankit/ tree under the app root 403s (verified
...
export const SVELTE_COMPONENT_ROOT = 'node_modules/.pankit-live';
// A short-lived interim location; swept so no project keeps a stray tree.
export const LEGACY_SVELTE_COMPONENT_ROOT = '.pankit/live/previews';
...
export const DEFERRED_ACCEPTS_FILE = '.pankit/live/deferred-svelte-component-accepts.json';

const MUSTACHE_RE = /\{([^{}]+)\}/g;

export function shouldUseSvelteComponentInjection(filePath) {
  if (/^(0|false|no)$/i.test(process.env.PANKIT_LIVE_SVELTE_COMPONENT || '')) return false;
...
export function ensureRuntimeHelper(cwd = process.cwd()) {
  ...
  if (!fs.existsSync(probe)) {
    fs.writeFileSync(probe, `export const pankitLivePreviewProbe = true;\n`, 'utf-8');
  }
  return file;
}
...
function buildInsertVariantStub(variantNum) {
  return `${buildPropsScript([])}<div class="pankit-insert-preview">Insert variant ${variantNum}</div>\n\n<style>\n  .pankit-insert-preview { display: block; }\n</style>\n`;
}
...
function sanitizeAcceptedSvelteCss(cssLines, variantNum, paramValues = null, rootTag = 'div') {
  ...
  if (!/data-pankit-variant|pankit-variant-ready/.test(css)) return cssLines;
...
function appendSanitizedCssRule(output, rule, variantNum, paramValues, rootTag) {
  ...
  if (!prelude || !body || /--pankit-variant-ready\s*:/.test(body)) return;

  if (/^@scope\b/i.test(prelude)) {
    if (/data-pankit-variant/.test(prelude) && !selectorHasVariant(prelude, variantNum)) return;
    ...
    for (const innerRule of inner) {
      ...
      if (!rewrittenPrelude || /--pankit-variant-ready\s*:/.test(innerRule.body)) continue;
...
function variantSelectorRegex(variantNum) {
  return new RegExp(`\\[data-pankit-variant=(["'])${escapeRegExp(String(variantNum))}\\1\\]`, 'g');
}
...
export function inlineSvelteComponentAccept(manifest, variantNum, paramValues = null, cwd = process.cwd()) {
  ...
  if (/data-pankit-variant|pankit-variant-ready/.test(variantCss)) {
...
function inlineSvelteComponentInsertAccept({
  ...
  if (/\bdata-pankit-[\w-]*\s*=/.test(markup)) {
    return { handled: false, error: 'Accepted Svelte insert variant contains preview-only data-pankit attributes', ...resultBase };
  }
...
/**
 * Stop-path sweep. The whole `node_modules/.pankit-live` tree is
 * pankit-owned and gitignored, so once no session should survive there is
...
export function deferredAcceptsPath(cwd = process.cwd()) {
  ...
  return path.join(os.tmpdir(), 'pankit-live', key, 'deferred-svelte-component-accepts.json');
}
...
export function buildSvelteComponentCssAuthoring(count) {
  ...
  return {
    ...
    requirements: [
      ...
      'Declare params in componentDir/params.json keyed by variant number (e.g. {"1": [...], "2": [...]}), NOT as a data-pankit-params attribute.',
      'Do not use @scope or data-pankit-variant selectors in component files.',
    ...
    forbidden: [
      ...
      'Do not add data-pankit-* attributes inside component files. Svelte parses { in attribute values as an expression, so data-pankit-params with JSON breaks the build; use componentDir/params.json instead.',
    ],
