 ...
 *     window.__PANKIT_LIVE_UI_SURFACES__ in the /live.js prelude.
 *   - skill/scripts/live-browser.js — publishes it on
 *     window.__PANKIT_LIVE_CHROME_CORE__ for adapters and E2E probes. That
 ...
 *   - the private pankit-site repo — site/components/LiveUiGallery.astro
...
export const LIVE_UI_PREFIX = 'pankit-live';

const id = (suffix) => `${LIVE_UI_PREFIX}-${suffix}`;

/**
 * The mount contract every Live chrome adapter (DOM, Svelte, ...) satisfies.
 * Published alongside the surfaces on __PANKIT_LIVE_CHROME_CORE__.
 */
