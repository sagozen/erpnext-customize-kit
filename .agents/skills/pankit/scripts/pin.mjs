 ...
 *
 * `pin audit` creates a lightweight audit skill that redirects to PanKit's audit workflow.
...
const PIN_MARKER = '<!-- pankit-pinned-skill -->';
...
/**
 * Find harness skill directories that have an pankit skill installed.
 */
function findHarnessDirs(projectRoot) {
  const dirs = [];
  for (const harness of HARNESS_DIRS) {
    const skillsDir = join(projectRoot, harness, 'skills');
    // Only pin in harness dirs that already have pankit installed
    const pankitDir = join(skillsDir, 'pankit');
    if (existsSync(pankitDir) || existsSync(join(skillsDir, 'i-pankit'))) {
      dirs.push(skillsDir);
    }
  }
  return dirs;
...
function generatePinnedSkill(command, metadata, commandPrefix, isCodex) {
  const desc = metadata[command]?.description || `Shortcut for ${commandPrefix}pankit ${command}.`;
...
This is a pinned shortcut for \`${commandPrefix}pankit ${command}\`.

Invoke ${commandPrefix}pankit ${command}, passing along any arguments provided here, and follow its instructions.
`;
}
...
function pin(command, projectRoot) {
  ...
  if (harnessDirs.length === 0) {
    console.log('No harness directories with pankit installed found.');
    return false;
  }
...
function unpin(command, projectRoot) {
  ...
  if (removed > 0) {
    ...
    console.log(`Use PanKit's '${command}' workflow directly to access it.`);
  } else {
