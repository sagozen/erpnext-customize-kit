#!/usr/bin/env node
/**
 * The PanKit hooks command manages the design hook runtime
 * via the `hook` key and shared detector ignores via the `detector` key in
 * .pankit/config.json / .pankit/config.local.json.
 *
 * Usage:
...
import { PANKIT_COMMAND } from './lib/provider.mjs';
...
const HOOK_MANIFEST_TARGETS = [
  {
    provider: '.claude',
    skillRel: '.claude/skills/pankit',
    destRel: '.claude/settings.local.json',
    sharedDestRel: '.claude/settings.json',
    manifest: () => ({
      description: 'PanKit design detector: immediate-tier checks after Edit/Write on UI files, full-rule deep pass on Stop.',
      hooks: {
        PostToolUse: [
          {
            matcher: 'Edit|Write',
            hooks: [
              {
                type: 'command',
                command: 'node "${CLAUDE_PROJECT_DIR}/.claude/skills/pankit/scripts/hook.mjs"',
        ...
        Stop: [stopManifestEntry('node "${CLAUDE_PROJECT_DIR}/.claude/skills/pankit/scripts/hook.mjs"')],
      },
    }),
  },
  {
    provider: '.agents',
    skillRel: '.agents/skills/pankit',
    ...
    manifest: () => ({
      hooks: {
        PostToolUse: [
          {
            ...
            hooks: [
              {
                type: 'command',
                command: 'node ".agents/skills/pankit/scripts/hook.mjs"',
                timeout: TIMEOUT_SECONDS,
                statusMessage: STATUS_MESSAGE,
              },
            ],
        ...
        Stop: [stopManifestEntry('node ".agents/skills/pankit/scripts/hook.mjs"')],
      },
    }),
  },
  {
    provider: '.cursor',
    skillRel: '.cursor/skills/pankit',
  ...
  },
  {
    ...
    provider: '.github',
    skillRel: '.github/skills/pankit',
    destRel: '.github/hooks/pankit.json',
    manifest: () => ({
      version: 1,
      hooks: {
        postToolUse: [
          {
            ...
            bash: 'node "$(git rev-parse --show-toplevel)/.github/skills/pankit/scripts/hook.mjs"',
...
function writeHookConfig(cwd, hookConfig, opts = {}) {
  ...
  // (consent, quiet, auditLog) survive an PanKit hooks edit.
...
function statusReport(cwd) {
  ...
  const envKill = process.env.PANKIT_HOOK_DISABLED;
  const envState = envKill ? `PANKIT_HOOK_DISABLED=${envKill}` : 'unset';
  const cfgPath = path.relative(cwd, getConfigPath(cwd)) || '.pankit/config.json';
  const localPath = path.relative(cwd, getLocalConfigPath(cwd)) || '.pankit/config.local.json';
  const cachePath = path.relative(cwd, getCachePath(cwd)) || '.pankit/hook.cache.json';
  ...
  // `rule=value [files]` shape `pankit ignores list` already prints.
...
function fileHasPanKitHookMarker(filePath) {
...
function valueHasPanKitHookMarker(value) {
  if (typeof value === 'string') {
    return PANKIT_HOOK_COMMAND_MARKERS.some((marker) => value.includes(marker));
  }
  if (Array.isArray(value)) return value.some(valueHasPanKitHookMarker);
  if (value && typeof value === 'object') return Object.values(value).some(valueHasPanKitHookMarker);
  return false;
}
...
function addIgnoreRule(cwd, args) {
  ...
  if (!rule) throw new Error(`Pass a rule id, e.g. ${PANKIT_COMMAND} hooks ignore-rule side-tab`);
  if (rule === 'overused-font' && !parsed.allValues) {
    throw new Error(`overused-font is value-specific by default. Use ${PANKIT_COMMAND} hooks ignore-value overused-font <font> for a confirmed font, or ${PANKIT_COMMAND} hooks ignore-rule overused-font --all-values only when the user asked to ignore overused fonts generally.`);
  }
...
function addIgnoreFile(cwd, args) {
  ...
  if (!glob) throw new Error(`Pass a glob, e.g. ${PANKIT_COMMAND} hooks ignore-file "src/legacy/**"`);
...
function parseIgnoreValueArgs(args) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === '--shared') {
      ...
      // reported success. Matches `pankit ignores add-value`.
...
function addIgnoreValue(cwd, args) {
  ...
  if (!parsed.rule || !parsed.value) {
    throw new Error(`Pass a rule id and value, e.g. ${PANKIT_COMMAND} hooks ignore-value overused-font Inter`);
  }
  ...
  // `files` scope, so require one — matching `pankit ignores add-value`.
  if (parsed.value === '*' && parsed.files.length === 0) {
    ...
    const projectWide = parsed.rule === 'overused-font'
      ? `${PANKIT_COMMAND} hooks ignore-rule ${parsed.rule} --all-values`
      : `${PANKIT_COMMAND} hooks ignore-rule ${parsed.rule}`;
    throw new Error(`Wildcard value ignores must be scoped with --file <glob>, e.g. ${PANKIT_COMMAND} hooks ignore-value design-system-font-size "*" --file "src/widget.js". To suppress the rule project-wide use ${projectWide}.`);
  }
