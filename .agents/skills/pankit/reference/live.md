...
1. `live.mjs`: boot. If the request names or implies a file, route, or app inside a monorepo, infer the concrete path and run `node .agents/skills/pankit/scripts/live.mjs --target <path>` instead; then run the rest of this live session from the returned `projectRoot`. The boot resolves the app root from dev-server config files and persists it in `.pankit/live/roots.json`; every helper re-anchors to that manifest at startup (a wrong cwd cannot fork session state), PRODUCT.md / DESIGN.md are discovered upward to the git root, and relative helper args like `--file` resolve against the app root.
...
3. Poll loop with the default long timeout (600000 ms). Run `live-poll.mjs` again immediately after every event or `--reply`; Codex runs this one-shot poll in the foreground. Never pass a short `--timeout=`. The global bar's **PanKit mark** dims with a pulsing amber dot when nothing is polling `/poll`; restart `live-poll.mjs` to reconnect.
...
7. If interrupted, run `live-status.mjs` or `live-resume.mjs` before guessing. The journal under `.pankit/live/sessions/` is canonical and replays unacknowledged work after a helper restart; the injected `live.js` re-attaches when the page reopens. Fall back to the direct-edit loop only when `live-resume.mjs` reports no active session, never because disconnects felt frequent.
...
```
LOOP:
  node .agents/skills/pankit/scripts/live-poll.mjs   # default long timeout; no --timeout=
...
```bash
node .agents/skills/pankit/scripts/live.mjs
```
...
The append-only journal under `.pankit/live/sessions/` is canonical durable state (not project source). When the chat was interrupted, polling was missed, the helper restarted, or the browser reloaded:

```bash
node .agents/skills/pankit/scripts/live-status.mjs      # helper state, active sessions, queued events; works with the helper down
node .agents/skills/pankit/scripts/live-resume.mjs --id SESSION_ID   # active snapshot, pending event, next safe action
node .agents/skills/pankit/scripts/live-complete.mjs --id SESSION_ID # canonical manual final acknowledgement after verified cleanup
```
...
```bash
node .agents/skills/pankit/scripts/live-insert.mjs --id EVENT_ID --count EVENT_COUNT --position after \
  --element-id "ANCHOR_ID" --classes "class1,class2" --tag "section" --text "ANCHOR_TEXT"
```

`--position` ← `event.insert.position`; anchor flags map exactly like wrap's. The scaffold has **no** `data-pankit-variant="original"`; variants are net-new HTML+CSS at `insertLine`. On source-preview targets the scaffold carries `sourceWritten: false` with `wrapperBlock` and `replaceEndLine < replaceStartLine` (an insertion): splice variants into `wrapperBlock` at the marker and insert at `replaceStartLine` in ONE edit, exactly as the wrap section describes. Decide the visitor mode from the surface. Svelte targets follow the same component flow as wrap below (`mode: "insert"` in the manifest): each variant is a real single-root component under `componentDir` with no `data-pankit-*` attributes; never edit the route during generation; accept splices the chosen markup into `sourceFile` mechanically. For non-Svelte targets, accept/discard removes the wrapper; the anchor is untouched.
...
```bash
node .agents/skills/pankit/scripts/live-wrap.mjs --id EVENT_ID --count EVENT_COUNT --element-id "ELEMENT_ID" --classes "class1,class2" --tag "div" --text "TEXT_SNIPPET"
```
...
<style data-pankit-css="SESSION_ID">
...
<div data-pankit-variant="2" style="display: none">
  <!-- variant 2 -->
</div>
<div data-pankit-variant="3" style="display: none">
  <!-- variant 3 -->
</div>
```
...
**JSX / TSX targets:** wrap `<style>` content in a template literal (CSS braces would parse as JSX), use `className=` / `style={{…}}`, keep `data-pankit-*` attributes as plain strings:

```tsx
<style data-pankit-css="SESSION_ID">{`
  @scope ([data-pankit-variant="1"]) { ... }
`}</style>
<div data-pankit-variant="2" style={{ display: 'none' }}>
  {/* variant 2 */}
</div>
...
```html
<div data-pankit-variant="1" data-pankit-params='[
...
2. **Preview in the served file**: manually write the same wrapper scaffold `live-wrap.mjs` produces (`<!-- pankit-variants-start ID --><div data-pankit-variants="ID" data-pankit-variant-count="3" style="display: contents">…</div><!-- end -->`) into the file the browser actually loaded, insert your variant divs, `--reply EVENT_ID done --file <served file>`. This edit is temporary; a regen wiping it is fine.
...
1. **Locate the carbonize block** in `_acceptResult.file`: bracketed by `<!-- pankit-carbonize-start/end SESSION_ID -->` with a `<style data-pankit-css>` element; read the `<!-- pankit-param-values -->` comment first when present, it drives steps 3 and 4.
2. **Move the CSS rules** into the project's real stylesheet (whichever already owns styling for the surrounding element).
3. **Bake param values while rewriting selectors**: retarget `@scope ([data-pankit-variant="N"])` to real semantic classes; keep only the `:scope[data-p-<id>="VALUE"]` branch matching the chosen value; substitute `var(--p-<id>)` literals or update the var's default.
4. **Unwrap the accepted content**: delete the inner variant div (and on JSX the outer `data-pankit-carbonize` div); drop `data-pankit-params` and all `data-p-*` attributes.
...
When native subagents are available, delegate source edits to `pankit_manual_edit_applier` / `pankit-manual-edit-applier`. Pass cwd, scripts path, event id, page URL, chunk/deadline, `batch`, `evidencePath`, and the canonical JSON result schema. The subagent must not poll or reply. If unavailable, apply inline with the same contract.
...
```bash
node .agents/skills/pankit/scripts/live-server.mjs stop
```

Stops the helper and runs `live-inject.mjs --remove` to strip the injected script (use `stop --keep-inject` to keep it for a quick restart; `.pankit/live/config.json` persists as project config). Then search for and remove any leftover `pankit-variants-start` wrappers and `pankit-carbonize-start` blocks.
