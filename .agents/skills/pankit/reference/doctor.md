...
```
node .agents/skills/pankit/scripts/doctor.mjs --json
```
...
- **`auto`** carries no decision. Run `node .agents/skills/pankit/scripts/doctor.mjs --fix` once to apply these, then report what it moved in one line. Do not ask permission first, and do not ask about them afterward.
